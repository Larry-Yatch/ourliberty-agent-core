...
- **Currently using:** Tier 1 = `agent.beacon.ourliberty@gmail.com` (droplet primary, since 2026-05-26); Tier 2 = Larry's personal Max OAuth via HOME-swap at `/home/larry/.claude-larry-personal/` (fallback per PR #118). Both Tier-1 bots (Beacon, Forge, Mirror, Pulse) and the inbox watcher share Tier 1; the Tier-2 fallback fires only on Tier-1 rate-limit or auth-401. See Beacon memory file `reference_droplet_tier1_tier2_oauth` for canonical Tier architecture.
...
## Chain discipline v3 (2026-05-26)

Closes four silent-failure modes surfaced across the 2026-05-25/26 incident window. Same pattern as v2: each gap gets a direct fix at the failure site AND (where applicable) a healer check for defense in depth. Additive — Checks 1-5 in `heal_pipeline_stall.py`, the existing REVIEW_REVISION DM path, and the AUTO_MERGE happy path are untouched.

### What shipped

- **`scripts/outbox_notifier.py`** — REVIEW_REVISION no-session WARN fallthrough now broadcasts a `larry_alerts.append_alert` instead of silently logging. GAP 1: when `routing_source != 'larry'` or `reply_chat_id` is missing, the chat-targeted `_dm_larry_no_session_revision` can't fire; the broadcast alert ensures Larry sees the rejection on the next bot sweep.
- **`scripts/outbox_notifier.py`** — Mirror's `pr_url` is now validated against an allowlist (`Larry-Yatch/ourliberty-agent-core`, `Larry-Yatch/ourliberty-dashboard`) BEFORE any auto-merge gate or shell-out. On mismatch, a single canonical-form rewrite attempts to fix known-wrong owner prefixes (`lyatch/`, `LarryYatch/`, `larry-yatch/`) and repo-name stripping (`agent-core` → `ourliberty-agent-core`). On rewrite success, logged at INFO `PR_URL_REWRITTEN` and the merge proceeds; on failure, `larry_alerts.append_alert` fires with the canonical-form guess + the merge is skipped entirely (no `gh pr merge` with an unverified URL). GAP 2.
- **`scripts/heal_pipeline_stall.py`** — Check 6 (`check_revision_dispatched_with_no_session`): scans `outbox-notifier.log` for the `no forge_build_session_id` WARN pattern and DMs Larry once per task with the 6h cooldown shared by Checks 1-5. Defense in depth for GAP 1.
- **`scripts/heal_pipeline_stall.py`** — Check 7 (`check_unrouted_open_prs`): cross-references OPEN PRs on tracked repos against `~/agents/logs/routing-events.jsonl`. For any PR older than 1h with no matching `beacon → mirror review-request` event, DMs Larry the manual-dispatch text. GAP 3 (externally-authored PRs — Claude-as-Forge, manual pushes — bypass the notifier's auto-dispatch entirely).
- **`agents/mirror/CLAUDE.md`** — explicit RIGHT/WRONG examples block alongside the existing marker.py mandate, covering all four observed wrong shapes: wrapper (PR #104), inline-prefix (PR #104), bare keyword (PR #107, 2026-05-26), task_id mismatch (PR #109, 2026-05-26). Each WRONG shape carries a one-sentence rationale. GAP 4.
- **`scripts/tests/test_mirror_marker_discipline.py`** — extended scanner with `_scan_text_for_bare_keyword` (verdict-position only; canonical-block presence suppresses false positives) and `_scan_text_for_task_id_mismatch` (conservative gates: task_id ≥8 chars, narrative ≥150 chars). Fixtures at `scripts/tests/fixtures/mirror_discipline_violations/{bare_keyword_pr107,task_id_mismatch_pr109}.jsonl` give the test concrete inputs. GAP 4 test extension.
- **`runbooks/heal-pipeline-stall.md`** — Check 6 + Check 7 sections (what each detects, how to manually unstick, how to silence false positives).

### Why each gap matters

- GAP 1 fired on `feedback_claude_as_forge_boundaries` today. The chain protocol assumes Forge owns the build session; when Claude authors a PR outside the dispatch chain (or when routing-source isn't `'larry'`), there's no session to `--resume` against. The old fallthrough WARNed and skipped silently; Larry only learned about the rejection if he was watching live.
- GAP 2 fired on PR #107 review. Mirror emitted `lyatch/agent-core` (missing 'L', stripped `ourliberty-` prefix) in `pr_url`; AUTO_MERGE couldn't parse it as a tracked repo. The canonical-form rewrite at the call site catches the known-wrong shapes; the allowlist ensures no future Mirror output mangles an attacker-influenced URL into a `gh pr merge` shell-out.
- GAP 3 reflects an architectural assumption: the notifier's review-request auto-dispatch fires when Forge opens a PR via the build-phase pipeline. PRs opened outside that pipeline (Claude-as-Forge, manual pushes) never auto-dispatch. Check 7 surfaces the gap so Larry knows to manually route.
- GAP 4 added two new wrong-marker shapes to the discipline test on top of the two already covered. Bare REVIEW_PASS keyword fired on PR #107; task_id-mismatched JSON body fired on PR #109. Both shapes dead-letter silently through the parser.

### Discipline boundaries (non-negotiable)

- **No new alert severity levels.** All GAP-1 broadcast + GAP-2 unrewritable + Check-6 + Check-7 alerts use the existing `'warning'` severity vocabulary.
- **Subject keys on task_id (or PR number).** Per-subject cooldown is 60 min for warnings; different mangled PRs don't suppress each other.
- **PR_URL_REWRITTEN is INFO not WARN.** The rewrite is a known-typo class auto-correction, not an alert-worthy event. The log line carries enough forensic detail (`from=` + `to=` + `task=` + reason) for grep-based investigation.
- **No overlap with the AUTO_MERGE serializer.** This work touches the REVIEW_REVISION handler (GAP 1) and pr_url validation (GAP 2) only; `_auto_merge_pr`, `_attempt_auto_merge_with_gates`, and the queue/release machinery are untouched.

### Codified additions worth recalling next session

59. **Claude-as-Forge boundary needs a broadcast fallback.** When REVIEW_REVISION fires on an envelope with no `forge_build_session_id`, the chain-targeted `_dm_larry_no_session_revision` only works for `source='larry'` with a `reply_chat_id`. For Beacon-dispatched revisions OR cases where chat_id propagation failed, the rejection used to be silent. The fix is `larry_alerts.append_alert` (broadcast) in the fallthrough — same severity vocabulary, subject keyed on task_id. Pattern for future protocol-violation paths: when the chain protocol assumes X (Forge owns the build session) and X is violated, the action is "tell Larry," not "log WARN and hope someone notices."
60. **PR URL validation belongs upstream of every gh shell-out.** Mirror's `pr_url` is attacker-influenceable (a future prompt-injection path could attempt to redirect the merge to a non-tracked repo). The allowlist + canonical-form rewrite gate runs BEFORE both serializer gates AND before `_parse_pr_url` for the same reason `_GH_PR_URL_RE` is anchored to start-of-string: never let an unverified URL reach `gh pr merge`. Pattern: any new external-input value that flows to a shell-out gets a single-source-of-truth allowlist; failures DM Larry with the canonical-form guess so the auto-correction is auditable.
61. **Externally-authored PRs need a healer.** The notifier's auto-dispatch only fires when Forge opens a PR via the build-phase pipeline (Forge's CLAUDE.md emits `PR opened: <url>` in the build result; the notifier scrapes it). Claude-as-Forge PRs and manual pushes bypass that path entirely — they're never auto-routed. Check 7 in `heal_pipeline_stall.py` watches `~/agents/logs/routing-events.jsonl` for matching `beacon → mirror review-request` events; absence past 1h DMs Larry the manual dispatch text. Same posture as the other Checks: never act on the stall, surface only.
62. **Marker discipline is a moving target — add WRONG examples as you observe them.** The marker.py mandate in `agents/mirror/CLAUDE.md` now lists four WRONG shapes (wrapper, inline-prefix, bare keyword, task_id mismatch). Each was observed in production; each dead-lettered silently. The test_mirror_marker_discipline.py scanner is forward-looking + print-only at the live-scanner level (LiveScannerWatcher), with hard-fail-on-fixture coverage for each named shape. Pattern for future marker-shape drift: if Mirror emits a NEW wrong shape, add the regex/heuristic + a fixture + a one-line CLAUDE.md rationale. The CLAUDE.md examples are documentation; the test fixtures are the regression guard.

## Healer read discipline (2026-05-26)

Closes two healer-design gaps that both share the same root cause: healers were reading from snapshots of state that aren't ground-truth-current, and acting on those snapshots. Five false positives on 2026-05-26 (three from historical log events long past resolution; two from a stale local checkout). The fix is canonical-source reads and bounded windows.

### What shipped

- **`scripts/heal_pipeline_stall.py`** — `SCAN_WINDOW_SECONDS = 86400` (24h) module-level constant. New `_within_scan_window()` helper gates every Check's stall-trigger event timestamp. Checks 1, 3, 4, 6 filter their log-line anchor ts; Checks 2 and 7 filter the PR `createdAt`. Check 5 (journalctl retry-exhausted) is already bounded by `RETRY_EXHAUST_WINDOW_MIN = 30` — well inside 24h, no change. Events older than the window are skipped silently — historical record, not stalls.
- **`scripts/heal_credential_registry_drift.py`** — registry reads switched from working-tree to canonical-source. New `load_registry_from_origin()` runs `git fetch origin main --quiet` + `git show origin/main:config/token-rotation-schedule.json` (30s timeout each, no shell interpolation, `subprocess.run` with explicit `cwd`-equivalent `-C` flag). New `resolve_registry(source, state)` orchestrator: `--source origin` (default) tries canonical, falls back to local with a single 6h-cooldown DM on fetch failure; `--source local` reads working tree directly with no fetch and no fallback DM (debugging mode).
- **`scripts/heal_credential_registry_drift.py`** — `--source local|origin` CLI flag via argparse. Default `origin`. Documented in `--help`.
- **`scripts/tests/test_heal_pipeline_stall.py`** — 10 new tests covering: constant value, `_within_scan_window` inclusive boundary + None handling, per-Check skip behavior for Checks 1, 2, 3, 4, 6, 7 with events outside the window, plus the in-window happy-path equivalent.
- **`scripts/tests/test_heal_credential_registry_drift.py`** — 5 new tests covering: origin happy path (subprocess args verified), fetch-fail fallback to local with single DM, fetch-fail DM cooldown suppression on second tick, `--source local` bypasses subprocess entirely, and the divergence test (origin/main has BAR registered, local doesn't — classification uses canonical so no drift; the dual `detect_drift` against the stale local registry documents the exact failure mode). 4 argparse tests for the CLI flag.
- **`runbooks/heal-pipeline-stall.md`** — new Scan window subsection: 24h default rationale, tuning conditions (long-running deployments, holiday gaps), tuning procedure (edit constant + restart timer), recovery path for the >24h-real-stall false-negative case.

### Why each gap matters

- The pipeline-stall healer had no event-age bound on its Check inputs. Three Mirror PASS markers from 2026-05-25 (PRs Larry had merged manually after the marker fired) kept tripping Check 3 on every 15-min tick of 2026-05-26, because the dedup cooldown (1h) had expired and the original event was still inside the 24h `LOG_LOOKBACK_HOURS` window. The fix retires events past `SCAN_WINDOW_SECONDS` from the alerting set entirely — at that age, another DM is noise, not signal. Larry hand-suppressed the dedup state at 10:42 MDT to stop the loop; the new constant makes the suppression structural.
- The credential-drift healer read `config/token-rotation-schedule.json` from the working tree. The droplet's `agent-core` checkout was on `larry/pulse-cycle-upgrade-spec`, not `main`; PR #113 had merged a new SUPABASE_DB_PASSWORD entry to main, but the local checkout didn't have it. The healer scanner saw the credential in `.env.larry` but no registry entry in its (stale) local read — `MISSING_REGISTRY_ENTRY` false positive, DMed Larry. Canonical reads against `origin/main` close this entirely.

### Discipline boundaries (non-negotiable)

- **No per-Check window override.** One global `SCAN_WINDOW_SECONDS` for V1. Per-Check tuning is YAGNI until a real operational reason emerges; the 24h default is defensible across all seven Checks.
- **Subprocess discipline.** All git invocations run with explicit list args (no `shell=True`), explicit `cwd` via `git -C`, fixed 30s timeout, and zero user-controlled input in the arg array (the only inputs are the module-level constants `REPO_ROOT` and `REGISTRY_REL_PATH`, neither of which is configurable). No injection surface.
- **Fallback DM is single-source-of-truth-aware.** The 6h-cooldown is keyed on `state['_meta']['fetch_fallback_last_dm']`, not the per-drift `state['drifts']` dict. The fetch fallback is an infrastructure event, not a credential drift; mixing them would let one stale per-drift cooldown suppress the fetch-fail signal.
- **`--source local` does NOT DM on fetch failure** (it doesn't fetch at all). This is intentional debugging mode; Mirror should verify that local-source runs never produce a fetch-fail DM as a regression check.

### Codified additions worth recalling next session

63. **Healer reads should be canonical-source, not local-checkout.** `heal_credential_registry_drift.py` reads `config/token-rotation-schedule.json` from `origin/main` (via `git fetch origin main` + `git show origin/main:<path>`), with a WARN + 6h-cooldown DM fallback to the local file on fetch failure. Two false positives on 2026-05-26 traced to a stale local checkout (PR #113 merged to main, droplet was on a feature branch — the merged registry entry looked like drift). The pattern generalizes: any healer reading repo-tracked config files should fetch + read from the canonical branch, not the working tree, to avoid false positives when the local checkout is on a feature branch. Sister discipline: `SCAN_WINDOW_SECONDS = 86400` in `heal_pipeline_stall.py` gates every Check's stall-trigger timestamp so historical events from already-resolved incidents are skipped silently. Both surfaces share one rule — read ground-truth-current state, not whatever the local snapshot happens to hold.

## Tier 2 quota+auth fallback (2026-05-26)

Closes two operational gaps surfaced today (2026-05-26):

1. **Bot + agent wrappers log `claude exit 1` with empty stderr.** Both rate-limit AND auth-401 messages from the Claude CLI go to stdout, not stderr. The 2026-05-26 11:05-11:09 MDT incident (Mirror's 5 review attempts on PR #116) surfaced as `'You've hit your limit · resets 11:30am'` on stdout; the chain misdiagnosed the auth-401 expiry as a rate-limit and waited rather than re-authing.
2. **Chain had no fallback** when the agent Claude Max account hit either failure. Larry's personal Claude Max was the system's primary before the agent account; restoring it as Tier 2 via HOME-swap means one account being limited no longer blocks the other (separate accounts = separate quota + auth buckets).

### What shipped

- **`scripts/beacon_telegram_bot.py`** — `call_beacon` now logs BOTH stdout and stderr on non-zero exit, classifies the failure via `classify_tier1_failure` (regexes for `hit your limit` / `5-hour` / `resets \d+` and `401` / `Invalid authentication credentials` / `Failed to authenticate`), and on either match retries once with `HOME=/home/larry/.claude-larry-personal`. Tier 2 success returns the reply; Tier 2 unavailable or Tier 2 failure DMs Larry via `larry_alerts.append_alert` with severity=warning and subject keyed on failure type.
- **`scripts/agent_runner.py`** — same detection + fallback inside the existing `MAX_RETRIES` loop. Resume-discipline rule (non-negotiable): `--resume <session_id>` sessions are NOT portable between accounts. When the failing invocation carries `session_id`, the wrapper skips Tier 2 entirely, marks `paused_on_tier1` in the in-flight state file, DMs Larry, and exits the loop terminally (further retries would hit the same wall). For non-resume tasks, Tier 2 attempt runs once per failed Tier 1 attempt; success replaces the result and the happy path continues.
- **`docs/runbooks/restore-larry-personal-claude-oauth-tier2.md`** (NEW) — end-to-end runbook for installing Tier 2 OAuth at `/home/larry/.claude-larry-personal/`, including the wrong-account-in-browser gotcha (Google's default-account behavior can silently land credentials in the agent account), the headless OAuth orchestrator pattern, and the annual scope-audit pointer.
- **`config/token-rotation-schedule.json`** — new `LARRY_PERSONAL_CLAUDE_MAX_OAUTH_TIER2` entry (full 4-artifact discipline: registry + runbook + calendar event `n2846s75hkl5s07uh3t7emdsns` 2027-05-26 + drift-healer-aware). Existing `CLAUDE_MAX_OAUTH` entry gets `calendar_event_url` populated (was null — the E1.5 4-artifact gap exposed by today's silent expiry; calendar event `l0mj6gp5040itb3kk2au8t1f1s` 2027-05-26). New `known_storage_locations` entry for the Tier 2 path.
- **`scripts/heal_credential_registry_drift.py`** — `scan_claude_cli` disambiguates Tier 1 vs Tier 2 by file path. Tier 2 missing or malformed → `MISSING_CREDENTIAL` drift → DM via the existing 6h-cooldown discipline.
- **`scripts/heal_pipeline_stall.py`** — Check 8: scans `~/agents/logs/forge.log` + sibling agent logs (any file containing the `TIER2_FALLBACK_*` INFO/WARN lines) for FAILED Tier 2 attempts OR SKIPPED-DUE-TO-RESUME markers in the last `SCAN_WINDOW_SECONDS`. Same 6h-cooldown + single-DM idempotency as Checks 1-7.
- **`runbooks/heal-pipeline-stall.md`** — Check 8 section (detection / cause / action recipe / false-positive silencing).
- **`scripts/tests/test_tier2_fallback_wrapper.py`** (NEW) — 28 tests covering: classification regexes against the 2026-05-26 incident phrasing, Tier 2 filesystem probe, in-flight `paused_on_tier1` marker, `call_beacon` Tier 1→Tier 2 success / Tier 2 unavailable / Tier 2 also-failed branches, and the credential-drift healer's Tier 1 vs Tier 2 path disambiguation.

### Why each gap matters

- The stderr-only log gap is what made today's outage a 15-min RTO instead of a 30-second one. Without the stdout in the log, the chain couldn't tell rate-limit from auth-401 (both phrases live in the same stream); the watchdog timed out waiting for a "reset" that was never going to happen because the actual signal was an expired refresh token. The fix logs both streams and runs detection regexes against the combined output.
- The no-fallback gap meant any Tier 1 quota or auth event made the entire agent chain idle. With Tier 2 wired, a single-account failure now causes one retry on the personal account; the work either completes or Larry gets one DM with a recovery recipe. The chain self-heals on rate-limit (Tier 2 just works); auth-401 still requires Larry to refresh Tier 1 OAuth, but Tier 2 keeps the chain alive while he does.
- The resume-discipline rule is the boundary condition Mirror should sanity-check: `--resume` session IDs live in the Claude CLI's per-account session store. A silent Tier 2 retry on a resume task would fail with "session not found" AND silently orphan whatever context the original session was carrying. The wrapper detects this upfront and DMs Larry rather than thrashing.

### Discipline boundaries (non-negotiable)

- **No new `larry_alerts` severity levels.** All Tier 2 alerts use the existing `'warning'` vocabulary. The `severity_if_lapsed='medium'` on the new registry entry is a different field (registry severity, separate enum that includes 'medium') — validator passes.
- **HOME-swap, not in-place env mutation.** Tier 2 retry runs with a `dict(env)` copy that has `HOME` overridden; we never modify `os.environ` or the parent process's env. Session isolation is structural — Tier 2's session store can't leak into Tier 1's directory or vice versa.
- **Detection is regex against combined stdout+stderr, not exit-code-only.** Some Tier 1 failures already returned non-zero exit; the gap was the detection layer, not the exit-code layer. Regexes are intentionally loose enough to survive minor CLI phrasing changes but tight enough to avoid false positives on success-shaped output.
- **No Tier 3.** If Tier 2 also fails (or is unavailable), Larry gets a DM and the chain stops on that work. Adding Tier 3 (a third account) would dilute the signal that something is wrong; the DM is the correct surface for "manual recovery required."

### Codified additions worth recalling next session

64. **Both wrappers detect quota+auth failures from STDOUT, not stderr.** The Claude CLI emits `'You've hit your limit'` (rate-limit) and `'401' / 'Invalid authentication credentials' / 'Failed to authenticate'` (auth-401) on stdout. Logging only stderr was the gap that made the 2026-05-26 incident a 15-min RTO. The new `classify_tier1_failure(stdout, stderr)` helper in both `beacon_telegram_bot.py` and `agent_runner.py` reads the combined output, picks rate-limit first when both match, and returns `None` on clean output. Pattern for any future subprocess-wrapper that reasons about an external CLI's failure modes: log BOTH streams unconditionally, and run detection against the combined output — never assume one stream over the other.
65. **Tier 2 fallback is HOME-swap, and `--resume` sessions skip it.** When Tier 1 (the agent account) hits rate-limit OR auth-401, both wrappers retry once with `HOME=/home/larry/.claude-larry-personal` (Larry's personal Claude Max). Separate accounts = separate quota + auth buckets. The resume-discipline rule is non-negotiable: `--resume` session IDs are not portable between accounts (each account has its own session store), so resume-mode tasks DM Larry instead of attempting a silent Tier 2 retry that would fail with "session not found." `heal_pipeline_stall.py`'s Check 8 surfaces both Tier-2-failed and Tier-2-skipped-due-to-resume cases via the same 6h-cooldown discipline. Pattern for future multi-tier auth fallbacks: per-tier credential isolation via HOME (not env-var splicing), upfront detection of "this attempt can't fall back" (resume, multi-step protocols, anything stateful), and a healer check that scans for "fallback was needed and didn't work" so silent failures still reach Larry.

## OAuth orchestrator promotion + heal-stale-daemon auto-restart (2026-05-26)

Closes two follow-on gaps from today's chain-discipline work:

1. **OAuth orchestrators lived in `/tmp`.** PR #118 recovered Tier 2 OAuth via headless device-code orchestrator scripts staged at `/tmp/auth_orchestrator.py` and `/tmp/auth_orchestrator_tier2.py`. `/tmp` is wiped between OAuth recovery sessions on the droplet, so the next time Tier 2 needs re-auth Larry would have had to recreate the scripts from runbook memory. PR #119 promoted both to canonical paths in `scripts/`. This PR closes the runbook-and-operating-manual half of that work (path updates in the Tier 2 runbook, registry rotation-date freshness, line 798 correction).
2. **`heal_stale_daemon_code.py` was DM-only.** The healer detects systemd daemons running pre-merge code (script mtime > `ActiveEnterTimestamp` by > 5 min). Twice today (after PR #114 serializer merge and after PR #118 Tier 2 wrapper merge) the healer DMed Larry and he manually ran `sudo systemctl restart`. The manual-restart loop is friction the healer should absorb; this PR adds non-interactive auto-restart inside the existing per-unit detection loop with a 30-min per-unit restart cooldown to prevent loops.

### What shipped

- **`scripts/heal_stale_daemon_code.py`** — `check_unit` now auto-restarts on stale-detection inside the existing per-unit loop. New flow: if stale + outside the 30-min restart cooldown, run `sudo -n systemctl restart <unit>` (sudoers contract: `(ALL) NOPASSWD: ALL`); on rc==0 append a `severity='warning'` alert with subject `auto-restarted:<unit>` including unit name, script mtime, pre-restart `ActiveEnterTimestamp`, and (best-effort via `git log --since=<pre_restart_iso> -- <script_path>`) the PR(s) that landed the new code; on rc!=0 append a `severity='warning'` alert with subject `auto-restart-failed:<unit>` carrying stderr + the manual recovery command, and DO NOT RETRY (broken units don't get hammered). If a unit is still stale after the 30-min cooldown elapses (auto-restart failed silently or the script kept regressing), the healer DMs `<unit> still stale after auto-restart at <ts>; manual investigation needed` instead of restarting again. Existing 6h DM cooldown for plain stale-detection unchanged. Cooldowns share the existing `~/agents/state/heal-stale-daemon-code-cooldowns.json` state file (a `last_restart_ts` key alongside the existing `last_alert_ts`).
- **`scripts/tests/test_heal_stale_daemon_code.py`** — four new tests: auto-restart success path (assert restart args + DM body shape + PR list inference), restart failure path (assert auto-restart-failed DM with stderr + manual command + exactly one `subprocess.run` call = no retry), restart-cooldown suppresses second restart within 30 min, post-cooldown-still-stale escalation DM (no second restart attempt). Existing kill-switch test (file-based `~/agents/healers.disabled`) covers the new path because `main()`'s short-circuit is unchanged.
- **`runbooks/heal-stale-daemon-code.md`** — behavior section flipped from "DM-only, never restarts" to "auto-restarts + DMs." Documents the 30-min per-unit restart cooldown, the `sudo -n systemctl restart` invocation, the no-retry failure path, the post-cooldown-still-stale escalation DM, and that the existing `~/agents/healers.disabled` kill switch now disables BOTH detection AND restart (already true structurally since `main()` short-circuits before the per-unit loop).
- **`docs/operating-manual.md`** — line 798 corrected to reflect Tier 1 = agent account, Tier 2 = HOME-swap (was the historical pre-Tier-1 claim). Part II appended with this section.
- **`config/token-rotation-schedule.json`** — `CLAUDE_MAX_OAUTH.last_rotated_at` and `next_rotation_due` advanced to 2026-05-26 / 2027-05-26 (the entry's dates predated today's 11:47 MDT re-auth).
- **`docs/runbooks/restore-larry-personal-claude-oauth-tier2.md`** — three `/tmp/auth_orchestrator*.py` references updated to the canonical `scripts/` paths landed by PR #119.

### Why each gap matters

- **Auto-restart absorbs the manual-restart loop.** Today's two manual restarts (after PR #114 + PR #118) were each ~2 min of Larry's attention plus a context switch. The healer already runs every 30 min via timer; making it self-heal on the same trigger removes the friction without changing the detection surface. The 30-min per-unit cooldown is the explicit guardrail against restart loops on a chronically-broken unit — one restart attempt per cycle, max.
- **`sudo -n` is the right invocation under the agent's sudoers contract** (`(ALL) NOPASSWD: ALL`). Interactive `sudo` would block the systemd one-shot indefinitely; `-n` errors fast if a future sudoers change ever revokes passwordless access, surfacing as a clean `auto-restart-failed` DM rather than a hung healer.
- **No-retry on restart failure** is the standard healer pattern (mirror Check 8 in `heal_pipeline_stall.py`, the cred-drift healer, etc.). A failing `systemctl restart` means the unit is fundamentally wedged (unit file syntax, missing binary, port collision); hammering it adds noise without changing the outcome. The DM with stderr + manual recovery command IS the recovery surface.
- **Operating-manual line 798** was load-bearing for new-session orientation: it told an agent reading the manual cold that the droplet ran on Larry's personal account, when in reality it had run on `agent.beacon.ourliberty@gmail.com` since 2026-05-26. A wrong "currently using" line in the most-read doc in the repo is the kind of stale-state premise that Beacon's plan-synthesis discipline (Phase E1.2, 2026-05-26) is explicitly designed to catch upstream — fixing it here closes the loop.

### Discipline boundaries (non-negotiable)

- **No new `larry_alerts` severity levels.** All three new auto-restart DMs (success, failure, post-cooldown-still-stale) use the existing `'warning'` vocabulary. Subjects (`auto-restarted:<unit>`, `auto-restart-failed:<unit>`, `still-stale-after-restart:<unit>`) provide per-unit-per-shape dedup, which is what an `intent=` field would have done in the chat-notification shape.
- **Kill switch is the existing file `~/agents/healers.disabled`, not a new env var.** `main()`'s existing `kill_switch_active()` short-circuit runs before any per-unit work, so it disables BOTH detection AND restart for free — no plumbing added. Documented explicitly in the runbook so future readers don't add a parallel env-var switch.
- **Per-unit restart cooldown is keyed by unit name, not by check-cycle.** Two different stale units in one healer tick both restart in the same cycle (separate cooldown buckets); the same unit doesn't retry within 30 min. The cooldown clock starts at the moment of the restart attempt regardless of outcome, so a failed restart also gets the 30-min cooldown — preventing a loop on a wedged unit between healer ticks.
- **PR-list inference is best-effort, never load-bearing.** The DM body falls back to omitting the PR list if `git log` returns empty or errors. Mirror should verify that an empty/failing `git log` does not cause the DM to drop or the restart to abort.

### Codified additions worth recalling next session

66. **Auto-restart absorbs the manual-restart loop; per-unit cooldown is the loop-prevention.** `heal_stale_daemon_code.py` now runs `sudo -n systemctl restart <unit>` on stale-detection (script mtime > `ActiveEnterTimestamp` by > 5 min) inside the existing per-unit detection loop. Per-unit 30-min restart cooldown lives in the existing state file as `last_restart_ts`; same unit can't restart twice in one cycle, post-cooldown-still-stale escalates to a `<unit> still stale after auto-restart at <ts>; manual investigation needed` DM instead of a second restart attempt. Pattern for future "DM-only → auto-act" healer promotions: pin the cooldown to the action (not the alert), key it by the actor (here: unit name), and route failure to the SAME larry_alerts vocabulary the original detection used (no new severity levels). The kill switch (the file `~/agents/healers.disabled`) covers the new path for free if the short-circuit runs at the top of `main()`.
67. **OAuth orchestrator scripts ship in `scripts/`, not `/tmp/`.** PR #119 promoted `auth_orchestrator.py` (Tier 1 device-code recovery) and `auth_orchestrator_tier2.py` (Tier 2 HOME-swap variant) from `/tmp` (wiped between OAuth recovery sessions) to `scripts/`. Runbooks reference the canonical paths; `/tmp` references in operational docs are a sign the work isn't done yet. Pattern: anything a runbook tells the operator to invoke under stress (incident response, recovery, fallback) MUST live in a stable path that survives a reboot — never `/tmp`, never an unmounted volume, never a path that exists only after a successful build.

## Healer alert plain-language translation layer (2026-05-26, stopgap)

Closes one operational-ergonomics gap surfaced today: healer alerts had been DMing Larry as raw technical jargon (`'Unit X is shipped in the repo but not installed under /etc/systemd/system/'`) with no plain-language context or recommended action. After 10+ healer DMs in one session, the manual-interpretation cost was the actual ergonomic gap, not the detection itself.

### Why stopgap

The planned Pulse cycle upgrade (follow-up to PR #108) will include healer-alert triage as a core Pulse responsibility — Pulse will read healer alerts, prioritize them, and surface them in consolidated cycle output rather than each healer DMing Larry independently. When that ships, this translation layer becomes redundant and can be retired. The regression test (translation coverage as a CI gate) stays useful even after Pulse triage — it prevents new healers from shipping without operational-ergonomics review.

### What shipped

- **`config/alert-translations.json`** (NEW) — lookup table keyed by `(source, subject)`. Initial V1 coverage: the ten operator-facing subjects surfaced in today's session (forge-no-pr, marker-invisible, no-session-revision, retry-exhausted, tier2-fallback-failed-rate_limit, tier2-fallback-unavailable-auth_401, install-drift, credential-drift:MISSING_REGISTRY_ENTRY, chain-event-shipper-stale, auto-merge-queue-corrupt). Each entry carries `severity` (display-only label: URGENT / WARNING / INFO, decoupled from the producer's queue `severity` field), `plain_language_summary` (Larry-on-phone framing — what happened, why), and `recommended_action` (concrete command or path).
- **`scripts/larry_alerts.py`** — `format_dm()` now consults the translation table for alerts. On match: layered render (severity word as first line, then summary, then recommended action, then `---technical detail---` footer with the original raw body verbatim). On miss: original raw body PLUS a `'[no translation; needs entry in config/alert-translations.json or Pulse triage scope]'` footer — silence on unmatched is impossible. New `translate_alert(source, subject)` helper implements the lookup rule: exact match on `subject` first; if miss, strip trailing `:`-segments one at a time and retry (longest-prefix match). Source must match exactly. Translations are loaded once per process (module-level cache); the render path has no LLM calls or I/O beyond the initial cached read.
- **`scripts/tests/test_alert_translations.py`** (NEW) — CI gate enforcing translation coverage. `ast.walk` over every `scripts/*.py` extracts `(source, subject_literal_or_prefix)` from `larry_alerts.append_alert(...)` call sites; for each in-V1-scope hit, asserts a translation entry resolves under the longest-prefix lookup rule. Fail-forward: missing-translation hits name file:line + the missing key. Pre-existing producers that fall outside the V1 ten (watchdog, pulse, ledger, beacon-telegram-bot, plus heal_pipeline_stall's non-failure-mode subjects) are documented in `_OUT_OF_V1_SCOPE_SUBJECTS` so the test doesn't trip on them today; future PRs can promote them by adding entries to the JSON and removing the out-of-scope marker.
- **`docs/runbooks/add-new-healer-alert.md`** (NEW) — discipline doc: when adding a new healer (or extending one with a new subject) you MUST add a translation entry before merging, or the CI gate fails.

### Why each gap matters

- **Plain-language headlines decide where Larry drills.** Larry's `larry_plain_language_summaries.md` memory (2026-05-15) codified that status updates and bug reports should lead with plain language. Healer alerts were a glaring exception — they led with raw jargon because the format was set in Phase D3.5-prep before that discipline was articulated. Translating at render time (rather than rewriting every producer) is the minimum-change path.
- **Severity is display-only; the queue vocabulary doesn't move.** Operating-manual #65 (Tier 2 fallback discipline boundary) codified "No new larry_alerts severity levels" as non-negotiable. The translation layer respects that: `VALID_SEVERITIES` remains `('warning', 'critical')`; URGENT / WARNING / INFO are render-time display labels. URGENT is reserved for matched entries where operational urgency is high regardless of producer severity (e.g., chain-event-shipper-stale fires queue=warning but display=URGENT because the dashboard is now blind).
- **Fallback path is non-silent.** The `[no translation]` footer is the load-bearing piece — it ensures that when a new healer ships without an entry, Larry's DM still arrives AND has a clear signal that the message is in pre-translation shape. Combined with the CI gate, this gives two layers of catching the missing-entry case.

### Discipline boundaries (non-negotiable)

- **No producer-side changes.** The translation layer is purely a render-time concern in `format_dm()`. The ten existing producer call sites are unmodified. The `append_alert(source, severity, message, subject, suggested_action)` signature is unchanged. The producer-side `severity` vocabulary stays `('warning', 'critical')`.
- **Lookup is deterministic and offline.** Exact match → longest-prefix on `:` segments. No regex matching. No fuzzy matching. No LLM calls in the render path. The JSON is read once at module load and cached for the process lifetime.
- **No emoji in the new layered render.** The severity word on line 1 of a matched DM is plain text (URGENT / WARNING / INFO). Existing emoji elsewhere in the DM body (🚨 / ⚠ in the technical-detail footer, ✓ / 🛑 / ✗ / 📬 in notifications and approval requests) are untouched — "do not break existing alerts" is the operative constraint.
- **V1 scope is the ten subjects from today's session.** Pre-existing producers (watchdog disk/memory/cgroup, pulse/ledger reports, beacon-telegram-bot Tier 1/2 surface) are explicitly out-of-V1-scope. They render via the fallback path until a future PR adds entries. The CI gate's `_OUT_OF_V1_SCOPE_SUBJECTS` list documents this; adding a healer to the V1 scope is a deliberate act of promoting it to the table.

### Codified additions worth recalling next session

68. **Healer alert plain-language translation layer (2026-05-26, stopgap).** `config/alert-translations.json` is a render-time lookup table that turns raw healer-alert subjects (e.g., `pipeline-stall:forge-no-pr:wt-forge-foo`) into layered DM output (URGENT/WARNING/INFO header + plain-language summary + recommended action + technical-detail footer). Lookup is deterministic: exact match on `(source, subject)`, falling back to longest-prefix on `:` segments — implemented as `larry_alerts.translate_alert(source, subject)`. Display severity (URGENT/WARNING/INFO) is decoupled from the producer's queue severity (`warning`/`critical`) per the operating-manual #65 discipline boundary. Unmatched alerts get the raw body plus a `[no translation]` footer — silence on unmatched is structurally impossible. `scripts/tests/test_alert_translations.py` is a CI gate: it `ast.walk`s every producer file, extracts `(source, subject_literal_or_prefix)` from `append_alert(...)` call sites, and fails when an in-scope hit has no translation entry; out-of-scope producers (pre-V1 watchdog/pulse/ledger surfaces) are documented in `_OUT_OF_V1_SCOPE_SUBJECTS` so the test stays green until they're deliberately promoted. **When this retires:** the planned Pulse cycle upgrade (follow-up to PR #108) will own healer-alert triage and DM consolidation — translation layer becomes redundant and can be retired; the CI gate stays useful as an ergonomics review trigger for new healers. Pattern for similar render-time translation layers: cache the lookup table at module load (one read per process), make the fallback path non-silent (footer the operator can't miss), and pair the artifact with a CI gate that's deliberately permissive on out-of-scope hits so it doesn't fail-closed on every producer that hasn't been migrated.

## Multi-step build orchestrator — spec adoption + Beacon authoring discipline (PR-S1 of 4, 2026-05-26)

Adopts the spec for the multi-step build orchestrator workstream — the design pattern Beacon uses to dispatch multi-PR builds (4-PR sequences like E4.4d, the Pulse cycle upgrade PR-α₁/α₂/β/γ, and this orchestrator itself) as hands-free sequences rather than one-paste-per-step manual relays. PR-S1 is the doc-only foundation: spec adoption + Beacon CLAUDE.md authoring discipline + this Part II entry. The runtime (advancer daemon, validator, dashboard ladder, Beacon shortcuts) lands in PR-S2 → PR-S3 → PR-S4 strictly sequential.

### Attribution (read carefully — this is not a PR-#-attributable workstream's first appearance)

The orchestrator spec was **direct-committed to main, not via a PR.** Larry authored it in his Claude session as part of the 2026-05-26 conversational design pass and committed it directly:

- Initial commit: `e097de9` — `docs(specs): pulse cycle upgrade design-pass brief + build-sequence orchestrator spec draft` (2026-05-26).
- Revision commit: `84d149b` — `docs(spec): fix internal-consistency bug in orchestrator spec § 5.2` (2026-05-26, same day; ties the `validate_no_concurrent_active()` helper reference in § 5.2 to its actual ship vehicle `scripts/build_sequence_validator.py` in § 6 PR-S2).

**Do NOT attribute this workstream to PR #128.** PR #128 is the unrelated e4-4e Approvals tab spec — a completely different workstream that happened to merge on the same day. The orchestrator workstream's first PR is THIS one (PR-S1, spec adoption + Beacon CLAUDE.md authoring discipline + Part II entry); subsequent PRs are S2/S3/S4 as outlined in spec § 6.

### What shipped

- **`agents/beacon/specs/build-sequence-orchestrator.md`** — adopted as canonical (no body rewrite). PR-S1's only edit to the spec is the addition of a reciprocal cross-reference: a one-line pointer from § 1 (Problem statement) and the Companion docs header line TO `agents/beacon/specs/pulse-cycle-upgrade.md` § 12.5 (which contains the corresponding pulse → orchestrator hand-off statement, landed via PR #130).
- **`agents/beacon/CLAUDE.md`** — appended new section `## How you author multi-step build sequences` transcribing spec § 5.5's three disciplines (spec-doc-first authoring; sequence file synthesis; Mirror preflight DAG verification) verbatim. Placed between the existing "How you dispatch work to Forge — the APPROVAL_REQUEST marker (Phase D3)" section and the "How you handle Forge's preflight markers" section — adjacent to PLAN_SYNTHESIS_DISCIPLINE and DIAGNOSTIC BASH as a related authoring-discipline cluster.
- **`docs/operating-manual.md`** — Part II appended with this entry (codified item #69 below).

### Explicit future-PR-gate markers on aspirational disciplines

Two disciplines in the new Beacon CLAUDE.md section reference capabilities that PR-S1 does NOT ship. Both carry explicit parenthetical markers identifying the future PR that makes them live:

- Discipline 2's reference to `python3 scripts/build_sequence_validator.py validate <seq-id>` is parenthetically marked **"live once PR-S2 ships the validator; this CLAUDE.md entry documents the discipline in advance for forward consistency."** PR-S2 ships the validator script per spec § 6 PR-S2 manifest. Until PR-S2 merges, hand-authored sequence files are unvalidated.
- Discipline 3's reference to Mirror's `review-sequence-dag` capability is parenthetically marked **"live once PR-S4 ships Mirror's DAG-verify capability; documented now so PR-S2/PR-S3 authoring matches the eventual contract."** PR-S4 ships the `agents/mirror/CLAUDE.md` addition teaching Mirror to recognize and execute the four-check verification. Until PR-S4 merges, Mirror does not interpret `review-sequence-dag` prompts.

Similarly, the six new Beacon shortcuts (`approve sequence` / `pause sequence` / `resume sequence` / `cancel sequence` / `retry sequence ... step` / `skip sequence ... step`) are documented in the new section as **landing in PR-S4** — they appear in advance so PR-S2/PR-S3 authoring matches the eventual contract.

### Why each gap matters

- **Direct-commit attribution prevents PR-attribution drift.** The orchestrator spec did not flow through the standard APPROVAL_REQUEST → Forge → PR path because Larry authored it himself in a real-time conversational design pass. The historical record needs to reflect that reality. Attributing the orchestrator workstream to "PR #128" (a same-day unrelated merge) would mislead any future agent or operator reading the operating manual cold; the workstream's PR sequence starts cleanly with PR-S1.
- **Future-PR-gate markers prevent false-positive discipline assertions.** The new CLAUDE.md section deliberately documents PR-S2 and PR-S4 capabilities in advance to lock the eventual contract. Without explicit "(live once PR-S2 ships)" / "(live once PR-S4 ships)" markers, a future-Beacon reading the section would interpret the disciplines as currently-live and either (a) call the missing validator and crash, or (b) emit a `review-sequence-dag` prompt that Mirror cannot interpret. The marker phrasing makes the aspirational-vs-live boundary explicit per § 5.5 of the spec — the section composes with, rather than overrides, the existing dispatch discipline.
- **Reciprocal cross-references between co-evolved specs.** The orchestrator spec and the Pulse cycle upgrade spec emerged from the same 2026-05-26 conversational design pass. The Pulse spec's § 12.5 (landed via PR #130) hands the multi-step orchestration scope off to the orchestrator spec; this PR adds the reciprocal pointer FROM the orchestrator spec TO that exact § 12.5. Future readers entering either spec land in a complete two-way reference graph rather than one direction of pointer.

### Discipline boundaries (non-negotiable)

- **PR-S1 is doc-only.** Diff is exclusively three files: the spec, Beacon's CLAUDE.md, and this operating-manual entry. No `scripts/` changes, no systemd unit configuration, no test additions — those land with the runtime in PR-S2 onward.
- **The spec body is locked.** PR-S1 edits to `agents/beacon/specs/build-sequence-orchestrator.md` are surgical: a reciprocal cross-reference addition only. No re-tuning of decisions A–J in § 2, no scope changes to § 4 out-of-scope, no architecture changes to § 5. If a § 5.2 inconsistency had remained after `84d149b`, the fix would have shipped in this same PR per spec-faithful-capture discipline; preflight verified § 5.2 is internally consistent (the validate_no_concurrent_active() reference resolves cleanly to PR-S2's `scripts/build_sequence_validator.py`).
- **Verbatim transcription of § 5.5 disciplines.** The new Beacon CLAUDE.md section transcribes spec § 5.5's discipline language verbatim per `spec_faithful_capture_of_locked_decisions` doctrine — only the future-PR-gate parenthetical markers and the surrounding "composition with existing disciplines" framing are PR-S1-additive prose. No paraphrasing of the canonical sub-discipline wording.
- **No modification to PR #130's `pulse-cycle-upgrade.md § 12` or PR #128's e4-4e spec.** Both are unrelated workstreams from PR-S1's perspective; the orchestrator → pulse cross-reference is the only edit that touches another workstream's referenced surface, and it points AT § 12.5 without modifying it.

### Codified additions worth recalling next session

69. **Multi-step build orchestrator workstream — spec adopted via direct commit, not PR; PR-S1 is doc-only (2026-05-26).** Spec at `agents/beacon/specs/build-sequence-orchestrator.md` direct-committed at sha `e097de9` (initial) + `84d149b` (§ 5.2 internal-consistency fix tying `validate_no_concurrent_active()` to PR-S2's `scripts/build_sequence_validator.py`). **NOT attributable to PR #128** — that's the unrelated e4-4e Approvals tab spec that merged the same day. Workstream PR sequence starts with PR-S1 (this PR: spec adoption + Beacon CLAUDE.md authoring discipline + Part II entry), continues PR-S2 (advancer daemon + validator + heartbeat healer + systemd units + tests) → PR-S3 (dashboard ladder panel + droplet API) → PR-S4 (Beacon shortcuts + Mirror preflight DAG-verify integration), all strictly sequential per spec § 6 sequencing constraint. The Beacon CLAUDE.md addition `## How you author multi-step build sequences` documents the three § 5.5 disciplines verbatim with explicit `'live once PR-S2 ships the validator'` / `'live once PR-S4 ships Mirror's DAG-verify capability'` parenthetical markers on the disciplines that depend on later-PR runtime — disciplines 1 (spec-doc-first) and the AUTHORING half of discipline 2 (sequence-file synthesis + dispatch_text ≤500 char + § 5.1 schema conformance) are live now; the validator-execution and Mirror-DAG-verify halves are aspirational until PR-S2 / PR-S4 merge respectively. Pattern for future "spec-adopted-in-advance, runtime-lands-later" CLAUDE.md additions: transcribe the discipline verbatim from the spec (per `spec_faithful_capture_of_locked_decisions`), and mark the aspirational sub-disciplines with explicit future-PR-gate parentheticals so a future-agent reading the section cold doesn't mistake the discipline for currently-live behavior. Reciprocal cross-reference: spec's § 1 + Companion docs header point at `pulse-cycle-upgrade.md § 12.5` (which PR #130 landed with the corresponding pulse → orchestrator hand-off statement) — co-evolved specs deserve two-way pointers, not one-direction-only.

---

## Pulse /cycle fixture-pattern allowlist — closing the hallucination class (2026-05-27)

Closes a self-inflicted-cost class surfaced today: Pulse's `/cycle` scans `~/agents/inboxes/`, `~/agents/outboxes/*/.archive/`, and recently-touched journal state for evidence of real failures. Fixture envelopes (test artifacts from yesterday's cascade or from in-flight test execution) sit in those same locations looking structurally identical to real failures. Pulse cannot tell them apart from prose alone, so she hallucinated `cycle-fix-<slug>.json` dispatches for ~18 fixture-pattern task_ids in a single window, each dispatch burning real Opus on a problem that doesn't exist. Pulse-bot was stopped manually to halt the bleed.

### Discovery + root cause

The leaked task_ids that triggered the hallucinations all matched a small set of synthetic prefixes Larry uses in tests (`t-pf`, `t-bad-rev`, `notify-t-fail`, `marker-error-t-zero-1`, `marker-error-opmanual-d35-5b-shipped-note-001-1`, `sess-abc-*`, `task-001`, `headless-001`, `pf-ok`, etc.). The producer-side fix for the leak class lives elsewhere — PR #137's AST-walk gate closed the test-isolation discipline that lets fixtures escape `tmp_path` boundaries — but residual artifacts from before that discipline still circulate in runtime state, and Pulse's scan window includes them. The consumer-side fix is this PR's allowlist: every surface that reads task_ids on the cycle path checks against the same canonical pattern set and refuses to treat a match as a real failure.

### What shipped — defense in depth across 5 surfaces

- **`scripts/fixture_patterns.py` (NEW)** — single source of truth. `FIXTURE_PATTERN_PREFIXES`, `FIXTURE_PATTERN_EXACT`, `is_fixture_task_id(task_id)`, `matched_fixture_pattern(task_id)`, plus `SHELL_FIXTURE_REGEX` for bash consumers. Stdlib only; importable from every script on the cycle path.
- **`runbooks/cycle-prompt.md`** — new `### Fixture-pattern allowlist — DO NOT dispatch on these` section between Check G (Pattern detection) and §3 (Auto-fix allow-list). Teaches Pulse-the-LLM the patterns explicitly, the audit-log line shape (`{"event": "fixture-suppressed", "task_id": ..., "pattern": ...}`), and the discipline that a match means SKIP — never dispatch.
- **`agents/pulse/CLAUDE.md`** — mirror of the allowlist with the incident framing ("18+ fixture envelopes, Pulse-bot stopped manually"), pointer back to cycle-prompt.md as canonical source, and pointer to this Part II entry for the long-form story.
- **`scripts/run_cycle.sh`** — `--- fixture-pattern commit guard ---` block inserted between the auto-commit `git add` and `git commit`. Scans `git diff --cached -U0` for fixture-pattern tokens (anchored on `"`, `` ` ``, or `=` to match JSON, markdown, and KV shapes). On match: `git reset` the staged paths, fire a `cycle-blocked:fixture-pattern-detected` larry_alert, exit 1. The cycle is allowed to *run* but not to *land* fixture-hallucinated commits on main.
- **`scripts/pulse_check_i.py`** — `is_fixture_task_id` filter applied at both signal-ingestion surfaces: `gather_retry_repeats()` drops fixture-prefixed bases after the marker-error unwrap, and the σ-anomaly synthesis in `synthesize_proposals` + `assemble_check_i` drops fixture task_ids before computing `has_signal` / before headline-counting. A fixture leak in either surface would have falsely flagged the cycle as actionable.
- **`scripts/pulse_check_iii.py`** — `SessionDuration` grew an optional `task_id` field populated by `fetch_durations_from_supabase`; `run_check` filters fixture task_ids before bucketing so a synthetic-noise duration cannot pull a real (agent, task_type) bucket's p90 around.
- **`scripts/tests/test_pulse_cycle_fixture_allowlist.py` (NEW)** — three test families: (A) pattern-match unit tests (every prefix matches a sample, every exact-match matches, recent-merge real task_ids do not match, degenerate inputs return False); (B) data-substrate filters in Check I + Check III; (C) run_cycle.sh subprocess test — fixture-pattern in staged content triggers exit-1 + canonical alert + zero commits, real-only content commits cleanly. Plus a drift gate that fails loudly if a pattern is added/removed in `fixture_patterns.py` and one of the four mirror surfaces (cycle-prompt, Pulse CLAUDE.md, run_cycle.sh, this test) wasn't updated.

### Why each gap matters

- **Self-optimizing Checks are the dangerous consumer.** Pulse's `/cycle` hallucination burned cost directly, but the Check I (weekly digest) and Check III (threshold-tuner) are the second-order risk: they ingest the same task_id stream and tune *future* behavior on it. A fixture leak into Check III's chain_events bucket would lower a real p90 — silently weakening every stuck-detector threshold for that bucket across all future cycles. Filtering at the data substrate is the only place to catch that class, because by the time a proposal lands in `pulse-threshold-proposals.json` the fixture noise is invisible.
- **Five surfaces, one canonical list.** Per the AI-knows-what (PRIME DIRECTIVE) the hallucination is the visible symptom; the durable fix is the allowlist. Putting that list in `fixture_patterns.py` and forcing all four mirror surfaces (cycle-prompt, Pulse CLAUDE.md, run_cycle.sh, test gate) to reference it via the drift-gate test is what prevents "we patched it in one place and forgot the others" — a class of regression that's bitten the codebase before (see operating-manual #54 — translation-layer drift).
- **No env-var off-switch.** The allowlist is a hardcoded guard. The brief explicitly ruled out a feature-flag toggle because (a) the override is never the right answer in production (no real task_id is a fixture), and (b) any off-switch becomes the first thing a future-Larry / future-Pulse silences when a fixture *does* leak — exactly the class of "the guard fired so we disabled the guard" mistake the operating manual already warns against in operating-manual #41 (test-regression-check.py).

### Discipline boundaries (non-negotiable)

- **The pattern list is canonical in `scripts/fixture_patterns.py`.** Edits to the list MUST update the four mirror surfaces in the same PR — the drift-gate test in `test_pulse_cycle_fixture_allowlist.py` (`AllowlistDriftTest`) fails loudly otherwise. The four surfaces are docs/prose; the Python module is the truth.
- **Skip is silent at the data-substrate layer; loud at the dispatch layer.** Check I's retry_repeats + σ-anomaly filters drop fixture rows without per-row logging (would flood the journal). The run_cycle.sh commit guard + the cycle-prompt teach-Pulse-to-log-`fixture-suppressed` shape are where the audit trail lives — every blocked dispatch produces a structured `cycle-actions.jsonl` line a future operator can grep.
- **No silent suppression.** A fixture-pattern match at the commit guard fires a `cycle-blocked:fixture-pattern-detected` larry_alert AND a `[run_cycle] fixture-guard:` log line AND exits 1. Pulse-the-LLM's allowlist match writes a `fixture-suppressed` line to `cycle-actions.jsonl` with task_id + matched pattern + timestamp. The journal entry says nothing happened; the audit trail says something specifically didn't happen and why.
- **No expansion via prose.** If a real task_id ever matches one of the allowlist patterns, that's a pattern bug to flag to Beacon, not an excuse to broaden the regex. The `test_real_task_ids_do_not_match` test gates against this — adding a fixture pattern requires demonstrating it doesn't collide with any real task_id from recent merges.

### Codified additions worth recalling next session

70. **Pulse /cycle fixture-pattern allowlist — closes hallucination class (2026-05-27).** Canonical pattern list at `scripts/fixture_patterns.py` (Python: `is_fixture_task_id(task_id)`; bash: `SHELL_FIXTURE_REGEX`). Prefix patterns: `t-`, `sess-abc-`, `notify-t-`, `notify-q-`, `marker-error-t-`, `marker-error-opmanual-`. Exact-match patterns: `task-001`, `headless-001`, `opmanual-d35-5b-shipped-note-001`, `pf-ok`, `bad-pf`, `no-preamble`, `no-chat`. Mirrored at four surfaces with explicit drift-gate test: (1) `runbooks/cycle-prompt.md § Fixture-pattern allowlist` teaches Pulse-the-LLM to log `fixture-suppressed` + skip dispatch; (2) `agents/pulse/CLAUDE.md` carries the same allowlist for cold-read continuity; (3) `scripts/run_cycle.sh` post-`git add` commit guard scans staged diff for fixture-pattern tokens (anchored on `"`, `` ` ``, or `=`), fires `cycle-blocked:fixture-pattern-detected` larry_alert + exits 1 on match; (4) `scripts/pulse_check_i.py` filters fixture task_ids out of `gather_retry_repeats` output AND out of `sigma_anomalies` inside `synthesize_proposals` / `assemble_check_i`; (5) `scripts/pulse_check_iii.py` filters fixture task_ids out of the chain_events durations before bucketing (`SessionDuration.task_id` field added for this). Test gate at `scripts/tests/test_pulse_cycle_fixture_allowlist.py` covers pattern unit tests + data-substrate filters + run_cycle.sh subprocess + four-surface drift gate. **No env-var off-switch by design** — the override is never the right answer in production; a flag becomes the first thing future-Larry silences when a fixture leaks. **Producer-side discipline** lives in PR #137 (AST-walk gate for test-isolation); this PR is the consumer-side defense for residual artifacts circulating from before that discipline. Pattern for similar "allowlist + four mirror surfaces" disciplines: put the truth in a Python module, force the mirror surfaces (prose docs, shell scripts, other code) to reference the same constants, and ship a drift-gate test that fails when the canonical list is edited without updating the mirrors — the test failure mode is the discipline.

---

## Droplet drift discipline — git-state healer + per-service overlay (2026-05-27)

Closes a staleness-incident class that surfaced three times in a single day during the 2026-05-27 orchestrator activation: bots ran pre-PR-S1 code until manual restart at 09:34 MDT, dashboard-api ran pre-PR-S3a code until manual restart at 09:45 MDT, and the droplet's working tree was 5 commits behind `origin/main` with 1 unpushed Pulse commit until manual rebase at 10:20 MDT. All three were preventable with proactive healers. This PR closes the class via (a) a new git-drift healer that observes + alerts on droplet drift vs origin, and (b) an additive per-service metadata overlay extending the existing stale-daemon-code healer so hot services get tighter thresholds + per-service strategy control.

### Discovery + root cause

Two distinct drift surfaces, both invisible without proactive healers:

- **Service runtime drift from on-disk source** — already partially covered by `heal_stale_daemon_code.py` (the auto-discovery healer that scans every `ourliberty-*.service` unit, checks script mtime vs the unit's ActiveEnterTimestamp, and auto-restarts past the 5-min race-avoidance window). The 2026-05-27 bots + dashboard-api incidents all fell within the 5-min default window's blast radius — the healer's default threshold is "loose enough" for cold paths but too loose for the hot bots / API, which Larry restarts manually because waiting another 30-min tick is too long.
- **Droplet working tree drift from origin/main** — no healer existed. The droplet quietly accumulated 5 un-pulled merges + 1 unpushed Pulse commit before anyone noticed.

### V1 reject + V2 reconciliation

The original `droplet-drift-discipline` dispatch (~21:19Z on 2026-05-27) was rejected at preflight. Forge's reject was substantive, not procedural: the original brief's "extend heal_stale_daemon_code" deliverable proposed replacing the auto-discovery mechanism (`list_ourliberty_services()` → `systemctl list-unit-files 'ourliberty-*.service'` → ~30 units) with an 8-name hardcoded allowlist. That would have NARROWED coverage by dropping `ourliberty-outbox-notifier.service` (the canonical PR #103 case cited in the existing healer's module docstring) and ~16 `ourliberty-heal-*.service` units from staleness detection — the exact opposite of the drift-discipline goal. The reject quoted the auto-discovery code at `scripts/heal_stale_daemon_code.py:221-252` and explained the regression class.

V2 reconciliation (Larry's call after the reject): auto-discovery is preserved verbatim. The new `config/tracked-services.json` is an OPTIONAL per-service metadata overlay that supplies override thresholds + `restart_strategy` for entries listed. Unlisted services use defaults (= today's behavior). Absent or malformed config file = today's behavior identically — fail-open is locked by test R6.

This is a positive example of the reject-with-substantive-finding pattern from D3.5: Forge's preflight is meant to catch exactly this class of design regression, and the right response is to reject with a concrete code citation, not to comply silently.

### What shipped

- **`scripts/heal_droplet_git_drift.py` (NEW)** — observation-only proactive guard. Per 30-min tick: refresh `origin` via `git fetch --quiet`, then evaluate three signals. AHEAD trip when `git rev-list origin/<upstream>..HEAD --count > 0` AND the oldest unpushed commit timestamp is more than 2h old; BEHIND trip when `git rev-list HEAD..origin/<upstream> --count > 2` (strict `>`, so a single new merge does not page); UNCOMMITTED trip when `git status --porcelain` is non-empty AND the newest mtime among touched files is more than 6h old. Each tripped condition fires a `droplet-{ahead,behind,uncommitted}:<branch>` larry_alert with the exact manual recovery command in the body. **No mutation by construction** — no auto-pull, no auto-push, no auto-commit. Broken main on origin or mid-edit local state must never be silently overwritten. Cooldown is per-subject 6h via a local state file at `~/agents/state/heal-droplet-git-drift-cooldowns.json`; larry_alerts' own 60-min cooldown is the floor, the 6h local gate is the binding constraint. Git-fetch failure (auth, network) logs a WARN and skips the tick cleanly — no alert about transient infrastructure, since it's not Larry-actionable.

- **`scripts/heal_stale_daemon_code.py` (EXTEND, additive)** — three surgical additions, zero changes to `list_ourliberty_services()` auto-discovery, zero changes to `auto_restart_unit()`, zero changes to the existing `is_stale()` predicate beyond using its already-present `race_avoidance_sec` parameter. (1) New `load_service_overrides()` reads `config/tracked-services.json` and returns `{}` on FileNotFoundError, malformed JSON, wrong top-level shape, or any other read error (fail-open — Test R1 + R6 lock this). (2) New `_resolve_override(overrides, unit)` returns `(threshold_seconds, strategy)`; unknown unit → defaults; bad threshold type → default with WARN; unrecognized strategy → `'auto'` (= today's behavior) so typos cannot silently disable auto-restart. (3) `check_unit()` grew an `overrides` parameter; after `is_stale()` returns True, the strategy gate branches BEFORE the restart-cooldown machinery — `'never'` returns `'strategy-never-skip'` silently; `'dm-only'` fires a `stale-code:<unit>` larry_alert via the new `dm_larry_dm_only_stale()` DM function and returns `'strategy-dm-only'`; `'auto'` (= default) falls through to the existing restart-cooldown + auto_restart_unit path VERBATIM.

- **`config/tracked-services.json` (NEW)** — initial overlay with 8 hot services: bots (beacon/mirror/pulse/forge at 600s/auto), dashboard-api (300s/auto), inbox-watcher (600s/auto), chain-event-shipper + build-sequence-advancer (1800s/auto). Keys use the full `ourliberty-foo.service` shape returned by `list_ourliberty_services()` — short-name keys would silently fail to match and the override would never apply.

- **`systemd/ourliberty-heal-droplet-git-drift.{service,timer}` (NEW)** — mirrors the heal-pipeline-stall unit pair. Service runs `python3 scripts/heal_droplet_git_drift.py` as `larry`. Timer fires `OnBootSec=10min` then `OnUnitActiveSec=30min` with 2-min jitter.

- **`systemd/INSTALL.md`** — new healer enable line + table entry.

- **Tests.** `scripts/tests/test_heal_droplet_git_drift.py` (20 tests, all 3 threshold trips at boundaries + clean-tree no-false-positive + fetch failsafe + 6h cooldown + porcelain parser + multi-signal simultaneous + kill switch). `scripts/tests/test_heal_stale_daemon_code.py` extended with R1–R6 plus loader + resolver families (13 new tests on top of the 44 pre-existing tests; 57 total; all pass). Test isolation per PR #137: every fixture in tmp_path; mocked systemd interrogation; no real `~/agents/` writes.

### Why each gap matters

- **Service-runtime drift was already partially covered; the missing piece was operator-tunability.** The existing healer's default 5-min `RACE_AVOIDANCE_SEC` is calibrated for cold paths. The bots / dashboard-api are hot enough that a 5-min window is the upper bound of what's acceptable, not a reasonable default. Without the per-service overlay, the only knob was the global constant — tightening it would have spammed false-positives on every cold healer + healer-of-healer unit. The overlay is the smallest possible change that adds the knob.
- **Droplet drift was completely uncovered.** A 5-commits-behind state plus an unpushed commit sat for hours; this is exactly the failure mode that proactive healers exist to surface. Observation-only (no auto-pull) is non-negotiable: broken `main` on origin or a mid-edit local state must never be silently overwritten by a healer. The healer surfaces the state and the exact recovery command; Larry decides.
- **Auto-discovery preservation is load-bearing.** The V1 reject's whole substance was that hardcoding a service list would regress the canonical PR #103 case (`ourliberty-outbox-notifier`) and ~16 ourliberty-heal-* units. V2's overlay makes that impossible to regress accidentally — the loader returns `{}` on absent file; the resolver returns defaults on unknown service; the strategy gate's default-`auto` branch is the same code path the healer has run since 2026-05-26. Test R1 (config absent) + R3 (unlisted service) + R6 (malformed config) explicitly lock this invariant against future drift.

### Discipline boundaries (non-negotiable)

- **Auto-discovery is the source of truth.** Adding a service to `config/tracked-services.json` does NOT add it to the scan list — only `list_ourliberty_services()` does. The overlay only configures behavior for already-discovered units. Removing the config file does NOT stop scanning anything; it only reverts the listed services to defaults.
- **`auto_restart_unit()` is preserved verbatim.** The new strategy gate runs BEFORE the restart-cooldown branch. Strategy `auto` (= default for every unit not listed in the config) hits the existing restart code path with zero behavioral change. Test R1 + R3 lock this.
- **Fail-open on config errors.** Loader returns `{}` on FileNotFoundError, malformed JSON, wrong shape. Resolver returns `(DEFAULT_STALE_THRESHOLD_SECONDS, 'auto')` on missing entry, bad threshold type, or unrecognized strategy. The whole overlay can silently disappear and the healer behaves identically to pre-extension. Test R6.
- **Git-drift healer is observation-only.** No mutation. The healer NEVER calls `git pull`, `git push`, `git commit`, `git restore`, or any mutating subcommand. Recovery is Larry's manual call after reading the alert body.
- **6h per-subject cooldown is local, not at larry_alerts.** larry_alerts has a 60-min warning cooldown; the git-drift healer's local 6h state-file gate is the binding cadence. An unpushed commit sitting for 7h is one alert, not seven.

### Codified additions worth recalling next session

71. **Droplet git-drift healer + per-service metadata overlay for stale-daemon-code — closes 3-incident staleness class from 2026-05-27 (2026-05-27).** `scripts/heal_droplet_git_drift.py` is a new observation-only healer (30-min cadence; `ourliberty-heal-droplet-git-drift.{service,timer}`) that emits `droplet-{ahead,behind,uncommitted}:<branch>` larry_alerts when the droplet's working tree drifts from origin: AHEAD when `> 0` unpushed AND oldest unpushed commit `> 2h` old, BEHIND when `> 2` commits behind upstream (strict), UNCOMMITTED when uncommitted files exist AND newest mtime `> 6h` old. No mutation — recovery commands live in the alert body. Per-subject 6h cooldown via `~/agents/state/heal-droplet-git-drift-cooldowns.json`. `scripts/heal_stale_daemon_code.py` extended with an additive per-service metadata overlay loaded from `config/tracked-services.json`; auto-discovery (`list_ourliberty_services()` → `systemctl list-unit-files 'ourliberty-*.service'` → ~30 units) is PRESERVED VERBATIM. Per-service overrides: `stale_threshold_seconds` (defaults to `DEFAULT_STALE_THRESHOLD_SECONDS = RACE_AVOIDANCE_SEC = 300s`) and `restart_strategy` ∈ {`auto` | `dm-only` | `never`}. Config keys MUST match the full `ourliberty-foo.service` shape that `list_ourliberty_services()` returns — short-name keys silently fail to match. Initial overlay covers 8 hot services (bots at 600s/auto, dashboard-api at 300s/auto, chain-event-shipper + build-sequence-advancer at 1800s/auto). Fail-open at every layer: absent file, malformed JSON, wrong shape, bad threshold type, unrecognized strategy all degrade to defaults (= pre-extension behavior). R1 (absent), R3 (unlisted service uses default), and R6 (malformed → fail-open) tests in `scripts/tests/test_heal_stale_daemon_code.py` lock these invariants. **V1 reject + V2 reconciliation:** the original brief proposed replacing `list_ourliberty_services()` with an 8-name hardcoded allowlist — that would have regressed coverage by dropping `ourliberty-outbox-notifier` (PR #103's canonical case) + ~16 `ourliberty-heal-*` units. Forge's substantive preflight reject (citing `scripts/heal_stale_daemon_code.py:221-252`) is the positive example of the reject-with-substantive-finding pattern from D3.5: preflight is meant to catch design regressions, not to comply silently. **PRIME DIRECTIVE link:** today's manual remediations (3 manual restarts/rebases between 09:34–10:20 MDT) become tomorrow's automated safety nets — the healers ARE the durable fix for the incident class.

## Test isolation v2 — indirect-log-write closure via OURLIBERTY_LOG_DIR override (2026-05-27)

Closes the indirect-log-write surface that escaped PR #136 (smoke-test-archive discipline) + PR #137 (AST-walk gate for direct inbox writes). Discovered 2026-05-27 11:56–11:57 MDT: `~/agents/logs/beacon_telegram_bot.log` contained literal test sentinel strings (`TIER_ONE_MARKER`, `'tier2 stderr distinct token'`, `'401 Unauthorized'`) with sub-second-spaced timestamps. Tests calling production functions (e.g. `beacon_telegram_bot.call_beacon` with a mocked subprocess) had reached the bot's internal `log()` helper, which wrote sentinel strings into the live log file. The PR #137 literal-pattern scanner could not catch this — the test never named the production path; the imported production module did.

### Discovery + root cause

- The 2026-05-27 11:56-11:57 incident proved tests writing through production-imported functions ALSO write to the production log paths those functions use. The `tier2_fallback_wrapper` tests in particular exercise `call_beacon()` with a mocked `_run_claude_once`; the rate-limit / auth-401 detection branches still call `log()`, and `log()` opens `LOG_DIR / 'beacon_telegram_bot.log'` for append.
- PR #136 closed the smoke-test outbox-vs-archive class. PR #137 added an AST-walk gate (`scripts/tests/test_no_production_path_leaks.py`) that catches DIRECT inbox writes — string literals matching `~/agents/inboxes/...` etc. in test files. Neither gate sees an INDIRECT write through an imported production helper.

### V1/V2/V3 dispatch sequence (transparency)

V1 was the original dispatch (full spec authored by Beacon, ~21:00Z). V2 was a verbatim re-dispatch under a new task_id after V1 forfeited operationally — Tier 1 rate-limit on `--resume` session bound to the agent_runner account, no spec change. V3 was the V2 spec re-dispatched a third time after another operational forfeit. V3 surfaced three substantive brief-vs-reality gaps in Forge's preflight CLARIFY_REQUEST:

1. The brief asserted a clean `LOG_FILE` module-level constant in both target files. Reality: both files used a `LOG_DIR` directory constant + inline filename construction at the `open()` call.
2. The brief asserted the canonical production log directory was `/home/larry/agent-core/logs/`. Reality: `Path.home() / 'agents' / 'logs'` (= `/home/larry/agents/logs/`) — verified by the live `beacon_telegram_bot.log` file (269KB, last written 2026-05-27 21:08 MDT). Using the brief's hardcoded default would have silently relocated the live log file — a P0 production regression, and the exact invariant Mirror's review focus calls out as #1.
3. The brief prescribed `def test_X(redirect_production_logs, ...)` pytest fixture parameter shape. Reality: both target test files (`test_tier2_fallback_wrapper.py`, `test_beacon_tier2_fallback.py`) are `unittest.TestCase` subclasses with `setUp`/`tearDown`; pytest function fixtures don't bind to unittest method parameters. The autouse pytest fixture works for unittest but is dead code when pytest isn't installed (the repo's regression-check tooling uses `python3 -m unittest`).

Beacon's clarification confirmed: (a) module-level `_LOG_DIR = resolve_log_dir()` pattern, (b) canonical default `Path.home() / 'agents' / 'logs'`, (c) autouse conftest fixture (with the `__init__.py` belt-and-suspenders added during implementation for unittest-only invocations), (d) AST gate narrowed to flag tests OUTSIDE `scripts/tests/`. This is a positive instance of the reject-with-substantive-finding pattern from D3.5 (and parallels the V1 reject pattern from the droplet-drift-discipline PR earlier the same day).

### What shipped

- **`scripts/beacon_telegram_bot.py` (refactored)** — added `resolve_log_dir()` returning `Path(os.environ['OURLIBERTY_LOG_DIR'])` when set, else `Path.home() / 'agents' / 'logs'` (preserving existing behavior). Module-level `LOG_DIR = resolve_log_dir()` evaluated at import time. `log()` re-resolves at write time so post-import env changes (e.g. test fixtures) still redirect — single `os.environ` lookup, no extra disk syscall.

- **`scripts/agent_runner.py` (refactored)** — same shape. Module-level `LOG_DIR = resolve_log_dir()`. `log()` re-resolves at write time. Per-agent log files (`forge.log`, `mirror.log`, etc.) all flow through the same helper.

- **`scripts/tests/__init__.py` (NEW)** — process-level log isolation for unittest invocation: sets `OURLIBERTY_LOG_DIR` to a fresh `tempfile.mkdtemp(prefix='ourliberty-test-logs-')` at package import time, before any test module imports a production module. Only sets the env var if not already set (idempotent + pytest-fixture-compatible).

- **`scripts/tests/conftest.py` (NEW)** — autouse pytest fixture providing per-test isolation when pytest is available. `monkeypatch.setenv('OURLIBERTY_LOG_DIR', str(tmp_path / 'logs'))`. Function-scoped; pytest applies autouse fixtures transparently to `unittest.TestCase` subclasses. No-op when pytest isn't loaded (the `__init__.py` already covered it).

- **`scripts/tests/test_log_dir_resolution.py` (NEW — Test family D)** — production-default verification. Locks the invariant that env-unset `resolve_log_dir()` returns `Path.home() / 'agents' / 'logs'`, preventing a future refactor from silently relocating the live log file. Also tests env-set override + empty-string defensive fallback + end-to-end log() write hits override dir.

- **`scripts/tests/test_no_production_log_writes.py` (NEW — AST gate)** — complementary to PR #137's `test_no_production_path_leaks.py`. PR #137 catches DIRECT writes (string literals naming prod paths). This gate catches INDIRECT writes (imports of protected modules). Scope: walks test files OUTSIDE `scripts/tests/` (`scripts/test_*.py`, `tests/**/test_*.py`) and flags any that import `beacon_telegram_bot` or `agent_runner` — those test trees don't load the autouse fixture or the package `__init__`, so they'd silently regain the leak class. ALLOWLIST + self-check tests (synthetic flag, synthetic from-import, conditional nested import, non-protected import, relative import) included.

- **Verification.** All 68 tests across `test_tier2_fallback_wrapper`, `test_beacon_tier2_fallback`, `test_no_production_path_leaks` (PR #137) + the new `test_no_production_log_writes` (this PR) pass. New Test family D (6 tests) passes. PR-specific sentinel strings (`'sentinel-write-from-test'`, `'test-isolation-v3-canary'`) do NOT appear in production logs after a test run; the same sentinels DO appear in `/tmp/ourliberty-test-logs-*/`. The 29 pre-existing `TIER_ONE_MARKER` / `'401 Unauthorized'` matches in `beacon_telegram_bot.log` are HISTORICAL pollution from the original 2026-05-27 incident — they cannot be retroactively scrubbed, but this PR prevents future tests from adding to them.

### Discipline boundaries (non-negotiable)

- **Production default is unchanged.** `OURLIBERTY_LOG_DIR` unset → `resolve_log_dir()` returns `Path.home() / 'agents' / 'logs'`, the same path the old `LOG_DIR` constant returned. Test family D locks this. Systemd unit `EnvironmentFile` for production bots does NOT set `OURLIBERTY_LOG_DIR`.
- **Test isolation works for BOTH pytest and unittest.** Two redundant mechanisms (the `__init__.py` process-level set + the autouse conftest per-test set) so either invocation pattern is protected. Belt + suspenders.
- **The new AST gate is COMPLEMENTARY, not a replacement.** PR #137's `test_no_production_path_leaks.py` continues to catch direct-write literals. This PR's `test_no_production_log_writes.py` catches indirect-write imports. Both must pass.
- **Helper scope is intentionally narrow.** Only `beacon_telegram_bot` + `agent_runner` got `resolve_log_dir()` because those are the confirmed offenders. Other production modules (Mirror/Pulse/Forge bots) get the same treatment in follow-up PRs as they surface — same env var, same helper shape; the autouse fixture already covers them once they adopt it.
- **No log rotation, formatter, or handler changes.** This is path-resolution only.

### Codified additions worth recalling next session

72. **Test isolation v2 — indirect-log-write closure (2026-05-27).** `scripts/beacon_telegram_bot.py` + `scripts/agent_runner.py` gained `resolve_log_dir()` returning `Path(os.environ['OURLIBERTY_LOG_DIR'])` if set, else `Path.home() / 'agents' / 'logs'` — preserving existing production behavior. `log()` re-resolves at write time so post-import env changes redirect writes. `scripts/tests/__init__.py` sets the env var to a fresh `tempfile.mkdtemp()` at package import for unittest invocations; `scripts/tests/conftest.py` autouse fixture sets it per-test for pytest invocations. `scripts/tests/test_log_dir_resolution.py` (Test family D) locks the production-default invariant. `scripts/tests/test_no_production_log_writes.py` is a NEW complementary AST gate that flags test files OUTSIDE `scripts/tests/` importing `beacon_telegram_bot` or `agent_runner` (those test trees don't load the autouse fixture or package `__init__`). Critically COMPLEMENTARY to PR #137's `test_no_production_path_leaks.py` — that gate catches DIRECT-write string literals; this gate catches INDIRECT-write imports. Both must pass. **V3 reject-with-substantive-finding shape:** Forge's preflight CLARIFY_REQUEST surfaced three brief gaps — no `LOG_FILE` constant exists (only `LOG_DIR` + inline filename), brief's canonical default `/home/larry/agent-core/logs/` would have silently relocated the live `beacon_telegram_bot.log` (real path: `/home/larry/agents/logs/`), and unittest test classes can't bind pytest function-fixture parameters. Beacon's clarification authorized the corrected shape. **PRIME DIRECTIVE link:** today's manual remediation (none — the leak was silent until log inspection) becomes tomorrow's automated safety net: the AST gate fails CI before a future test reintroduces the class, the autouse + `__init__.py` isolate by default, and Test family D prevents a default-path regression. Followup PRs apply the same `resolve_log_dir()` shape to other long-running daemon modules as the same incident class surfaces (Mirror bot, Pulse bot, Forge bot).
