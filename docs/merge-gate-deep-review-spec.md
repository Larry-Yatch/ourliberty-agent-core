...
## 0. Goal
A **deep-review hold**: when a PASS'd PR touches a configured critical-path fileset (or carries a `deep-review-required` label), the auto-merge path returns a new terminal `held_deep_review` outcome instead of merging, DMs Larry the manual-merge command, and lets the existing healers leave it alone until he merges it after `/code-review high`. Small and additive — it reuses the existing `held_conflict` seam end-to-end; the merge shell-out path is untouched.
...
New `_deep_review_required(repo_coords, pr_number, changed_files) -> bool`: True if any label from `gh pr view <pr> --json labels` equals `deep-review-required`, OR any path in `changed_files` matches a glob from `_load_deep_review_paths()` (via `fnmatch`). If `changed_files` is None, resolve it with the existing `_gh_pr_changed_files` (~line 7694) — no extra gh call in the common case since `changed_files` is already threaded into the gate (~line 8772). Fail-open-safe: on a gh error resolving labels, still honor the fileset check (a fileset match alone must hold).
...
- New `_dm_larry_deep_review_hold(...)` modeled on `_dm_larry_rebase_needed` (~line 8129), using `larry_alerts.append_notification` (closes the chain in the originating chat) with `append_alert` fallback. Body: "Mirror PASSed, but this PR touches the approval/resolve critical path — held for your `/code-review high` + manual merge," followed by the exact `scripts/merge_reviewed_pr.sh <PR>` command and the PR url.
