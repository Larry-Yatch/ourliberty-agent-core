...
**Enforcement:** the runtime check that makes the helper structurally authoritative over any in-prompt Tier-4 classification is now LIVE — the once-deferred check is a hard code mechanism at the CLI write boundary, not more prose. `scripts/alert_triage_state.py guard_tier4` (subcommand `guard-tier4`) is the gate: a Tier-4 outcome is authoritative ONLY when BOTH (a) a same-iter `triage-alert` call is recorded for that alert-id (its persisted `last_triaged_iter` equals the current iter) AND (b) a fresh deterministic `classify()` of the alert also returns Tier 4. The guard emits ONE JSON verdict `{authoritative_tier, accepted, helper_tier, same_iter_call, reason}`; `accepted` is true only when `helper_tier==4 AND same_iter_call`, else `authoritative_tier == helper_tier` (fail toward the helper). A missing/unparseable alert never yields Tier 4 — it falls to the lowest safe tier. The low-level `triage --tier 4` write path routes through the SAME guard, so a hand-written Tier 4 that skipped or overrode the helper (the 2026-07-29 `medic/medic-diagnosis` incident: `classify()` returned Tier 3, the cycle LLM wrote Tier 4 anyway) is *rejected, not persisted*. **Pulse Check 0 must record a Tier-4 outcome ONLY via the guarded path — call `triage-alert` for the alert THIS iter, then adopt the guard's `authoritative_tier`; never hand-assert Tier 4.** Covered by `scripts/tests/test_alert_triage_state.py` (`TestGuardTier4`, `TestGuardTier4CLI`, `TestLastTriagedIterPersistence`). Doctrine-of-doctrine pairing: this paragraph is the contract; the guard + tests are its durable enforcement.

**One row, one classification (never compose fields across rows).** Check 0 classifies each `larry-alerts.jsonl` row **AS WRITTEN** — the `(source, intent, subject)` triple of a SINGLE row. Never weld fields from two rows into one alert, however adjacent or causally related they look. A healer alert and the `medic-diagnosis` notification it triggers are **two separate alerts with two separate classifications**, even when they land seconds apart about the same incident: `source=medic` rows always carry `subject=null` (0 of 69 across all history), so a `medic` row bearing a healer's subject is a fabrication, not a novel alert. Composing them manufactures a triple that exists in no row and that classifies Tier 4 while BOTH real rows classify Tier 3 — a spurious Tier-4 verdict that accrues G-rule occurrences and dispatches direction-asks resting on a false premise (the 2026-08-06 incident: `source=medic, intent=medic-diagnosis` welded onto `subject=pipeline-stall:unrouted-pr:PR#192`; two G-rule cycles were spent on false premises from this one gap).

**Enforcement:** payload fidelity is a hard code gate, not prose. `guard_tier4` accepts Tier 4 ONLY when the supplied alert corresponds to a REAL row: on the Tier-4 accept path it matches the payload's `(source, intent, subject)` triple against the trailing `FIDELITY_TAIL_ROWS` rows of `~/agents/blackboard/larry-alerts.jsonl`. No matching row → `accepted: false` and `authoritative_tier` falls to the safe tier, with a `reason` naming the fidelity failure so a composite surfaces in the log instead of silently downgrading. A missing/unreadable ledger leaves the gate INERT (an IO error must never manufacture a misclassification — the same fail-soft posture the module takes on unreadable config), and Tiers 1/2/3 are untouched: the gate only ever removes an unearned Tier 4. Covered by `scripts/tests/test_alert_triage_state.py` (`TestGuardTier4PayloadFidelity`, plus `TestGuardTier4CLI.test_triage_cli_composite_payload_rejected` at the write boundary). Doctrine-of-doctrine pairing: the paragraph above is the contract; this gate + its tests are its durable enforcement.
...
**Scheduling change (2026-07-07): every periodic check (I, III, IV, V, VI, VIII, IX, X, XI) now fires from its own systemd timer (`ourliberty-pulse-check-<id>.timer`). Do NOT invoke any of them from /cycle.** Agent-invoked scheduling chronically missed late runbook sections (journal G-rules `check-iii-invoke-gap-sunday-001`, `check-ix-x-invoke-gap-monday-001`); timers never miss (IV and XI proved the pattern). Every unit carries `ConditionPathExists=!/home/larry/agents/blackboard/EMERGENCY_HALT`, so a tripped halt skips all of them at the systemd layer. Your §5 duties are now **triage and journaling only**: read each check's new artifacts / journal blocks since your last iter, fold them into your cycle entry, and triage any `pulse-check-failed:<id>` alert per Check 0. The §5.0 self-gating one-shots below are the exception — they remain agent-run every cycle.
...
**Scheduling:** `ourliberty-pulse-check-i.timer` fires the analyzer Mon/Wed/Fri/Sun, deliberately hours after Ledger's Monday run (exact times live in the unit files). Do NOT invoke it from /cycle. The analyzer self-gates on weekday, EMERGENCY_HALT, and the Ledger sentinel (`~/agents/blackboard/ledger/ledger-ready-<most-recent-Monday>`), journaling its own one-line skip note when a condition fails.
...
| EMERGENCY_HALT tripped | Unit skipped (ConditionPathExists halt gate); the script's own halt gate is a second net | Same as during halt |
...
**Scheduling:** `ourliberty-pulse-check-viii.timer` fires the analyzer Mondays. Do NOT invoke it from /cycle. The analyzer's own week-Monday sentinel (`~/agents/blackboard/pulse-check-viii-proposals/check-viii-<this-week-Monday>.json`) makes any same-week re-run a clean no-op.
...
| EMERGENCY_HALT tripped | Unit skipped (ConditionPathExists halt gate) | Same as other checks |
...
**Scheduling:** `ourliberty-pulse-check-ix.timer` fires the analyzer Mondays. Do NOT invoke it from /cycle. The analyzer's own week-Monday sentinel (`~/agents/blackboard/pulse-check-ix-proposals/check-ix-<this-week-Monday>.json`) makes any same-week re-run a clean no-op.
...
**Scheduling:** `ourliberty-pulse-check-x.timer` fires the analyzer Mondays. Do NOT invoke it from /cycle. The analyzer's own week-Monday sentinel (`~/agents/blackboard/pulse-check-x-proposals/check-x-<this-week-Monday>.json`) makes any same-week re-run a clean no-op.
...
(**Check VII retired 2026-07-07.** It was to tune the Decision III cost-ceiling thresholds from Pulse's escalation-response log, but the producer for its substrate never shipped — and the two path names never even agreed (the analyzer read `~/agents/state/pulse-cost-escalations.jsonl` while the § 6.8 doctrine logs responses to `~/agents/state/cost-escalation-responses.jsonl`; neither file was ever created) — so the check never ran once, and its `event_driven` cadence entry made the staleness watcher skip it forever. `scripts/pulse_check_vii.py` and its cadence entry were removed; revive both from git history if a deterministic escalation-response producer ever ships.)

| Check | What it tunes | Cadence (systemd timer) | Data substrate | Silent until | Analyzer script |
|---|---|---|---|---|---|
| III | Stuck-detection thresholds | 14-day Sunday-anchored (`ourliberty-pulse-check-iii`, every Sunday; the analyzer skips the off week) | `chain_events` (live now) | Live (Sun 2026-05-31 first run) | `scripts/pulse_check_iii.py` |
| IV | Marker-drift enforcement strictness AND known-pattern allowlist tuning | Weekly (`ourliberty-pulse-check-iv`, Mondays) | `chain_events` query for `mirror_marker_invisible:*` PLUS `alert-triage.json` triage_decisions | Live immediately (has data now); allowlist tuning silent until ~50 triage decisions accumulate | `scripts/pulse_check_iv.py` |
| V | Tier-1 action-template trust list (guard-list graduation per Decision I) | Monthly (`ourliberty-pulse-check-v`, first Monday) | `cycle-prime-ledger.jsonl` + `alert-triage.json` | ~30d of cycle ledger data | `scripts/pulse_check_v.py` |
| VI | PRIME DIRECTIVE posture (Generous / Neutral / Strict) per Decision II — **inert since 2026-08-03: `verification_pending` is retired, so the check short-circuits to `rule_fired: "retired"` with zero proposals (§ 6.2)** | Monthly (`ourliberty-pulse-check-vi`, first Monday) | `cycle-prime-ledger.jsonl` `verification_pending` rates + auto-promote ratios + ratio-trend | ~30d of cycle ledger data | `scripts/pulse_check_vi.py` |
...
- **Check VI — PRIME DIRECTIVE posture.** **All three triggers are inert as of 2026-08-03**: `verification_pending` is retired (§ 6.2), so when no new rows of that kind land in the window the check short-circuits to `rule_fired: "retired"` and emits ZERO proposals BEFORE any trigger is evaluated. The short-circuit is load-bearing rather than cosmetic — a retired category drives the pending rate to 0.0, which would otherwise satisfy trigger 2 forever and re-propose "tighten" on an empty category every month. The triggers are documented here as the contract the analyzer still implements:
  ...
  **Cross-reference to Decision II (§ 6.7):** the starting posture was Neutral; Check VI proposed adjustments to Generous / Neutral / Strict based on the three triggers above. Retained as design history — see the § 6.7 banner.
...
**TODO comments, "I'll watch this" deferrals, and untested patches DO NOT count.** A fix that's been written but not verified is **not** a `systemic_fix` row — record an `intervention`. A `systemic_fix` row requires that Pulse actually confirmed the fix landed and worked.

**`verification_pending` is RETIRED (2026-08-03).** No new rows of that kind are written. Historical rows stay in the append-only ledger and stay readable — the G5 weekly retrospective still surfaces the expired ones. Why: no falsifiable anchor was ever recorded at filing time. Of the 48 expired-unpromoted rows in full history, only 2 carried a `verification_anchor` at all (both free-text prose, not machine-checkable) and every one had `verifies_at: null` — so a pending row could never be promoted, and Check VI's 94% "stuck forever" rate was measuring a category that was never wired at the write end. If a real verification mechanism is ever wanted, the correct shape is REQUIRING a machine-checkable predicate at filing time (`scripts/freshness_probe.py` already implements that evaluator); § 6.7 keeps the Decision II design history as the reference for that.

**Enforcement:** two hard mechanisms, not prompt discipline. (1) `scripts/pulse_check_vi.py` short-circuits to `rule_fired: "retired"` with ZERO proposals when no new `verification_pending` rows land in the window — so the retired category can no longer fire a monthly posture proposal. (2) `scripts/cycle_prime_ledger.py append --kind verification_pending` exits non-zero with an actionable message and writes nothing — so a partial revert of this doctrine still cannot produce a new row.
...
- **An alert that is itself a REAL escalation is not a noise occurrence — never tally it toward a G-rule's ≥3 threshold.** The pattern-detection window counts *recurrences of the noise you want to eliminate*, not every alert that happens to share a source or a tier with it. A genuine finding (a regression the gate caught, a deadline about to expire) or any row carrying `needs_larry` is signal that WORKED; counting it as an occurrence inflates a 1/3 pattern into a false 3/3 and dispatches a fix for a problem that isn't there. Before tallying, ask what would have been lost had that alert never fired — if the answer is "a real thing Larry needed to know," it is not an occurrence. **Motivating incident:** G-rule `pulse-triage-self-report-should-be-tier3-001` reached 3/3 only because two real escalations — a regression-gate `REGRESSED` finding and `pr1081-72h-gate-imminent` — were counted alongside the single genuine self-report duplicate (`tier4-novel:forge-wip-redispatch`). The true occurrence count was 1.

  **Enforcement:** `alert_triage_state.classify` Gate 0 (`SELF_AUTHORED_SOURCES`) is the structural half — self-authored alerts now classify Tier 3 at Gate 0 and never reach the Tier-4 novel path, so they cannot generate the occurrence rows a G-rule tallies in the first place. The judgment half (deciding whether a *non*-self-authored alert was real signal) stays prose, with Mirror's review checklist as the residual backstop: a dispatched systemic fix whose occurrence evidence includes rows that are plainly real escalations gets flagged at PR time.
...
> **RETIRED 2026-08-03 — design history, not operative doctrine.** `verification_pending` is retired per § 6.2: no new rows are written, historical rows remain readable. Everything below describes the lifecycle as designed and is kept as the reference for why Neutral was chosen — read it if a real verification mechanism is ever built, not as an instruction to file the kind.
...
*(The blockquote above is the spec verbatim; its Check VII self-tuning leg was retired 2026-07-07 — see the cross-reference note below. The escalation-DM behavior itself is unchanged and live.)*
...
The single canonical Pulse → Larry message format. All DMs Pulse sends — Check 0 triage outcomes, Decision III cost escalations, Decision IV immediate DMs and the daily digest, Check III-VI proposal artifacts, Check 0 Tier-2 guarded-category gates, Check 0 Tier-4 novel-alert guidance requests — use this template verbatim. **No other DM shape is canonical for Pulse.** Earlier α₁ DM examples that used a different format must be re-rendered through this template; mirror's α₂ review checks this explicitly per brief Mirror-focus item #5.
...
**Composition with Check VIII / Check IX (existing producer-side DMs).** Check VIII and Check IX already DM Larry per α₁'s § 5.2 and § 5.3 respectively. The plain-language template applies prospectively to NEW Pulse DM surfaces (Check 0, Decision III escalations, Decision IV immediate + digest, Check III-VI proposals); Check VIII's existing burn-rate-signal DM shape and Check IX's mission-registration flow continue to operate per their existing contracts. If the brief surfaces an inconsistency post-α₂, the resolution path is a small follow-up PR to update Check VIII / IX DM rendering — NOT an inline α₂ modification of § 5.2 / § 5.3 (those sections are NOT in the α₂ extend list per the brief).
...
3. **Self-optimization backlog.** Any Check III/IV/V/VI proposal that's been queued for >7 days without Larry's approve/reject? (Read the relevant `pulse-<check>-proposals.json` artifacts.)
...
3. **Self-optimization backlog.** No Check III/IV/V/VI proposal artifacts older than 7 days without an applied/rejected flag. No candidate from this surface.
...
| 4. Decision III — Soft cost ceiling | § 6.8 | § 6.10 (escalation DM rendering), § 5.4 (Check VII tuning — retired 2026-07-07) | § 12.2 Decision III |
...
| 7. 5-Check family overview | § 5.4 | § 6.6 (Check V → guard list), § 6.7 (Check VI → posture), § 6.8 (Check VII → cost ceiling — retired 2026-07-07), § 6.11 (Check IV → allowlist) | § 12.3 |
...
- `cost_escalation_ledger.append_response(band: str, response: str, preceding_spend_usd: float, iter_n: int, ts: str) -> None` — appends to `cost-escalation-responses.jsonl` (was Check VII's planned training data; the check is retired, the log stays).
...
- `cost_escalation_ledger.read_recent_responses(window_days: int = 30) -> list[dict]` — returns the trailing-window response log (was for Check VII's consistency triggers; retired 2026-07-07).
