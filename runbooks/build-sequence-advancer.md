...
| **30-min gate-mismatch** (one side says MERGED, the other doesn't) | After 30 min of mismatch, sets `status: paused` + audit_log `gate-mismatch-timeout` event. DMs Larry. | DM with the chain/gh mismatch summary. | Investigate which side is lying: `gh pr view <pr_url> --json state` against the chain_events row. The mismatch usually resolves itself within a few minutes of merge; a >30-min mismatch indicates a real ingestion or gh-cache problem. |
...
- **Belt-and-suspenders gate is non-negotiable.** Both `chain_events.auto_merge.outcome=merged` AND `gh pr view = MERGED` must agree before a step transitions to `merged` (spec § 5.3). One side alone is a `gate-mismatch` event that triggers a 30-min countdown to pause + DM. Do not edit the gate logic to "just trust chain_events" — that's exactly the failure mode the spec is engineered to prevent.
