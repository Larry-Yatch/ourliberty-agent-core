...
decoration:
  - runtime: every skip reason is its OWN tick-line counter, so a daemon that
    dropped out of coverage lands in ``skip-ephemeral`` and can never be
    confused with the ~95 healers in ``skip-oneshot`` that were never in scope.
    UNREADABLE never borrows one of those buckets: an absent ``Type=`` or
    ``ReadWritePaths=`` is ``skip-unknown``, because a truncated property dump
    is not a positive fact about the unit.
...
A richer coverage detector — a ``coverage=`` line naming the monitored set, with
departures carried across ticks — belongs with the fleet-wide detect pass that
can compute it in one systemctl read per unit. That pass is not in this change
(``main()`` here is one ``check_unit`` call per unit, as it has always been), so
the detector ships with it rather than being bolted onto a loop that would have
to read every unit twice to feed it.
...
#
# `forking` is in the set: a classic double-forking daemon is exactly as
# long-lived as a `simple` one and its MainPID (the forked child, which systemd
# tracks via PIDFile=) holds the same pinned bind-mount. Omitting it made every
# forking carve-out unit classify skip-oneshot — silently out of coverage, with
# the build-time lint skipping it for the same reason, so neither detector could
# see it. Nothing in systemd/ is Type=forking today; this is the gap closed
...
class UnitFacts(NamedTuple):
    ...
    ``type_`` and ``read_write_paths`` are Optional and are ``None`` when the
    property was ABSENT from the dump, which is not the same as empty. A
    truncated ``systemctl show`` — a partial read under dbus pressure — must not
    read as ``Type=`` "not persistent" or ``ReadWritePaths=`` "carves nothing",
    both of which are indistinguishable in the tick line from a unit that was
    never in scope. Absent means UNKNOWN, and unknown means we do not act."""
    unit: str
    present: bool
    type_: Optional[str]
    active_state: str
    main_pid: Optional[int]
    read_write_paths: Optional[str]
...
def unit_facts(unit: str) -> UnitFacts:
    ...
    if not props:
        return UnitFacts(unit, False, None, 'unknown', None, None, None, '')
    try:
    ...
    if pid is not None and pid <= 0:
        pid = None
    # .get() with NO default on the three properties whose absence must stay
    # distinguishable from their empty value.
    return UnitFacts(
        unit=unit,
        present=True,
...
def classify_unit(facts: UnitFacts) -> tuple[str, str]:
    ...
    UNREADABLE IS NOT A CLASS. Both cuts fail the same way, and that symmetry is
    the point: an absent or empty `Type=` is SKIP_UNKNOWN exactly as an absent
    `Restart=` is, never SKIP_ONESHOT. `skip-oneshot` asserts a POSITIVE fact —
    systemd told us this unit runs to completion — and a truncated property dump
    told us nothing. Reading a partial read as 'oneshot' put the unit in the same
    tick-line bucket as ~95 healers that are genuinely out of scope, which is
    where a real daemon goes to disappear."""
    ...
    if not facts.type_:
        return CLASS_SKIP_UNKNOWN, (
            'Type= property unreadable'
            if facts.type_ is None else 'Type= property empty')
    if facts.type_ not in PERSISTENT_TYPES:
        return CLASS_SKIP_ONESHOT, f'Type={facts.type_} is not persistent'
...
def inspect_unit(unit: str, facts: UnitFacts, klass: str, state: dict,
    ...
    if facts.read_write_paths is None:
        # ABSENT, not empty — the property dump was truncated. `skip-nocarve`
        # asserts systemd told us this unit carves nothing; it told us nothing.
        return OUTCOME_SKIP_UNKNOWN
...
def check_unit(unit: str, state: dict, now: Optional[float] = None) -> str:
    """Inspect (and if needed repair) ONE unit. This is the production path:
    ``main()`` calls it once per unit and does nothing else per-unit, so a test
    that drives ``check_unit`` is testing what systemd actually executes.
...
def main() -> int:
    ...
    state = load_state()
    # Pre-seeded from OUTCOMES so a reason that never fires is still a KEY the
    # tick line could have printed, and a new outcome cannot be added without
    # appearing here.
