...
def build_review_bounded_step_system_prompt():
    ...
    return (
        ...
        "For the TEST REGRESSION CHECK specifically, use these EXACT ceilings —\n"
        "do NOT lower them and do NOT pass a smaller `--timeout-per-sha`. A full\n"
        "suite pass MEASURES ~540s (it is wait/IO-bound, not slow-because-loaded),\n"
        "so a smaller ceiling KILLS a healthy run mid-suite and forces a FALSE\n"
        "`REVIEW_ESCALATE` on clean code (#747/#763/#790). These fit one run (hit)\n"
        "and the rare two-run parent+head (miss) inside the review-session budget:\n\n"
        "    bash ~/agent-core/scripts/run_review_step.sh --timeout 1500 \\\n"
        "        --label 'regression check' -- \\\n"
        "        python3 ~/agent-core/scripts/test_regression_check.py \\\n"
        "        --parent-sha <baseRefOid> --head-sha <headRefOid> --output json\n\n"
        "The gate's own per-SHA cap defaults to 800s (one run ~540s); leave it at\n"
        "the default. If it still exits 124 at 1500s, THAT is a real inconclusive\n"
        "— escalate; do not retry with an ad-hoc larger number.\n\n"
