#!/usr/bin/env python3
"""Retraction tests for heal_pr_terminal_fanout_heartbeat.py (slice 2 adopter).
...
for this single-subject adopter:
  ...
  - a degraded / unreadable probe (not-stale-but-not-fresh, or the real stale
    path) does NOT retract. Gate is `reason == 'fresh'`.

Run:
    python3 -m unittest scripts.tests.test_heal_pr_terminal_fanout_retraction
"""
...
import heal_pr_terminal_fanout_heartbeat as h  # noqa: E402
...
class _IsolatedQueue(unittest.TestCase):
    ...
    def _seed_stale_escalate(self):
        ok = larry_alerts.append_alert(
            ...
            message='fanout heartbeat stale', subject=h.ALERT_SUBJECT,
            route='escalate',
        )
        self.assertTrue(ok)
...
class DegradedReadDoesNotRetractTest(unittest.TestCase):
    ...
    def test_stale_path_does_not_retract(self):
        ...
        rc, retract = self._run_main((True, 9999.0, 'heartbeat mtime 9999s old'))
        self.assertEqual(rc, 0)
        retract.assert_not_called()
