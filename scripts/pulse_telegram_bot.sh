...
# below — same fix as beacon_telegram_bot.sh, same reason: a hardcoded
# ~/agents/logs hint points the operator at a path this script does not
# write once the override is set.
...
tmux new-session -d -s "$SESSION" \
  "set -a; . '$ENV_FILE'; set +a; export AGENT=pulse; cd '${HOME}/agent-core/agents/pulse' && exec python3 '$BOT_PY' 2>&1 | tee -a ${LOG_DIR}/pulse_telegram_bot.tmux.log"
...
echo "Tail log:   tail -f ${LOG_DIR}/pulse_telegram_bot.log"
