...
`expects_no_pr(task_id, *, outbox=None) -> (verdict, reason)`:
  ...
  * ``EXPECTS_PR``  — the task should have opened/updated a PR; a missing PR is
    ...
    ack-proceed then died; a `doc-only` task; a `rebase-*` that updates an
    existing PR).
...
tasks open a PR and ``rebase-*`` tasks update an existing one, so they are
encoded as ``EXPECTS_PR`` — a real stall on them is never hidden.

PURITY
------
No network and no disk I/O at import or in ``expects_no_pr`` itself. The
outbox-truth rules operate on an injected ``outbox`` dict (the archived Forge
outbox OR a dispatch envelope — both carry ``result`` / ``exit_code`` /
``phase`` / ``task_type``), so callers load it and pass it in; tests inject a
dict directly. The only side effect is a best-effort append to the observability
log on an UNKNOWN verdict, which never raises.
...
#                       decision approval — emits a verdict, never a PR.
...
#   rebase-      operates on an EXISTING PR by design (updates it).
...
def expects_no_pr(task_id: str, *, outbox: Optional[dict] = None
    ...
    Evaluation order is precedence-sensitive:
      ...
      2. outbox-truth (when a dict is provided): REJECT/CLARIFY → LEGIT_NO_PR;
         ...
         Placed BEFORE the ``build-``/``rebase-``/``opmanual-`` shape prefixes
         because a build-SEQUENCE preflight task is legitimately named
      ...
      3. must-never-suppress SHAPES (build-/rebase-/opmanual-, doc-only/build
