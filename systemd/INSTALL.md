...
sudo systemctl enable --now ourliberty-heal-stale-escalation-recheck.timer  # stale session-less escalation cards (every 20 min; OnCalendar) — re-checks whether a pending card's cited blocker still exists; needs EnvironmentFile=.env.larry for gh
...
sudo systemctl enable --now ourliberty-gh-burn-sampler.timer  # gh-api-burn phase 1 (every 5 min) — appends one FREE rate_limit reading to ~/agents/logs/gh-api-burn.jsonl; pure measurement, needs EnvironmentFile=.env.larry for gh
sudo systemctl enable --now ourliberty-gh-burn-analyzer.timer  # gh-api-burn phase 1 (daily 07:00 MDT) — measures the burn from the sampler log; pings Larry ONCE to authorize the phase-2 durable fix; needs EnvironmentFile=.env.larry for Supabase + gh
...
| `stale-escalation-recheck` (RSDPM #111) | 20 min | A session-less Mirror decision card is a SNAPSHOT at emit time and nothing re-evaluates it; the only automatic retirement is `outbox_notifier._reconcile_no_session_decision_on_merge`, which fires only when the PR merges. On RSDPM #111 the cited blocker cleared 2m31s AFTER the card fired and it sat wrong for 9h. This healer re-checks each pending `mirror-review-*` card that carries a `recheck_target`, via a first-match ladder: PR MERGED/CLOSED → retire; PR head moved off the card's head → retire as superseded (a review of the new head emits its own card); the STALE-GATE signature (base CI green **and** branch behind base **and** the PR's own checks red **and** non-draft, non-conflicting) → run `gh pr update-branch` and retire, letting the existing Mirror + auto-review pipeline finish. Anything else stays pending — uncertain is never stale, and every unreadable probe is uncertain. Order is act → ledger → retire, so a crash mid-sequence leaves the card for the next tick rather than losing it. `state/heal-stale-escalation-recheck.json` caps one action per (repo, PR, head) and `MAX_ACTIONS_PER_RUN=3` caps CI runs per tick. `--dry-run` probes without writing. Needs `EnvironmentFile=.env.larry` for gh. |
