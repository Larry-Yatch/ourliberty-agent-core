...
## G-rule pr-fanout-probe-health-tier4-001 → RE-OPENED ⚠️ (iter ~4774, occ 2/3 post-re-open)

**Rule:** `source=pr-terminal-fanout, subject=pr-fanout-probe-health` alerts classify Tier-4 (novel, no translation match). Root cause: GH API rate-limit saturation (transient, same window as outbox-notifier rate-limit WARNs; not auth failure). outbox-notifier already DMes Larry via route=escalate; Pulse journals only, no duplicate DM. Fix: add `pr-fanout-probe-health` sub-key under `pr-terminal-fanout` in `config/alert-translations.json` (severity=INFO, tier=FYI) — AS A FORGE PR, NOT a direct session edit. **Re-opened iter ~4774:** prior direction-ask-001 resulted in Beacon directly editing the file in its worktree session — not a PR, not in main. direction-ask-002 re-dispatched to Beacon at iter ~4774 specifying fix MUST go through Forge PR. Forge preflight APPROVAL_REQUEST (pr-fanout-probe-health-tier3-translation-002) queued in Larry's pending since 12:46:21Z. Occurrences: iter ~4654 (1/3); iter ~4760 (2/3); iter ~4761 (3/3, false-verify); iter ~4774 (1/3 post-re-open, direction-ask-002 sent); iter ~4789 (2/3 post-re-open, L915). Watch for 3/3 post-re-open — if hit before Forge preflight approved, dispatch may need escalation.
...
## G-rule outbox-notifier-auto-merge-queue-stale-promoted-tier4-001 — 1/3 (new, iter ~4752)

**Rule:** `source=outbox-notifier, subject^=auto-merge-queue-stale:*::promoted` alerts classify Tier-4 (novel, no translation match). Fires when a PR's auto-merge queue entry is promoted from route=hold (first 24h, digest/suppressed) to route=escalate (>24h, DM delivered by outbox-notifier). outbox-notifier already DMs Larry on the escalate path; Pulse should silence (Tier-3 journal-only) rather than duplicate DM. Fix: add `source=outbox-notifier, subject^=auto-merge-queue-stale:` + `subject$=::promoted` → Tier-3 entry in `config/alert-translations.json`. Dispatch to Beacon at 3/3. First occurrence: iter ~4752 (PR #857 stuck behind PR #847 held_deep_review since 2026-07-08T06:36Z, >27h). Note: DISTINCT from `outbox-notifier-auto-merge-queue-stale-merged-pr-001` (COMPLETE ✅ PR #893) which was about already-merged PRs being re-queued.

---

## Status snapshot — updated 2026-07-09T15:48Z UTC (Iter ~4798, **Tier 1**)

**Iter ~4798 summary (2026-07-09T15:48Z):** 2 new alerts: L915 dispatch-branch-cleanup/gh-unavailable → Tier-3 resolved; L916 pr-terminal-fanout/pr-fanout-probe-health → Tier-4 (G-rule 3/3 post-re-open, fix in PR #894 AUTO_MERGE_HELD #854). Watermark: 916. pending=2 unchanged. **KEY:** outbox-notifier consec=3 rate-limit WARNs at 15:38Z UTC (backoff 241s, expired ~15:42Z) — elevated vs prior but within PR #880 design. PR #894 Mirror REVIEW_PASS, AUTO_MERGE_HELD blocker=#854. G-rule pr-fanout-probe-health-tier4-001 → VERIFY on PR #854+#894 merge. **STANDING:** zombie PID 1834248 (~41d+20h+29m+); PR #847 OPEN (HELD_DEEP_REVIEW); PR #854/860/874/890/891 OPEN (PR #854 blocking PR #894); PR #894 Mirror REVIEW_PASS AUTO_MERGE_HELD blocker=#854; 2 APPROVAL_REQUESTs awaiting Larry. HEAD=3f81b777. Sync last_sync=15:39:48Z. PRIME: ratio≈21.08 (interventions=1644, systemic_fixes=78, vp=36). **ACTIVE G-rules:** pr-fanout-probe-health-tier4-001 [3/3 post-re-open, Mirror REVIEW_PASS PR #894, AUTO_MERGE_HELD #854, VERIFY on #854+#894 merge]; build-sequence-advancer-sequence-complete-tier4-001 [2/3]; forge-marker-task-id-mismatch-xii-v1 [2/3]; outbox-notifier-auto-merge-queue-stale-promoted-tier4-001 [2/3]; outbox-notifier-merge-held-deep-review-tier4-001 [1/3]; mirror-malformed-verdict-heal-reap-path-001 [1/3]; forge-wip-redispatch-exhausted-genuine-no-pr-001 [1/3]; sentinel-inflight-stall-tier4 [VP, fix=PR #854]; notifier-concurrent-scan-dup (PR #847 held, occ 10+); ourliberty-health-subject-key-mismatch-001 [3/3, vp]; forge-revision-preamble-missing-pr711-001 [VP, occ 7 post-dispatch]; unreviewed-merge-larry-authored-pr-001 [13+]; forge-wip-redispatch-digest-tier4-001 [vp]. **VERIFIED (closed):** dispatch-branch-cleanup-gh-unavailable-001 (CLOSED ✅ iter ~4768); outbox-notifier-auto-merge-queue-stale-merged-pr-001 (PR #893 ✅); outbox-notifier-auto-merge-rate-limit-orphan-001 (PR #892 ✅); sequence-invalid-completeness-pr3-fanout-sentinel (PR #871 ✅); no-session-revision-merged-pr-fp-001 (PR #873 ✅); notifier-gh-rate-limit-no-backoff-001 (PR #880 ✅); heal-pipeline-stall-stalled-active-step-tier4-001 (PR #883 ✅); auto-merge-conflict-promoted-merged-pr-001 (PR #889 ✅).
