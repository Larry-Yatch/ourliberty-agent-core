...
- **Steady-state degraded hold: 79 consecutive iters (377–455) with 0 pre-existing G-rule advances.** Iter 455: 0 new escalations. 0 new alerts. 0 open PRs. heal-stale-daemon-code G-rule stable at 2/3. Awaiting Larry action on APPROVAL_REQUEST `Tier 2 OAuth restore`. Cooldown files: 112 total (stable); pipeline-stall files: 27 (stable). Alert watermark: 1133.
...
- **SYNC-PUSH-REBASE-FALLBACK-001 — RECURRING (11th total / 5th post-clear, iter 451; 12th occurrence pending).** Pattern: fires every PR merge day, 11 for 11. PR #241 MERGED — 12th occurrence expected when sync next runs. APPROVAL_REQUEST `sync-push-rebase-fallback-001` URGENT (must ship before next PR merge). [yellow] escalation idx=1125 at iter 440.
