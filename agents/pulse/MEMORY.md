...
## G-rule outbox-notifier-auto-merge-queue-stale-promoted-tier4-001 → DISPATCHED ✅ (iter ~4839, 3/3), vp

**Rule:** `source=outbox-notifier, subject^=auto-merge-queue-stale:` alerts classify Tier-4 (novel, no translation match). Fires when a PR's auto-merge queue entry is stale (>24h held behind a blocking PR). Fix: add `source=outbox-notifier, subject^=auto-merge-queue-stale:` → Tier-3 entry in `config/alert-translations.json` WITHOUT `::promoted` suffix constraint (catches both promoted and non-promoted variants). direction-ask-auto-merge-queue-stale-promoted-tier3-translation-001.json dispatched to Beacon inbox at iter ~4839. verification_pending. Occurrences: iter ~4752 (1/3, PR #857 >27h); iter ~4834 (2/3, PR #874 >24h, behind PR #847); iter ~4839 (3/3, PR #874 promoted again, bot DM'd Larry at idx=944). Note: DISTINCT from `outbox-notifier-auto-merge-queue-stale-merged-pr-001` (COMPLETE ✅ PR #893) which was about already-merged PRs being re-queued.

---

## Status snapshot — updated 2026-07-09T22:09Z UTC (Iter ~4849, **Tier 1**)

**Iter ~4849 summary (2026-07-09T22:09Z):** 1 new alert (ourliberty-health line 948, Tier-4, sync_freshness push race, Larry DM'd by notifier idx=946, G-rule vp). beacon PID 1592338; outbox_notifier PID 1592524 (alive, rate-limit clear, active — dispatched pr3-activation Mirror review 22:05Z); inbox_watcher PID 1606096. Zombie PID 1834248 (~42d+02:49:04) carry. pending=0. Sync push race carry (status=error, git HEAD=07995c52=origin/main). Stall dry-run 22:07Z: stalled_active_step:mirror-two-slot-review-001:pr3-activation (Tier-3, PR #883, Mirror review now in-flight). Watermark: 948. PRIME: ratio=20.35 (interventions=1648, systemic_fixes=81, vp=37, trend=worsening). Open PRs: #898 (Mirror review in-flight since 22:05Z), #847 (HELD), #854 (no labels), #860 (no labels), #874 (auto-review). Forge build in-flight: silence-auto-merge-queue-stale-001 (~40 min, no PR yet). dashboard PR #123 MERGED 21:58:48Z. Tier 1, consecutive_clean=0. **ACTIVE G-rules:** sentinel-inflight-stall-tier4 [VP, fix=PR #854 OPEN]; build-sequence-advancer-sequence-complete-tier4-001 [2/3]; forge-marker-task-id-mismatch-xii-v1 [2/3]; outbox-notifier-auto-merge-queue-stale-promoted-tier4-001 [DISPATCHED ✅, vp, Forge building]; outbox-notifier-merge-held-deep-review-tier4-001 [1/3]; mirror-malformed-verdict-heal-reap-path-001 [1/3]; forge-wip-redispatch-exhausted-genuine-no-pr-001 [1/3]; ourliberty-health-subject-key-mismatch-001 [3/3, vp]; forge-revision-preamble-missing-pr711-001 [VP]; unreviewed-merge-larry-authored-pr-001 [13+]; forge-wip-redispatch-digest-tier4-001 [vp]; auto-merge-queue-stale-promoted-tier3-translation [DISPATCHED ✅, vp, Forge building]. **VERIFIED (closed):** watchdog-outbox-notifier-restart-tier4-001 (PR #897 ✅); pr-fanout-probe-health-tier4-001 (PR #894 ✅); notifier-gh-rate-limit-no-backoff-001 (PR #880 ✅); heal-pipeline-stall-stalled-active-step-tier4-001 (PR #883 ✅); auto-merge-conflict-promoted-merged-pr-001 (PR #889 ✅); outbox-notifier-auto-merge-queue-stale-merged-pr-001 (PR #893 ✅); outbox-notifier-auto-merge-rate-limit-orphan-001 (PR #892 ✅); sequence-invalid-completeness-pr3-fanout-sentinel (PR #871 ✅); no-session-revision-merged-pr-fp-001 (PR #873 ✅).
