...
## G-rule outbox-notifier-auto-merge-queue-stale-merged-pr-001 → DISPATCHED ✅ (iter ~4722, 3/3)

**Rule:** outbox-notifier fires `auto_merge_queue_stale` for PRs that are already MERGED. The stale-entry check does not verify PR state before alerting. Fix: pre-stale-alert gate calls `gh pr view`; MERGED/CLOSED entries cleaned silently. direction-ask-auto-merge-queue-stale-merged-pr-3of3-001.json dispatched to Beacon inbox at iter ~4722. verification_pending. Occurrences: iter ~4696 (PR #883); iter ~4705 (PR #121 dashboard); iter ~4722 (PR #853, confirmed MERGED).

---

## G-rule outbox-notifier-auto-merge-rate-limit-orphan-001 — **2/3** (updated iter ~4705)

**Rule:** When outbox-notifier's auto-merge attempt is skipped due to GH rate-limit backoff (reason=pr-not-found), there is no retry queue — the PR remains open until a new outbox event triggers the next scan. Once rate-limit clears, the missed auto-merge is permanently orphaned unless Pulse intervenes. This is distinct from PR #880's backoff fix (which retries GH API calls, not completed-review auto-merges). Fix: outbox-notifier should maintain a "pending-auto-merge" state file for PRs where auto-merge was attempted but skipped, and re-attempt on the next scan cycle after backoff expires. Pulse recovery pattern: `gh pr merge <num> --repo <repo> --auto --squash`. Dispatch to Beacon at 3/3. Occurrences: iter ~4691 (PR #883, agent-core, consecutive=4; Pulse gh pr merge recovery); iter ~4705 (PR #121, ourliberty-dashboard, consecutive=4; Pulse gh pr merge recovery → MERGED ✅).
...
## G-rule auto-merge-conflict-promoted-merged-pr-001 → DISPATCHED ✅ (iter ~4705, 3/3)

**Rule:** Promoted `::promoted` alerts (persistence:3-cycles) fire for PRs that are already MERGED. The promoter does not check PR state before promoting. Scope: covers any `::promoted` suffix (`auto-merge-conflict:`, `auto-merge-deep-review-hold:`, `auto-merge-queue-stale:`, etc.) where the named PR is MERGED/CLOSED. `direction-ask-promoter-merged-pr-state-gate-3of3-001.json` dispatched to Beacon at iter ~4705 (03:40Z 2026-07-09). Fix: pre-promotion gate that calls `gh pr view` for PRs referenced in `::promoted` subject lines; skips and cleans queue entry silently if PR MERGED/CLOSED. verification_pending. Occurrences: iter ~3317 (PR #764, L1113); iter ~4449 (PR #830, L1004); iter ~4705 (PR #843, L1040).
...
## G-rule watchdog-outbox-notifier-restart-tier4-001 → COMPLETE ✅ (iter ~4452)

**Rule (historical):** `source=watchdog, subject=ourliberty-outbox-notifier` alerts classified Tier-4. Plan was to dispatch to Beacon at 3/3. **COMPLETE:** triage helper confirmed Tier-3 (known-pattern, last_updated=2026-06-23) at iter ~4452 for L1010. Translation was already live in alert-translations.json (added during June 23 G-rule batch). First occurrence iter ~3452. Moved to Completed G-rules.
...
## G-rule pr-fanout-probe-health-tier4-001 — 1/3 (new, iter ~4654)

**Rule:** `source=pr-terminal-fanout, subject=pr-fanout-probe-health` alerts classify Tier-4 (novel, no translation match). Fires when the PR terminal fanout probe hits errors (2/2 probes errored >20% threshold). Root cause this iter: GH API rate limit exceeded (5,000/hr shared). Bot delivers route=escalate DM; Pulse journals only, no duplicate DM. Fix: add `source=pr-terminal-fanout, subject=pr-fanout-probe-health` → Tier-3 entry to `config/alert-translations.json` (after confirming rate-limit is the expected error class, not a genuine GitHub auth failure). Dispatch to Beacon at 3/3. First occurrence iter ~4654 (2026-07-08T21:24:21Z).
...
## Status snapshot — updated 2026-07-09T06:07Z UTC (Iter ~4724, **Tier 1**)

**Iter ~4724 summary (2026-07-09T06:07Z):** ⚠️ 11 new alerts (10×Tier-3 heal-systemd-install-drift batch + 1×doorbell Tier-3). PR #891 Mirror REVIEW_ESCALATE at 23:55:43Z (test_outbox_notifier flake BLOCK; 21 failures on unmodified module; known flake class). New pending=3: mirror-review-pr2-slot-aware-healers (Beacon APPROVAL_REQUEST, awaiting Larry `approve mirror-review-pr2-slot-aware-healers`). 5 new systemd units now on system (factory-utilization, govern-loop-readiness, main-suite-guardian, mission-rank, system-resource-watch). Stall DRY-RUN FP carry (no_session_revision:promoter-pr-state-gate-001). Watermark=1060. **STANDING:** zombie PID 1834248 (~41d+10h+44m+); PR #847 OPEN (held_deep_review); PR #854 OPEN PREFLIGHT_EXIT; PR #860/874 OPEN; PR #889 OPEN (Mirror rev1+dup queued; stall DRY-RUN FP); PR #890 OPEN (Mirror rev1 queued); PR #891 OPEN (Mirror REVIEW_ESCALATE; APPROVAL_REQUEST pending); VI/VIII proposals idx=990,991 awaiting Larry; APPROVAL_REQUEST outbox-notifier-pending-auto-merge-queue-001 + fix-auto-merge-queue-stale-merged-gate-001 + mirror-review-pr2-slot-aware-healers awaiting Larry. PRIME: ratio≈22.1 (interventions≈1636, systemic_fixes=74, vp=36). **ACTIVE G-rules:** outbox-notifier-auto-merge-queue-stale-merged-pr-001 [DISPATCHED ✅ → vp, Beacon plan ready]; build-sequence-advancer-sequence-complete-tier4-001 [2/3]; forge-marker-task-id-mismatch-xii-v1 [2/3]; pr-fanout-probe-health-tier4-001 [1/3]; outbox-notifier-merge-held-deep-review-tier4-001 [1/3]; mirror-malformed-verdict-heal-reap-path-001 [1/3]; forge-wip-redispatch-genuine-no-pr-001 [1/3]; forge-wip-redispatch-exhausted-genuine-no-pr-001 [1/3]; sentinel-inflight-stall-tier4 [VP, fix=PR #854]; notifier-concurrent-scan-dup (PR #847 held, occ 8); ourliberty-health-subject-key-mismatch-001 [3/3, vp]; forge-revision-preamble-missing-pr711-001 [VP, occ 6 post-dispatch]; unreviewed-merge-larry-authored-pr-001 [13+]; forge-wip-redispatch-digest-tier4-001 [vp]; auto-merge-conflict-promoted-merged-pr-001 [DISPATCHED ✅ → vp]; outbox-notifier-auto-merge-rate-limit-orphan-001 [DISPATCHED ✅ 3/3 → vp]. **VERIFIED (closed):** sequence-invalid-completeness-pr3-fanout-sentinel (PR #871); no-session-revision-merged-pr-fp-001 (PR #873); notifier-gh-rate-limit-no-backoff-001 (PR #880 ✅); heal-pipeline-stall-stalled-active-step-tier4-001 (PR #883 ✅).
