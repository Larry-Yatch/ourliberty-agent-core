...
## G-rule outbox-notifier-auto-merge-queue-stale-promoted-tier4-001 — 2/3 (updated iter ~4834)

**Rule:** `source=outbox-notifier, subject^=auto-merge-queue-stale:` alerts classify Tier-4 (novel, no translation match). Fires when a PR's auto-merge queue entry is stale (>24h held behind a blocking PR). Fix: add `source=outbox-notifier, subject^=auto-merge-queue-stale:` → Tier-3 entry in `config/alert-translations.json`. Note: iter ~4752 occurrence (PR #857) had `::promoted` suffix; iter ~4834 occurrence (PR #874, route=hold) lacked it — fix should use `subject^=auto-merge-queue-stale:` WITHOUT `$=::promoted` constraint to catch both variants. Dispatch to Beacon at 3/3. Occurrences: iter ~4752 (1/3, PR #857 >27h); iter ~4834 (2/3, PR #874 >24h, behind PR #847). Note: DISTINCT from `outbox-notifier-auto-merge-queue-stale-merged-pr-001` (COMPLETE ✅ PR #893) which was about already-merged PRs being re-queued.

---

## Status snapshot — updated 2026-07-09T20:57Z UTC (Iter ~4838, **Tier 1**)

**Iter ~4838 summary (2026-07-09T20:57Z):** 0 new alerts. All services alive: beacon=1411813 (02:50:22), inbox_watcher=1414370 (02:48:35), outbox_notifier=1414371 (02:48:35). Zombie PID 1834248 (~42d+01:37:46) carry. pending=2 carry (~15h / ~14.2h). Stall dry-run clean (FORGE_NO_PR_SKIP ×15). GH rate-limit burst 20:37–20:40Z resolved (~17 min quiet at 20:57Z). gh-burn timers inactive (Larry DM'd idx=935). Watermark: 944. PRIME: ratio=20.32 (systemic_fixes=81, vp=36, trend=worsening); iter_clean appended 20:59:27Z. PRs #890/#891 now MERGEABLE (improved from UNKNOWN; still pending REVIEW_ESCALATE approvals). PRs #854/#847/#860/#874 OPEN (UNKNOWN). PR #897 MERGED (confirmed not in open list). Tier 1, consecutive_clean=0. **ACTIVE G-rules:** sentinel-inflight-stall-tier4 [VP, fix=PR #854 OPEN]; build-sequence-advancer-sequence-complete-tier4-001 [2/3]; forge-marker-task-id-mismatch-xii-v1 [2/3]; outbox-notifier-auto-merge-queue-stale-promoted-tier4-001 [2/3]; outbox-notifier-merge-held-deep-review-tier4-001 [1/3]; mirror-malformed-verdict-heal-reap-path-001 [1/3]; forge-wip-redispatch-exhausted-genuine-no-pr-001 [1/3]; ourliberty-health-subject-key-mismatch-001 [3/3, vp]; forge-revision-preamble-missing-pr711-001 [VP]; unreviewed-merge-larry-authored-pr-001 [13+]; forge-wip-redispatch-digest-tier4-001 [vp]. **VERIFIED (closed):** watchdog-outbox-notifier-restart-tier4-001 (PR #897 ✅); pr-fanout-probe-health-tier4-001 (PR #894 ✅); notifier-gh-rate-limit-no-backoff-001 (PR #880 ✅); heal-pipeline-stall-stalled-active-step-tier4-001 (PR #883 ✅); auto-merge-conflict-promoted-merged-pr-001 (PR #889 ✅); outbox-notifier-auto-merge-queue-stale-merged-pr-001 (PR #893 ✅); outbox-notifier-auto-merge-rate-limit-orphan-001 (PR #892 ✅); sequence-invalid-completeness-pr3-fanout-sentinel (PR #871 ✅); no-session-revision-merged-pr-fp-001 (PR #873 ✅).
