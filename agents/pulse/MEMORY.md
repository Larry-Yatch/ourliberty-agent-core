...
## G-rule sentinel-stale-lease-tier4-001 → DISPATCHED ✅ (iter ~4969)

**Rule:** `source=sentinel, subject^=stale-lease:` alerts classify Tier-4 (novel, no translation match). Fire when orphaned review-head or inbox leases from dead PIDs hit the 0.3h renewal gap after a service restart chain. All three occurrences fired from the same root cause: post-PR-merge deploy restart killed old inbox_watcher PIDs; sentinel fired on stale leases. New inbox_watcher acquires fresh leases and continues normally — self-resolving. Fix: add Tier-3 translation to `config/alert-translations.json` for `source=sentinel, subject^=stale-lease:` covering both review-head:* and inbox:* variants. direction-ask-sentinel-stale-lease-tier4-translation-001.json dispatched to Beacon inbox at iter ~4969. verification_pending. Occurrences: iter ~4968 (L918, d662604176 review-head merged PR #906); iter ~4969 (L920, 36fcb8168 review-head PR #854 orphan); iter ~4969 (L921, inbox:mirror:1 slot lease orphan).
...
## Status snapshot — updated 2026-07-10T16:55Z UTC (Iter ~4970, **Tier 1**)

**Iter ~4970 summary (2026-07-10T16:55Z):** 1 new alert (L922 Tier-3 outbox-notifier approval_request delivery confirm). watermark=922. HEAD=d04cf5bd=origin/main; clean tree. sync last_sync=16:16:14Z status=error (transient, benign). pending=1 (sentinel-stale-lease-tier3-silence-001 APPROVAL_REQUEST; chat_id=7998341473 ✅; force_ask delivered 10:51:18 MDT). PR #907 MERGED ✅ (92ba9fc3 feat(dashboard-api): team-reply badge). Beacon PID 2669988 ✅; outbox-notifier PID 2672330 ✅; inbox_watcher PID 2672329 ✅. Daemon heartbeat 16:49:21Z UTC. Zombie PID 1834248 (42d+21:33) [carry]. PR #908 (doorbell-tab-reconciler): Mirror review in flight (RECONCILE_MISSING_REVIEW dup, G-rule carry, occurrence 8). PR #905 HELD #854. PR #854/#847 long-standing HELD_DEEP_REVIEW. Check 3: "no stalls detected" (7× FORGE_NO_PR_SKIP). PRIME: iter_clean appended. Tier 1 (consecutive_clean=1). **ACTIVE G-rules:** sentinel-stale-lease-tier4-001 [DISPATCHED ✅, APPROVAL_REQUEST pending Larry]; mirror-queue-wait-gauge-tier4-001 [1/3]; sentinel-inflight-stall-tier4 [VP, PR #854]; build-sequence-advancer-sequence-complete-tier4-001 [2/3]; forge-marker-task-id-mismatch-xii-v1 [2/3]; outbox-notifier-merge-held-deep-review-tier4-001 [2/3]; heal-undispatched-pr-review-claimed-race-fp-001 [2/3]; mirror-malformed-verdict-heal-reap-path-001 [1/3]; forge-wip-redispatch-exhausted-genuine-no-pr-001 [1/3]; heal-unregistered-approval-null-chat-id-001 [1/3]; medic-escalation-recurrence-gauge-tier4-001 [1/3]; ourliberty-health-subject-key-mismatch-001 [3/3, vp]; forge-revision-preamble-missing-pr711-001 [VP]; forge-wip-redispatch-digest-tier4-001 [vp]; notifier-concurrent-scan-dup occurrence 8 (PR #847 HELD).
