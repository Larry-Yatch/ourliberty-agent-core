...
- **Steady-state degraded hold: 80 consecutive iters (377–456) with 0 pre-existing G-rule advances.** Iter 456: 0 new escalations. 0 new alerts. 0 open PRs. heal-stale-daemon-code G-rule stable at 2/3. Awaiting Larry action on APPROVAL_REQUEST `Tier 2 OAuth restore`. Cooldown files: 112 total (stable); pipeline-stall files: 27 (stable). Alert watermark: 1133. Sync RECOVERED.
