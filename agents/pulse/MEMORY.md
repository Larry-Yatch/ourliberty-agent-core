...
## G-rule pr-fanout-probe-health-tier4-001 → RE-OPENED ⚠️ (iter ~4774, occ 1/3 post-re-open)

**Rule:** `source=pr-terminal-fanout, subject=pr-fanout-probe-health` alerts classify Tier-4 (novel, no translation match). Root cause: GH API rate-limit saturation (transient, same window as outbox-notifier rate-limit WARNs; not auth failure). outbox-notifier already DMes Larry via route=escalate; Pulse journals only, no duplicate DM. Fix: add `pr-fanout-probe-health` sub-key under `pr-terminal-fanout` in `config/alert-translations.json` (severity=INFO, tier=FYI) — AS A FORGE PR, NOT a direct session edit. **Re-opened iter ~4774:** prior direction-ask-001 (dispatched 3/3 iter ~4761, processed by Beacon 10:44-10:45Z 2026-07-09) resulted in Beacon directly editing the file in its worktree session — the change was never dispatched to Forge as a PR and is NOT present in main. The "VERIFIED ✅" at iter ~4761 was a false positive (L903 triage returned Tier-3 from cached state, not from a live translation entry). direction-ask-002 re-dispatched to Beacon at iter ~4774 specifying fix MUST go through Forge PR. Occurrences: iter ~4654 (1/3); iter ~4760 (2/3); iter ~4761 (3/3, false-verify); iter ~4774 (1/3 post-re-open, direction-ask-002 sent). Watch for recurrence.
...
## Status snapshot — updated 2026-07-09T12:50Z UTC (Iter ~4775, **Tier 1**)

**Iter ~4775 summary (2026-07-09T12:50Z):** 1 new alert L910 (approval_request pr-fanout-probe-health-tier3-translation-002, Tier-3 silenced). Alert watermark: 910. pending=3 (new: pr-fanout-probe-health-tier3-translation-002 — Forge preflight for translation fix; Larry needs to `approve pr-fanout-probe-health-tier3-translation-002`). **STANDING:** zombie PID 1834248 (~41d+17h+30m+); PR #847 OPEN (HELD_DEEP_REVIEW); PR #854/860/874/891/890 OPEN; APPROVAL_REQUEST mirror-review-pr2-slot-aware-healers + mirror-review-pr-ourliberty-agent-core-890 + pr-fanout-probe-health-tier3-translation-002 awaiting Larry. **No active Forge/Mirror sessions.** PRIME: ratio≈21.06 (interventions=1643, systemic_fixes=78, vp=36). **ACTIVE G-rules:** pr-fanout-probe-health-tier4-001 [RE-OPENED, 1/3 post-re-open, chain advancing — Forge preflight in Larry queue]; build-sequence-advancer-sequence-complete-tier4-001 [2/3]; forge-marker-task-id-mismatch-xii-v1 [2/3]; outbox-notifier-merge-held-deep-review-tier4-001 [1/3]; mirror-malformed-verdict-heal-reap-path-001 [1/3]; forge-wip-redispatch-genuine-no-pr-001 [1/3]; forge-wip-redispatch-exhausted-genuine-no-pr-001 [1/3]; outbox-notifier-auto-merge-queue-stale-promoted-tier4-001 [1/3]; sentinel-inflight-stall-tier4 [VP, fix=PR #854]; notifier-concurrent-scan-dup (PR #847 held, occ 10+); ourliberty-health-subject-key-mismatch-001 [3/3, vp]; forge-revision-preamble-missing-pr711-001 [VP, occ 7 post-dispatch]; unreviewed-merge-larry-authored-pr-001 [13+]; forge-wip-redispatch-digest-tier4-001 [vp]. **VERIFIED (closed):** dispatch-branch-cleanup-gh-unavailable-001 (CLOSED ✅ iter ~4768); outbox-notifier-auto-merge-queue-stale-merged-pr-001 (PR #893 ✅); outbox-notifier-auto-merge-rate-limit-orphan-001 (PR #892 ✅); sequence-invalid-completeness-pr3-fanout-sentinel (PR #871 ✅); no-session-revision-merged-pr-fp-001 (PR #873 ✅); notifier-gh-rate-limit-no-backoff-001 (PR #880 ✅); heal-pipeline-stall-stalled-active-step-tier4-001 (PR #883 ✅); auto-merge-conflict-promoted-merged-pr-001 (PR #889 ✅).
