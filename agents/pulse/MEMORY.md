...
- **2026-05-21 (iter 60) — inbox-watcher MemoryMax raised 2G→4G→8G. ROOT CAUSE CONFIRMED 2026-05-25 (off-cycle investigation). FIX A+B SHIPPED PR #102 (2026-05-25T18:34Z).** Root cause: 82% page cache from dashboard worktrees with ~720MB node_modules each. Fix A: worktree retention 24h→4h (`scripts/cleanup_stale_worktrees.py:83` MAX_AGE_SECONDS=14400). Fix B: MemoryHigh=3G systemd drop-in. Both shipped in PR #102. Monitoring: close after 5+ cycles show no memory.events.max growth from worktree page cache accumulation. Check after next heavy build burst.
