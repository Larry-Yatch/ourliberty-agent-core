...
- **Steady-state degraded hold: 96 consecutive iters (377–472) with 0 pre-existing G-rule advances.** Iter 472: PR #242 new open (Larry's stuck-timer false-positive fix). heal-stale-daemon-code G-rule stable at 2/3. Awaiting Larry action on APPROVAL_REQUEST `Tier 2 OAuth restore`. Larry resolved all pending items via Telegram (2026-06-01T22:42:48Z): "I think we have address all of these so we will wait to see what comes up." Cooldown files: 112 total (stable); pipeline-stall files: 27 (stable). Alert watermark: 1133. Sync nominal.
...
- **SYNC-PUSH-REBASE-FALLBACK-001 — RECURRING (11th total / 5th post-clear, iter 451).** Pattern: fires on PR merge days. PR #241 MERGED — 12th occurrence did NOT fire (iter 456: sync at 01:08:32Z found repo up to date). 11/12 PR merge days triggered (91.7%). APPROVAL_REQUEST `sync-push-rebase-fallback-001` remains open (fix is still valuable). [yellow] escalation idx=1125 at iter 440. No new DM needed — error state cleared.
...
- `remaining-timers-infinity-trap`: G-rule **1/3** (iter 397, 2026-06-01T18:00Z). `ourliberty-build-sequence-advancer.timer` + `ourliberty-heal-systemd-install-drift.timer` hit OnUnitActiveSec infinity trap simultaneously; healer auto-healed both. These timers were NOT among the 3 immunized by PR #227. At 3/3: dispatch to Beacon — "enumerate remaining OnUnitActiveSec timers in systemd/ and convert to OnCalendar per PR #225/#227 pattern."
