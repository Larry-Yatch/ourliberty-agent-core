...
## Status snapshot — updated 2026-07-11T03:21Z UTC (Iter ~5017, **Tier 1**, consecutive_clean=0)

**Iter ~5017 summary (2026-07-11T03:21Z):** 2 new alerts. PR #918 MERGED by Larry (03:10:41Z UTC). PR #874 stuck `held_stale_regression` (outbox-notifier killed mid-revalidation via SIGTERM; failing closed). PR #922 now blocked by #874. Beacon + outbox-notifier restarted (new PIDs 3663513, 3662991). Pulse sent [yellow] escalation for PR #874. Tier reset 3→1. Zombie PID 1834248 (43d+07:58h). pending=6. consecutive_clean=0. ratio ~19.963 +1 intervention +1 systemic_fix. **NEW G-rule:** outbox-notifier-auto-merge-stale-revalidation-tier4-001 [1/3]. **ACTIVE G-rules:** outbox-notifier-merge-held-deep-review-tier4-001 [3/3 DISPATCHED ✅, vp]; heal-undispatched-pr-review-claimed-race-fp-001 [PR #912 MERGED ✅, vp]; notifier-concurrent-scan-dup [VERIFIED ✅ PR #918, monitoring]; mirror-queue-wait-gauge-tier4-001 [1/3]; build-sequence-advancer-sequence-complete-tier4-001 [2/3]; forge-marker-task-id-mismatch-xii-v1 [2/3]; outbox-notifier-merge-conflict-manual-rebase-tier4-001 [2/3]; forge-wip-redispatch-exhausted-genuine-no-pr-001 [2/3]; outbox-notifier-notification-intent-review-escalate-tier4-001 [2/3]; RECONCILE_MISSING_REVIEW-.claimed-blindspot [2/3]; mirror-malformed-verdict-heal-reap-path-001 [1/3]; heal-unregistered-approval-null-chat-id-001 [1/3]; inbox-watcher-tier-pool-all-unavailable-tier4-001 [1/3]; heal-pipeline-stall-unrouted-deep-review-required-fp-001 [1/3]; heal-pulse-check-staleness-single-flight-skip-fp-001 [1/3]; outbox-notifier-auto-merge-stale-revalidation-tier4-001 [1/3 NEW]; ourliberty-health-subject-key-mismatch-001 [3/3, vp]; forge-revision-preamble-missing-pr711-001 [vp]; forge-wip-redispatch-digest-tier4-001 [vp]; forge-wip-redispatch-exhausted-pr-exists-fp-001 [APPROVAL_REQUEST QUEUED iter ~3279, vp]; decision-needed-approval-forge-dispatch-no-target-repo-001 [vp]; no-session-revision-active-mirror-session-fp-001 [vp].


## G-rule outbox-notifier-auto-merge-stale-revalidation-tier4-001 — 1/3 (new, iter ~5017)

**Rule:** `source=outbox-notifier, subject^=auto-merge-stale-revalidation:` alerts classify Tier-4 (novel, no translation match). Fires when outbox-notifier detects a held PR's approval went stale after a base change (overlapping PR merged), then fails to re-validate the regression gate (here: SIGTERM exit -15 during heal-stale-daemon-code restart). Failing closed: notifier emits `held_stale_regression` and route=hold (DM suppressed). Outbox-notifier already retries on next sweep; Pulse DM is duplicate noise. Fix at 3/3: add `source=outbox-notifier, subject^=auto-merge-stale-revalidation:` → Tier-3 entry to `config/alert-translations.json`. First occurrence iter ~5017 (L988, PR #874, 03:15:23Z UTC, #918 merge triggered cascade).
