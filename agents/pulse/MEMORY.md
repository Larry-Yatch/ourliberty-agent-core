...
- **2026-05-30 (iter 120) — dashboard-dispatch-source-blocked REGRESSION.** PR #207 fixed ALLOWED_SOURCES in dispatch_validator.py at 22:32:51Z. But inbox-watcher restarted at 22:36:44Z and loaded the updated file BEFORE sync wrote it to disk (sync updated dispatch_validator.py at 22:39:20Z). Result: inbox-watcher is running old code with no 'dashboard' in ALLOWED_SOURCES. 34 `larry-reject-*.json` in beacon/.invalid/. [yellow] DM sent. ask-then-do: restart inbox-watcher (after auth-setup-token-wiring completes) + re-deposit recent .invalid/ files. Close when inbox-watcher restarted and new dashboard actions go through.
