...
## G-rule no-session-revision-merged-pr-fp-001 — MERGED ✅ (PR #873, iter ~4644)

**Rule:** `heal_pipeline_stall.py` dry-run fires `no_session_revision` for tasks whose corresponding PR is already MERGED. Root cause: `check_revision_dispatched_with_no_session` has no skip-on-merged guard (unlike `check_forge_built_no_pr` which has FORGE_NO_PR_SKIP). `direction-ask-no-session-revision-merged-pr-3of3-001.json` dispatched to Beacon at iter ~4627. Fix: add skip-on-merged check to `check_revision_dispatched_with_no_session`. **PR #873 MERGED e5ca9124 at iter ~4644 (Mirror REVIEW_PASS + pipeline auto-merge). Fix live in scripts/heal_pipeline_stall.py (+77 lines +tests).** verification_pending live behavior. Occurrences: iter ~2676 (PR #693, 1/3); iter ~3272 (PR #753 MERGED, dry-run FP, 2/3); iter ~4627 (PR #865 MERGED 17:44:10Z, dry-run FP, 3/3).
...
## G-rule heal-pipeline-stall-stalled-active-step-tier4-001 — 1/3 (new, iter ~4608)

**Rule:** `source=heal-pipeline-stall, subject^=stalled-active-step:` alerts classify Tier-4 (novel, no translation match). These are delivered to Larry via route=escalate by the bot pipeline; Pulse's Tier-4 triage creating a duplicate DM is noise. Fix: add Tier-3 translation for `source=heal-pipeline-stall, subject^=stalled-active-step:` in `config/alert-translations.json`. Dispatch to Beacon at 3/3. First occurrence iter ~4608 (L989, stalled-active-step:completeness-pr3-fanout-sentinel:completeness-pr3-build, 39 min dispatched, DM delivered at 09:53:02 MDT).
...
## G-rule sequence-invalid-completeness-pr3-fanout-sentinel → MERGED ✅ (PR #871, iter ~4644)

**Rule:** `source=build-sequence-advancer, subject=sequence-invalid:completeness-pr3-fanout-sentinel` fires on every advancer run because the sequence is paused (dispatch_text 565>500 chars) and the advancer re-alerts even with no state change. Dispatched `direction-ask-build-seq-advancer-refire-paused-3of3-001.json` to Beacon at 3/3 (iter ~4536). Fix: (a) suppress re-alerts when sequence.status==paused for same error; (b) fix dispatch_text ≤500 chars. **PR #871 MERGED 2d7ab96f at iter ~4644 (Pulse squash-merged; Mirror REVIEW_PASS confirmed 13:33:42 MDT; rate-limit storm prevented outbox-notifier auto-merge at pass time).** Fix live in scripts/build_sequence_advancer.py (+91 lines +tests). verification_pending live behavior. Occurrences: iter ~4534 (1/3), iter ~4535 (2/3), iter ~4536 (3/3).
...
## Status snapshot — updated 2026-07-08T20:12Z UTC (Iter ~4644, **Tier 1**)

**Iter ~4644 summary (2026-07-08T20:12Z):** 1 new alert (L1008, forge-wip-redispatch:retry1, Tier-4 journal-only). watermark=1008. git HEAD=2d7ab96f=origin/main (clean+up-to-date). Daemon heartbeat 20:04:45Z (~8 min). Watchdog 14:02:16 MDT overall=healthy. Pipeline stall dry-run: 2 alerts would fire (PR#871 mirror_pass_unmerged FIXED; stalled_active_step:suite-green-guardian:pr1-detector-shadow pending stall timer). PR #871 squash-merged 20:12:23Z (Mirror REVIEW_PASS, rate-limit skip recovery). PR #873 merged via pipeline (heal-no-session-revision-skip). **STANDING:** zombie PID 1834248 (~41d+0h+47m); PR #847 AUTO_MERGE_HELD; VI/VIII proposals idx=990,991 awaiting Larry; stalled_active_step:suite-green-guardian:pr1-detector-shadow (stall timer pending). Beacon: EMPTY. Forge: build-pr1-detector-shadow.json (active). Mirror: 8 reviews (heal-no-session-revision-pending-cleanup, live-sys-build-seq-rev1, live-sys-build-seq-dup, #874, #875, #876, #877, suite-green-guardian-retry1). pending=0. PRIME: ratio≈21.64, trend worsening. **ACTIVE G-rules:** build-sequence-advancer-sequence-complete-tier4-001 [1/3]; heal-pipeline-stall-stalled-active-step-tier4-001 [→2/3 when stall fires]; outbox-notifier-merge-held-deep-review-tier4-001 [1/3]; sentinel-inflight-stall-tier4 [VP, fix=PR #854]; notifier-concurrent-scan-dup (PR #847 held); auto-merge-conflict-promoted-merged-pr-001 [2/3]; forge-marker-task-id-mismatch-xii-v1 [2/3]; ourliberty-health-subject-key-mismatch-001 [3/3, vp]; mirror-malformed-verdict-heal-reap-path-001 [1/3]; forge-revision-preamble-missing-pr711-001 [VP]; unreviewed-merge-larry-authored-pr-001 [12+]; forge-wip-redispatch-digest-tier4-001 [vp]; notifier-gh-rate-limit-no-backoff-001 [2/3, PR#871 stall consequence]. **MERGED this iter (verify live):** sequence-invalid-completeness-pr3-fanout-sentinel (PR #871); no-session-revision-merged-pr-fp-001 (PR #873). **WATCHING:** promoter-stale-sequence-dag-pass-promoted-001 [1/1]; forge-wip-redispatch-mirror-dag-preflight-FP [1/1] (healer mis-identifies completed Mirror DAG preflight as WIP-only abandoned).
