...
## G-rule watchdog-outbox-notifier-restart-tier4-001 → RE-OPENED ⚠️ (iter ~4816, dispatch v2 sent)

**Rule:** `source=watchdog, subject=ourliberty-outbox-notifier` alerts fire during the ~83s restart window when heal-stale-daemon-code issues a `systemctl restart` for outbox-notifier. Watchdog catches the brief DOWN state; service recovers via SystemD RestartSec+ordering in ~83s. Triage helper: Tier-4 (no translation match). Fix: add `watchdog` → `ourliberty-outbox-notifier` sub-key to `config/alert-translations.json` (tier=FYI, summary: restart-window artifact).

**COMPLETE claim (iter ~4452) was STALE:** alert-translations.json watchdog section only has `bots:*:*` sub-keys; no `ourliberty-outbox-notifier` entry. The prior COMPLETE claim was based on triage helper returning Tier-3 for L1010 — that match must have been via a different mechanism (now absent). VERIFIED ABSENT iter ~4816 by reading config/alert-translations.json directly.

**direction-ask-watchdog-outbox-notifier-tier3-002.json** dispatched to Beacon inbox at iter ~4816. systemic_fix appended to PRIME ledger. **Larry approved ("Go") at 12:21:19 MDT 2026-07-09 (18:21:19Z UTC). Beacon dispatched `watchdog-outbox-recovered-subject-001` to Forge inbox; inbox-watcher picked up as `build-watchdog-outbox-recovered-subject-001.json` (iter ~4818). Forge build in progress.** verification_pending (PR merge + Tier-3 return confirmed). Occurrence iter ~3452 (1/3 original); iter ~4816 (1/3 new series).
...
## Status snapshot — updated 2026-07-09T18:53Z UTC (Iter ~4822, **Tier 1**)

**Iter ~4822 summary (2026-07-09T18:53Z):** Nominal. 1 alert (L940 review-pass PR #897, Tier-3 silence). PR #897 Mirror REVIEW_PASS ✅ at 18:45:51Z; AUTO_MERGE_HELD behind PR #854 (overlap: config/alert-translations.json, scripts/tests/test_watchdog.py, scripts/watchdog.py). All services alive: beacon=1411813 (45:33 Ss), inbox_watcher=1414370 (43:46 Ssl), outbox_notifier=1414371 (43:46 Ss). Zombie PID 1834248 (~41d23h32m+) carry. pending=2 carry (~17h). gh-burn timers inactive (Larry DM'd idx=935). Watermark: 940. PRIME: ratio≈20.55 (interventions=1644, systemic_fixes=80, vp=36; iter_clean appended 18:53:25Z). **PIDs:** inbox_watcher=1414370, outbox_notifier=1414371, beacon=1411813, zombie=1834248 (~41d23h32m+). **ACTIVE G-rules:** sentinel-inflight-stall-tier4 [VP, fix=PR #854 OPEN — also blocking PR #897 auto-merge]; watchdog-outbox-notifier-restart-tier4-001 [PR #897 Mirror REVIEW_PASS, AUTO_MERGE_HELD behind PR #854, vp]; build-sequence-advancer-sequence-complete-tier4-001 [2/3]; forge-marker-task-id-mismatch-xii-v1 [2/3]; outbox-notifier-auto-merge-queue-stale-promoted-tier4-001 [2/3]; outbox-notifier-merge-held-deep-review-tier4-001 [1/3]; mirror-malformed-verdict-heal-reap-path-001 [1/3]; forge-wip-redispatch-exhausted-genuine-no-pr-001 [1/3]; ourliberty-health-subject-key-mismatch-001 [3/3, vp]; forge-revision-preamble-missing-pr711-001 [VP]; unreviewed-merge-larry-authored-pr-001 [13+]; forge-wip-redispatch-digest-tier4-001 [vp]. **VERIFIED (closed):** pr-fanout-probe-health-tier4-001 (PR #894 ✅ iter ~4813); notifier-gh-rate-limit-no-backoff-001 (PR #880 ✅); heal-pipeline-stall-stalled-active-step-tier4-001 (PR #883 ✅); auto-merge-conflict-promoted-merged-pr-001 (PR #889 ✅); outbox-notifier-auto-merge-queue-stale-merged-pr-001 (PR #893 ✅); outbox-notifier-auto-merge-rate-limit-orphan-001 (PR #892 ✅); sequence-invalid-completeness-pr3-fanout-sentinel (PR #871 ✅); no-session-revision-merged-pr-fp-001 (PR #873 ✅).
