...
## G-rule heal-stale-daemon-entrypoint-not-tracked-001 — 1/3 (new, iter ~5139)

**Rule:** heal-stale-daemon-code detects when a service's IMPORTED shared libraries change and restarts it — but does NOT detect when the service's own entrypoint script changes. When inbox_watcher.py was modified in PR#935 (origin_task_id allow-list added to `_build_outbox`), the healer ran at 19:17:10Z and left PID 3940207 running stale code. inbox_watcher remains on pre-PR#935 code until manually restarted or until a shared library it imports changes. Impact: delegated card flows won't propagate origin_task_id through inbox_watcher for new outbox entries. Fix: healer should also compare service entrypoint mtime against service start time (same logic as shared library check). Dispatch to Beacon at 3/3. First occurrence iter ~5139.

---

## Status snapshot — updated 2026-07-11T19:17Z UTC (Iter ~5139, **Tier 1**, consecutive_clean=0)

**Iter ~5139 summary (2026-07-11T19:17Z):** 0 new alerts, all checks nominal. New finding: heal-stale-daemon-entrypoint-not-tracked-001 [1/3] — healer ran at 19:17:10Z and did not restart inbox_watcher after PR#935 changed its entrypoint script. PR #860 CONFLICTING. gh-burn-phase2 build in Forge inbox (no PR yet). inbox_watcher PID 3940207 still on pre-PR#935 code. HEAD=b433be7f=origin/main. **ACTIVE G-rules:** heal-stale-daemon-entrypoint-not-tracked-001 [1/3 NEW]; outbox-notifier-notification-intent-reject-tier4-001 [3/3 DISPATCHED vp]; outbox-notifier-merge-conflict-manual-rebase-tier4-001 [2/3]; forge-wip-redispatch-exhausted-genuine-no-pr-001 [2/3]; outbox-notifier-notification-intent-review-escalate-tier4-001 [2/3]; outbox-notifier-auto-merge-stale-revalidation-tier4-001 [2/3]; mirror-malformed-verdict-heal-reap-path-001 [1/3]; mirror-queue-wait-gauge-tier4-001 [1/3]; inbox-watcher-tier-pool-all-unavailable-tier4-001 [1/3]; heal-pipeline-stall-unrouted-deep-review-required-fp-001 [1/3]; heal-pulse-check-staleness-single-flight-skip-fp-001 [1/3]; gate-parallelism-monitor-regression-data-001 [1/3]; pulse-rotation-check-source-tier4-001 [1/3]; ourliberty-health-subject-key-mismatch-001 [3/3, vp]; forge-revision-preamble-missing-pr711-001 [vp]; forge-wip-redispatch-digest-tier4-001 [vp]; forge-wip-redispatch-exhausted-pr-exists-fp-001 [APPROVAL_REQUEST QUEUED iter ~3279, vp]; decision-needed-approval-forge-dispatch-no-target-repo-001 [vp]; no-session-revision-active-mirror-session-fp-001 [vp]. unreviewed-merge-larry-authored-pr-001: auto-route fix REJECTED (already-implemented); label-application gap back to Beacon (Step 2 from iter ~3372).
