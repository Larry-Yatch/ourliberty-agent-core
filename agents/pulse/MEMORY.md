...
## G-rule sync-service-deploy-restart-head-drift-tier4-no-translation-001 — RE-OPENED 2/3 (iter ~9780, 2026-08-25T07:40Z UTC; +1 iter ~10130, 2026-08-28T09:39Z UTC)

**Rule:** `source=sync.service, subject=deploy-restart-head-drift` returns Tier-4 from the triage helper (no translation match in config/alert-translations.json). Iter ~9780 (2026-08-25T07:40Z UTC): guard-tier4 confirmed authoritative Tier-4 (same-iter triage-alert call + classify()==4). Iter ~10130 (2026-08-28T09:39:10Z UTC): 2nd occurrence — automated cycle claimed (line 500 larry-alerts.jsonl), outbox-notifier delivered idx=510. Alert fires each Pulse-commit cycle when sync.service sees local HEAD moved to new commit but deploy target is still the prior commit — condition self-resolves on the same sync tick. Outbox_notifier already delivers the alert directly (Larry sees it as a raw FYI). Fix: add Tier-3 (digest/silence) translation entry for `source=sync.service, subject=deploy-restart-head-drift` in config/alert-translations.json. **Dispatch to Beacon at 3/3** (next expected after next Pulse commit cycle).
