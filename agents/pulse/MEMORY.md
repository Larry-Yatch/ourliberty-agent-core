...
- **Monday [yellow] DM: PRE-FIRED iter 259.** Scope: Tier 2 OAuth ELEVATED (active stalls; Beacon notify stalled) + sync-push-rebase-fallback-001 ELEVATED + Check VIII/IX first-firing BLOCKED (timer stuck) + pulse-grule-prompt-template-001 (F24 fix, doc-only, APPROVAL_REQUEST #8). Delivered via `larry_alerts.append_alert(source=pulse, subject=monday-dm:2026-06-01-stuck-timer)`.
