...
## G-rule main-suite-guardian-skip-no-heartbeat-001 — 1/3 (new, iter ~4881)

**Rule:** `heal-pulse-check-staleness` fires `pulse-check-stale:main-suite-guardian` when `main_suite_guardian.py` skips its nightly run due to `ol-regbaseline-warm.lock` being held (single-flight guard). Skip path does NOT emit a heartbeat → staleness healer declares the check stale (FP). **Ground truth:** `ourliberty-main-suite-guardian.timer` IS installed (active/waiting, next fire ~03:35Z UTC 2026-07-11). Service ran 2026-07-09T21:33:14 MDT (03:33:14Z UTC) — exited 0 but skipped. Fix: emit a skip-heartbeat or `lock-held-skip` signal in `main_suite_guardian.py`'s skip path so the staleness healer sees a valid check-in. Dispatch to Beacon at 3/3. First occurrence iter ~4881 (L985, 06:01:55Z UTC).

---

## Status snapshot — updated 2026-07-10T10:27Z UTC (Iter ~4915, **Tier 1**)

**Iter ~4915 summary (2026-07-10T10:26Z):** 1 new alert (line 890: pulse-check catalog-accuracy-drift, Tier-3 silence; watermark→890). Check XI fired 10:21:21Z UTC: **8/64 drifted (12.5%, gate 10%, over_gate=true) — improved from 24/64 (37.5%) yesterday**. Drifted: atomic_io, chain_event_shipper, dashboard_api, human-approval-gate, larry_alerts, medic_ledger, task_terminal_state, universal-card. Bot delivered route=digest. Check 3 clean (10:26Z dry-run; "no stalls detected"; 8× FORGE_NO_PR_SKIP, 1× MIRROR_PASS_UNMERGED_SKIP PR #904 held_deep_review). outbox-notifier 401 carry (escalated iter ~4883; PID 1881715 alive; last notifier.log 10:08:45Z UTC — same as iter ~4914). Repo clean (HEAD=53052871=origin/main). Sync 10:11:07Z UTC (~16 min, within 2h). Daemon heartbeat 10:25:54Z UTC. pending=1 `unreg-approval-f5079f4c5369` (chat_id=null — carry). Check I fires today at ~14:10Z UTC; latest artifact: check-i-2026-07-08.json. All daemons alive. Zombie PID 1834248 (~42d+15:07). PRIME: iter_clean appended (10:27:36Z). Ratio=20.46, trend=worsening. Tier 1 (consecutive_clean=0). **ACTIVE G-rules:** sentinel-inflight-stall-tier4 [VP, PR #854 OPEN]; build-sequence-advancer-sequence-complete-tier4-001 [2/3]; forge-marker-task-id-mismatch-xii-v1 [2/3]; outbox-notifier-merge-held-deep-review-tier4-001 [2/3]; mirror-malformed-verdict-heal-reap-path-001 [1/3]; forge-wip-redispatch-exhausted-genuine-no-pr-001 [1/3]; heal-undispatched-pr-review-claimed-race-fp-001 [1/3]; heal-unregistered-approval-null-chat-id-001 [1/3]; medic-escalation-recurrence-gauge-tier4-001 [1/3]; main-suite-guardian-skip-no-heartbeat-001 [1/3]; ourliberty-health-subject-key-mismatch-001 [3/3, vp]; forge-revision-preamble-missing-pr711-001 [VP]; unreviewed-merge-larry-authored-pr-001 [13+]; forge-wip-redispatch-digest-tier4-001 [vp]; notifier-concurrent-scan-dup (PR #847 HELD).
