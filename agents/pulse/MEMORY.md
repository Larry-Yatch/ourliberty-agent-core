...
**system-health.json PATH + bots section (CORRECTED iter ~10736, re-verified iter ~11153 — bots path corrected):** Correct path is `/home/larry/agents/blackboard/system-health.json` — NOT `state/`. Reading from `state/` returns NOT FOUND. Top-level keys are `['timestamp', 'checks', 'overall']` — **NO top-level `bots` key** (the iter ~10736 note was wrong on this point). Bots section is at `d['checks']['bots']['bots']` — a dict keyed by agent name (beacon/forge/mirror/pulse) with fields `desired`, `alive`, `unit`, `action`. Access: `d["checks"]["bots"]["bots"][agent]["alive"]`. Bots run under systemd, not tmux — `tmux ls` returns "no tmux sessions" and that is expected. Fall back to overall=healthy + Check 1 only if bots section missing.

**Suite guardian vs daemon-code timestamp confusion (iter ~11506, 2026-09-14T22:21Z UTC):** `heal-stale-daemon-code.heartbeat` (Check 5) and `pulse-check-main-suite-guardian.heartbeat` are DIFFERENT files that can share the same second timestamp if both healers happen to tick simultaneously. When verifying the suite guardian, `cat` the suite guardian file explicitly and check its `"check": "main-suite-guardian"` field. Do NOT assume the timestamp reported in a prior iter's journal for the suite guardian is correct if you haven't re-read the file — iter ~11505 incorrectly cited 21:54:51Z UTC for the suite guardian when that was the daemon-code heartbeat; the actual suite guardian was at 03:50:54Z UTC (normal nightly run). The suite guardian fires nightly at ~03:38–03:49Z UTC only; any non-nightly timestamp for the suite guardian should be treated as suspect until verified.
...
## G-rule heal-approvals-surface-drift-missing-card-cooldown-collision-001 — DISPATCHED (iter ~7585) → FALSE PREMISE → IMPL DISPATCHED (iter ~8237) → G-RULE 3/3 RE-HIT, FRESH DISPATCH (iter ~11297, 2026-09-10T02:44Z UTC)

**Rule (retracted):** Original dispatch assumed the root cause was cooldown-collision: approval cards created before cooldown engages → promote predicate fails → `missing_card` flood. **BEACON FOUND FALSE PREMISE (iter ~7586):** The REAL bug is the Approvals tab has a binary-only contract — `needs_larry` alerts whose `suggested_action` is a runbook string (not an A/B choice) hit `SKIP_NEEDS_TRIAGE` and are permanently barred from the Approvals tab. **Larry chose Option B** (dashboard reject = Option B, 2026-08-05): widen the tab so non-binary `needs_larry` alerts become informational cards with a single Acknowledge button. Spec shipped as PR#1102 (commit cd886496, 2026-08-06). **G-rule `heal-approvals-surface-drift-tier4-nonbinary-001` hit 3/3 at iter ~8237 (2026-08-07T01:37Z UTC):** direction-ask `direction-ask-approvals-opt-b-implement-001` dispatched to Beacon inbox to implement 3 steps: step-verb (ourliberty-agent-core, acknowledge verb + guards + ledger semantics, no deps), step-render (ourliberty-dashboard, informational card UI + Acknowledge button, no deps — parallel with step-verb), step-promote (ourliberty-agent-core, promoter mints informational cards + retire_needs_triage_cards removed + skip fingerprinting + resolution-on-acknowledge, depends on step-verb AND step-render merged). Full spec at agents/beacon/specs/approvals-tab-informational-cards.md. Missing-card drift will continue firing until step-promote merges. **Do NOT add a Tier-3 silence translation for this alert class** — that would gag a legitimate checker.

**Fresh G-rule 3/3 hit (iter ~11297, 2026-09-10):** Three consecutive `heal-approvals-surface-drift:missing_card` alerts for RSDPM PR#246 (pipeline-stall:unrouted-pr:PR#246) fired over 2026-09-08 through 2026-09-10 (unreg-approval-06211b4e2d66, unreg-approval-604e0aa4b8d4, unreg-approval-e3d6de4d84e2). All Tier-4 (novel). Indicates step-promote has NOT yet landed on main as of iter ~11297. Fresh direction-ask `direction-ask-approvals-surface-drift-rsdpm246-status-001` written to Beacon inbox at ~02:44Z UTC — Beacon to assess which Option B steps are merged and whether a heal_unregistered_approval.py code fix is needed. **Do NOT re-dispatch.** Track Beacon's response; when step-promote merges, close this G-rule entry.
...
## G-rule outbox-notifier-approval-request-tier4-no-translation-001 — FALSE PREMISE CLOSED (Beacon post-iter-~8174)

**Rule (retracted — false premise):** The triage-helper Tier-4 is generated by a fabricated non-null subject, not a missing translation. The translation for `source=outbox-notifier, kind=approval_request` already shipped in **PR #491 (d3f88523, 2026-06-13)** (kind-fallback in `_translation_match`; `classify()` returns `tier=3 / silence`). The 3 cited occurrences (Aug 5–6) do NOT exist in the live larry-alerts.jsonl; the producer went silent 2026-06-30. Mechanism: only `(kind=approval_request, subject=<task_id>)` reaches Tier-4 — a composed/fabricated subject defeats the kind-fallback. `guard-tier4-payload-fidelity-001` already covers this path (source-agnostic triple-match check). **Zero dispatch. Do NOT add a translation entry for an already-present key. Do NOT re-open this G-rule.** Fourth pre-dispatch check added: confirm rows of the described shape exist in the CURRENT larry-alerts.jsonl window before filing.
...
## G-rule check-v-auto-fix-patterns-no-commit-path-001 → RESOLVED ✅ (PR #1093, iter ~7374)

**Rule:** Check V systemd timer writes graduation tracking data (clean_streak counter updates) to `config/auto-fix-patterns.json`, but that file is NOT in `PULSE_RUNTIME_PATHS` or `SYNC_EXTRA_RUNTIME_PATHS`. Result: run_cycle.sh's stray-edit guard detects it as non-managed dirt, archives the diff, and reverts the file — losing Check V's streak data each cycle. First occurrence: iter ~7374 (2026-08-03T11:08Z UTC). The clean_streak for `auto-merge-clean-pr` updated 0→338 was reverted by the stray-edit guard. Symptom also seen: Check A fires as dirty-tree finding each time Check V runs. **RESOLVED by PR #1093 (MERGED 2026-08-03, `b8c9e3a5`) — do NOT dispatch, do NOT reopen.** NEITHER proposed fix was taken. The write itself was removed: `run_promotion_loop` now calls `reconcile_streaks(doc, store)` WITHOUT `save_registry(doc)`, so Check V no longer writes that file at all. This was safe because `clean_streak` is write-only — `find_graduation_candidates()` recomputes the streak fresh from the executions store on every run and no code anywhere reads the stored field (verified at review; the value was 0 in every commit that ever touched the file, so nothing was ever actually "lost"). **Option (a) is explicitly forbidden:** PULSE_RUNTIME_PATHS defines what Pulse may do unsupervised and must get NARROWER, not wider; `test_promotion_loop_does_not_persist_the_streak_cache` pins this. Expect zero further stray-edit incidents and no Check A dirty-tree finding from Check V. `config/auto-fix-patterns.json` will read `clean_streak: 0` forever BY DESIGN.
...
## heal-stale-daemon-code.heartbeat is at ~/agents/blackboard/ NOT ~/agents/state/ (learned iter ~9110, 2026-08-11)

**Rule:** Check 5 heartbeat is `/home/larry/agents/blackboard/heal-stale-daemon-code.heartbeat` (blackboard dir), NOT `~/agents/state/`. Prior Check 5 reads checking `~/agents/state/` returned NOT FOUND (path was wrong). Content: ISO 8601 timestamp of last heal_stale_daemon_code.py successful tick (updated each service run). The systemd service `ourliberty-heal-stale-daemon-code.service` is a one-shot timer-triggered service; `inactive (dead)` status after a successful run is normal — verify via heartbeat timestamp, not service Active state.
...
**DM history (UPDATED iter ~11332, 2026-09-11T09:46Z UTC):** SUPABASE_SERVICE_ROLE_KEY last DM = **2026-09-09T01:48:59Z UTC** (as read from pulse-rotation-window-dms.json at state/ path). Dedup window ACTIVE until **2026-09-23T01:49Z UTC**. next_rotation_due=**2026-08-22** (cadence_days=90 from last_rotated_at=2026-05-24) — currently **~20 days overdue** as of 2026-09-11. Do NOT report `credential-rotation-dedup.json` as missing — it never existed.

**PATH TRAP (RECURRED iters ~11322 and ~11332):** Multiple manual-session iters falsely reported "pulse-rotation-window-dms.json NOT FOUND" because they checked `/home/larry/agents/blackboard/` instead of the correct `/home/larry/agents/state/` path. ALWAYS use state/ path. This MEMORY.md already documents it — if you see NOT FOUND, re-read MEMORY and correct the path first.
...
## Check I firing days are Mon/Wed/Fri/Sun — call WITHOUT --force on firing days (learned 2026-06-15 iter ~1899, updated iter ~2612, timing corrected iter ~5871)

**Rule:** Check I fires on Mon/Wed/Fri/Sun per spec (UTC weekday ∈ {0,2,4,6}). Always invoke `python3 ~/agent-core/scripts/pulse_check_i.py` (no `--force`) on scheduled firing days — the weekday gate passes naturally and the dm_route journal-peek (PR #674) functions correctly, suppressing repeat same-week DMs. Use `--force` ONLY for `/optimize` (ad-hoc, any day). Using `--force` on a firing day bypasses dm_route and emits spurious route=escalate alerts (G-rule check-i-force-bypass-dm-route). Confirmed fixed at manual level iter ~2612; code-level dispatch to Beacon at 3/3. **Timing:** The systemd timer fires at ~08:13 MDT (UTC-6) = ~14:13 UTC. Artifact timestamps in `pulse-check-i/` are local MDT, NOT UTC. Do NOT confuse "artifact written at 08:13 local time" with "08:13Z UTC" — prior iters made this error (iter ~5870 said "timer fired ~08:13Z UTC" when the timer actually fires mid-afternoon UTC). Correct assertion: "Check I timer fires ~14:13 UTC on Mon/Wed/Fri/Sun."
...
## G-rule outbox-notifier-merge-held-deep-review-tier4-001 → COMPLETE ✅ (PR #927, iter ~5054)

PR #927 (`chore(config): tier-3-silence merge_held_deep_review deep-review-hold alert`) MERGED 7a754dfd at 01:52:26 MDT 2026-07-11. Translation `source=outbox-notifier, intent=merge_held_deep_review` → Tier-3 live in config/alert-translations.json. L1020 (approval_request) Tier-3 silenced at iter ~5054, confirming translation working. systemic_fix appended PRIME ledger 07:58:22Z UTC. Moving to Completed G-rules. Occurrences: iter ~4558 (1/3); iter ~4869 (2/3); iter ~5002 (3/3); dispatched iter ~5002; PR #927 built; Mirror REVIEW_PASS; AUTO_MERGE.

---

## G-rule forge-wip-redispatch-exhausted-genuine-no-pr-001 → DISPATCHED ✅ (iter ~5201), verification_pending

**Rule:** forge-wip-redispatch fires EXHAUSTED when a task has NO PR (genuine no-PR — retry1 also WIP-only). Bot delivers route=escalate to Larry; Pulse journals only, no duplicate DM. Fix: add Tier-3 translation for `source=forge-wip-redispatch` exhausted escalations in alert-translations.json so Pulse's re-triage silences (bot already handles DM). Coordinate with pending `forge-wip-redispatch-digest-tier4-001` fix (which renames exhausted subject to exhausted:{base}). direction-ask-forge-wip-redispatch-exhausted-genuine-no-pr-001.json dispatched to Beacon inbox at iter ~5201. verification_pending. Occurrences: iter ~4657 (1/3, review-sequence-dag-suite-green-guardian); iter ~5002 (2/3, dag-preflight-spec-gauntlet-gate-001); iter ~5201 (3/3, rebase-pr-860-001); iter ~5216 (4th post-dispatch, rebase-enhance-pr945-target-pr-terminal-001 — superseded task, bot DM'd idx=971).

---

## G-rule outbox-notifier-notification-intent-reject-tier4-001 → FORGE DISPATCHED ✅ (iter ~5136), verification_pending

**Rule:** `source=outbox-notifier, kind=notification, intent=reject` alerts classify Tier-4 (novel, no translation match). These are routine Forge-rejection delivery confirmations — outbox-notifier already DMs Larry; Pulse DM is duplicate noise. Fix: add `source=outbox-notifier, intent=reject` → Tier-3 (`severity: INFO, tier: FYI`) in `config/alert-translations.json` (table key is literal `reject` under `outbox-notifier` — intent-only fallback, no subject). **Beacon verified ground-truth (L11375 + L11715 in outbox_notifier.py; no subject field; intent-only lookup confirmed at alert_triage_state.py L643-644) and dispatched to Forge as doc-only preflight (trust policy should auto-approve). verification_pending (Forge PR).** Occurrences: iter ~2810 (1/3); iter ~2810 (2/3); iter ~5136 (3/3, L900 at 18:49:50Z).
...
## G-rule notifier-concurrent-scan-duplicate-review-dispatch-001 → VERIFIED ✅ (PR #847 + PR #918, iter ~5017)

PR #918 (`fix(notifier): block duplicate review when a re-review is queued for the same task`) MERGED 2026-07-11T03:10:41Z UTC (7413b2d8, Larry external review). Second-layer fix building on PR #847 (durable dedup-flag). Together these close the round-0 dispatch race. systemic_fix appended to PRIME ledger iter ~5017. Monitoring for 0 new occurrences. Moving to Completed G-rules at next clean confirmation. Occurrences: iter ~3710 (1/3) through iter ~5016 (14+15, RECONCILE path gg-s3). Fix chain: PR #847 (durable flag) + PR #918 (queue-aware block).
...
## G-rule forge-marker-task-id-mismatch-xii-v1 → COMPLETE ✅ (PR #929, iter ~5069)

PR #929 (`fix(heal-undispatched-pr-review): resolve forge branch to canonical task_id via build-outbox URL match`) MERGED 2026-07-11T09:51:25Z UTC (squash, auto-merged). `heal_undispatched_pr_review.py` now scans build-outbox archive bounded by `RECONCILE_WINDOW_HOURS` for PR-URL match to resolve truncated branch → canonical task_id. Mirror REVIEW_PASS all 4 success criteria + regression gate PASS. systemic_fix appended PRIME ledger 09:59:41Z UTC. Moving to Completed G-rules. Occurrences: iter ~4464 (1/3); iter ~4508 (2/3); iter ~5052 (3/3 dispatch, PR #928 branch truncated).
...
## G-rule build-sequence-advancer-sequence-complete-tier4-001 → CLOSED ✅ (iter ~5051)

**Rule (historical):** Tracked as 1/3 since iter ~4630. At iter ~5051, verified that `config/alert-translations.json` already has `outbox-notifier/sequence-complete` → Tier-3 FYI entry covering this pattern (confirmed when L1012 sequence-complete:spec-gauntlet-gate-001 was triaged). The G-rule was tracking the wrong source (`build-sequence-advancer`) — the alert that actually reaches Pulse's triage is from `source=outbox-notifier` (build-sequence-advancer routes through the notifier). Translation was present all along. No dispatch needed. Closing. First occurrence iter ~4630.
...
## G-rule heal-undispatched-pr-review-claimed-race-fp-001 → VERIFIED ✅ (PR #912 + PR #874, iter ~5031)

**Rule (historical):** healer FP when review was in `.claimed/`. PR #912 fixed `.claimed/` blind spot; PR #874 (`fix(heal-undispatched-pr-review): consult pipeline ground truth before declaring a PR orphaned`) merged 2026-07-11T05:15Z UTC (iter ~5031) — adds multi-signal ground-truth consultation. PR #874's cascade merged cleanly (no FP undispatched-pr-review alerts fired). Verification complete: both fixes live in production. Moving to Completed G-rules. Occurrences: iter ~4864 (PR #903, 2s race); iter ~4960 (PR #905, 2s race); iter ~4977 (PR #910, 3s race).

---

## G-rule heal-unregistered-approval-null-chat-id-001 → COMPLETE ✅ (PR #933, iter ~5132)

PR #933 (`fix(approvals): resolve chat_id from TELEGRAM_ALLOWED_CHAT_IDS at approval-creation time`) MERGED bf46ffaa at 2026-07-11T18:12:34Z UTC. Mirror REVIEW_PASS + AUTO_MERGE. Fix: approval creators now look up chat_id from `TELEGRAM_ALLOWED_CHAT_IDS` at creation time, eliminating null-chat_id delivery failures. systemic_fix appended to PRIME ledger 18:18:05Z UTC. Moving to Completed G-rules. Occurrences: iter ~4865 (1/3); iter ~5094 (2/3); iter ~5122 (3/3, dispatched).
...
## G-rule pulse-check-xiv-tier4-001 — 2/3 (updated iter ~5682)

**Rule:** `source=pulse-check-xiv` alerts classify Tier-4 (novel, no translation match). These are timer-managed periodic Check XIV outputs (over-silence surface + precision digest). outbox-notifier already DMs Larry via route=escalate; Pulse re-triage returning Tier-4 prompts a duplicate DM. Fix at 3/3: add `source=pulse-check-xiv` → Tier-3 (severity: INFO, tier: FYI) to `config/alert-translations.json`. Dispatch to Beacon at 3/3. Occurrences: iter ~5351 (L933-L935, 2026-07-13T11:50:43Z UTC firing); iter ~5682 (L766-L768, 2026-07-20T11:53:28Z UTC firing — same three alert shapes: oversilence:doorbell, oversilence:heal-dashboard-api-sha-drift, digest). Note: both oversilence findings confirmed correct (doorbell=PR #648; heal-dashboard-api-sha-drift=routine deploy auto-restart); no config change needed for those entries.
...
## G-rule outbox-notifier-merge-conflict-manual-rebase-tier4-001 → COMPLETE ✅ (PR #949, iter ~5201)

PR #949 (`chore(alert-translations): classify outbox-notifier merge_conflict_manual_rebase as Tier-3 FYI`) MERGED cb3838f6 at 2026-07-12T02:24Z UTC (Mirror REVIEW_PASS + AUTO_MERGE). Translation `source=outbox-notifier, intent=merge_conflict_manual_rebase` → Tier-3 live in config/alert-translations.json. Moving to Completed G-rules. Occurrences: iter ~4977 (1/3); iter ~5002 (2/3); iter ~5183 (3/3, dispatch).
...
## G-rule RECONCILE_MISSING_REVIEW-.claimed-blindspot → VERIFIED ✅ (iter ~5063)

PR #924 (`fix(outbox-notifier): count .claimed/ reviews as already-dispatched in _review_request_already_dispatched`) MERGED 98f0a140 at 2026-07-11T08:59:37Z UTC. PR #928 healer (`sweep_claimed_orphans()` + round-aware `claim_concluded()`) already live since 01:53:17 MDT. Both code fix + healer now fully live. outbox-notifier restarted at 02:59 MDT with new code; deep-review-hold cleared at 02:59:46 MDT. systemic_fix appended PRIME ledger 09:09:19Z UTC. Moving to Completed G-rules. Occurrences: iter ~4986 (1/3); iter ~5028 (2/3); iter ~5029 (3/3 dispatch); iter ~5036/5039/5051/5052 (4th–7th, post-notifier-restart RECONCILE fires).
...
## G-rule watermark-rotation-gap → CLOSED ✅ (REJECTED by Larry, iter ~5134)

**Rule (historical):** `repair-watermark` fires `repaired=true` when watermark drifts 1 line ahead of file_length (small Pulse set-watermark/append race, NOT the bulk retention job). repair-watermark auto-heals on every iter (always-allowed). **CLOSED: Larry REJECTED the durable-fix invest at 2026-07-11T18:26:22Z UTC** — Beacon REJECT recommendation: "close as already-mitigated; repair-watermark is doing its designed job; the residual 1-line drift is a minor, self-healing edge case." Direction-ask dispatched iter ~5131; Beacon created APPROVAL_REQUEST (decision-001); Larry rejected. 4 total occurrences: iter ~5063 (1/3, wm=1027, fl=1026); iter ~5125 (2/3); iter ~5131 (3/3, wm=897, fl=896); iter ~5134 (4th, wm=899, fl=898 — post-rejection auto-repair). Moving to Completed G-rules.

---

## G-rule sync-push-fail-/dev/stdout-systemd-001 → COMPLETE ✅ VERIFIED ✅ (PR #955, iter ~5218, verified iter ~5219)

PR #955 (`fix(sync): replace /dev/stdout push-log path with temp-file capture for systemd context`) MERGED 4f7fc37a at 2026-07-12T04:30:28Z UTC (Mirror REVIEW_PASS + AUTO_MERGE). Fix: `_lib_push_with_rebase.sh` no longer writes to `/dev/stdout` (unavailable in systemd service context). systemic_fix appended to PRIME ledger iter ~5218. **VERIFIED iter ~5219: ourliberty-sync.service ran 04:31:16Z UTC (48s post-merge); status=success, push_failures=0 confirmed.** G-rule fully closed. Moving to Completed G-rules. Occurrences: iter ~5179 (2 fires MDT 2026-07-11); iter ~5204 (02:50:50Z UTC, 3/3 DISPATCHED).

---

## G-rule pulse-auto-dispatch-null-reply-chat-id → COMPLETE ✅ (PR #950, iter ~5201)

PR #950 (`fix(pulse): resolve reply_chat_id at direction-ask envelope creation (pulse_envelope_builder)`) MERGED 13fb4b07 at 2026-07-12T02:23Z UTC (Mirror REVIEW_PASS + AUTO_MERGE). Fix: pulse_envelope_builder resolves reply_chat_id from TELEGRAM_ALLOWED_CHAT_IDS at direction-ask creation time, eliminating null-reply-chat-id WARNs on pulse-auto-dispatch APPROVAL_REQUEST path. Moving to Completed G-rules. Occurrences: 1st (pre-iter ~5182); 2nd (iter ~5183); 3rd (iter ~5184, dispatch).

---

## Status snapshot — updated 2026-07-20T17:31Z UTC (Iter ~5699, **Tier 3**, consecutive_clean=0)

**Iter ~5699 summary (2026-07-20T17:27Z):** ✅ Nominal. wm=776 (fl=776, 1 new: heal-wedged-review-sessions Tier 3 silence). HEAD=a36c0d3b==origin/main (wrapper: Pulse cycle 20260720T170936Z). inbox_watcher PID 3801575 (~15h42m). beacon PID 3801553 (~15h42m). outbox-notifier PID 3801576 (~15h42m). Zombie PID 1834248 static carry (~52d22h8m). last_sync=2026-07-20T16:51:19Z UTC (no-change, push_failures=0). ratio≈22.87 (trailing-30d). **Tier 3 PROMOTED** (consecutive_clean=0; last_signal_at=2026-07-20T16:13:11Z UTC). [yellow] sync-deploy-targets-missing-registry-001 [1/3] — rsdpm absent from deploy_targets.json; bot DM'd idx=773. [yellow] probe-blind:ourliberty-cycle.service (carry; idx=780). [yellow] dashboard-api-sha-drift-path-aware-restart-001 pending approval — DMed Larry 11:14 MDT 2026-07-20. Check VIII: PR #964 OPEN (fix(pulse): Check VIII stops re-proposing deprecate) — Mirror review in progress (worktree active). 2026-07-13 proposal still pending. Check VI awaiting Larry response (idx=990). Check XIV [2/3] — dispatch at 3/3. G-rule auto-dispatch-APPROVAL_REQUEST-task-id-mismatch: verification_pending (carry). Check I: FIRED Monday 2026-07-20 at 14:14Z UTC — `pulse-cycle-model-downgrade-001` dispatched. Next firing Wed 2026-07-23. Check III: OFF-WEEK — next fire 2026-07-26 04:42:51 MDT. Check I dm_route second-emission-Sunday: 1st occurrence 2026-07-19T14:14Z, monitor Wed 2026-07-23. [NEW G-rule 1/3] heal-pipeline-stall-retry-exhausted-pr-exists-fp-001: stall healer fires retry_exhausted when wip-redispatch exhausts after successful primary build + worktree wedge-reap; PR #964 was FP confirmation.

---

## G-rule heal-pipeline-stall-retry-exhausted-pr-exists-fp-001 — 1/3 (new, iter ~5699)

**Rule:** `heal_pipeline_stall.py` dry-run fires `retry_exhausted:<task>` when the forge outbox archive contains a `.1.json` entry with `result="All retries exhausted"` (wip-redispatch retry failed immediately after primary build session was wedged+reaped). FP class: the PRIMARY build succeeded (PR exists + Mirror review dispatched); the "retry" was the wip-redispatch trying to re-run an already-completed build. Fix: stall healer `check_retry_exhausted` should gate-out when a PR exists for the task. Dispatch to Beacon at 3/3 for code fix in `scripts/heal_pipeline_stall.py`. First occurrence: iter ~5699 (check-viii-suppress-deprecate-when-already-disabled-001; PR #964 OPEN MERGEABLE; build session PID 4022894 wedged 958s → reaped 17:23Z UTC; Mirror review dispatched 11:20 MDT).

---

## G-rule heal-pipeline-stall-forge-reject-no-pr-fp-001 → COMPLETE ✅ (PR #939 + PR #959, iter ~5399)

**Rule:** `heal_pipeline_stall.py` fires `forge_built_no_pr:<task>` for tasks whose archive entry has `result=~"=== REJECT ==="` and `branch=null` (Forge REJECTED at preflight). Two-part fix: (1) add `preflight_reject` skip guard in `check_forge_built_no_pr` (PR #939, MERGED 2026-07-11T22:54:47Z UTC; fix #1 VERIFIED iter ~5170); (2) treat closed-not-merged rebase target PR as valid resolution (PR #959, MERGED 2026-07-13T~20:10Z UTC — 24431ed0; fix#2 live as of iter ~5373). **VERIFIED iter ~5399: stall dry-run shows FORGE_NO_PR_SKIP reason=pr_exists for fix-rebase-closed-pr-reconciliation-001/PR #959 (MERGED); "no stalls detected."** systemic_fix appended PRIME ledger 20:13Z UTC iter ~5373. Moving to Completed G-rules. Occurrences: iter ~5159 (dry-run, `retry1`, REJECT); iter ~5160 (real alert idx=918, Tier-3 silenced); iter ~5163 (dry-run, `retr-retry1`, also REJECT — 3/3).


## G-rule sync-deploy-targets-missing-registry-001 — 1/3 (new, iter ~5693)

**Rule:** `source=sync-deploy-targets, subject^=deploy-targets-sync:MISSING_FROM_REGISTRY:` alerts classify Tier-4 (novel, no translation match). Fires when the sync-deploy-targets script detects a Vercel project connected to the account but absent from `config/deploy_targets.json`. Deploy notifier (E2.2) is blind to pushes for unregistered projects. Bot delivers via route=escalate. Pulse journals only (no double-DM). At 3/3: if the alert is recurring (persistent unregistered project), dispatch direction-ask to Beacon for Tier-3 translation + config fix PR via Forge. First occurrence iter ~5693 (L774, ts=2026-07-20T16:03:19Z UTC, project=rsdpm, prj_Yxqyk19dzUmAfdb0pd6azsimlIcX; bot delivered idx=773 at 10:06:30 MDT).
...
## rsdpm-v0-001 kickoff blocked — BUILD_PLAN.md missing (new, iter ~5850, 2026-07-22T05:59Z)
...
## PRIME ledger interventions count is a 30d rolling window — count decreases are EXPECTED (learned iter ~7107, 2026-08-01)
