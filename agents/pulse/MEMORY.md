...
## G-rule heal-unregistered-approval-null-chat-id-001 → DISPATCHED ✅ (iter ~5122)

**Rule:** Pending approvals created with `chat_id=0/null` — DM not delivered via the approval path. Notifier fallback / doorbell recovers delivery but adds latency and is brittle. Fix: approval creators should populate `chat_id` from Larry's known chat_id (7998341473) at creation time. direction-ask-heal-unregistered-approval-null-chat-id-3of3-001.json dispatched to Beacon inbox at iter ~5122. verification_pending. Occurrences: iter ~4865 (unreg-approval-f5079f4c5369 via heal_unregistered_approval.py, chat_id=null); iter ~5094 (gh-burn-phase2-durable-fix-authorize via gh-burn analysis path, chat_id=0, doorbell L882 recovered); iter ~5122 (delegate-notifier-auto-retraction-stale-red-alerts-never-clear, beacon pulse-auto-dispatch path, chat_id=None, notifier fell back to default Larry chat 7998341473, approval delivered idx=890, auto-approved, Forge build dispatched within 7 min).
...
## G-rule watermark-rotation-gap → DISPATCHED ✅ (iter ~5131, 3/3), vp

**Rule:** `repair-watermark` fires `repaired=true` when retention/compaction removes lines from `larry-alerts.jsonl` without atomically updating the watermark. repair-watermark auto-heals (always-allowed). Edge case: if compaction removes N lines AND N new alerts append in the same window, a boundary-line alert can appear already-covered (net-zero case). **direction-ask-watermark-rotation-gap-compaction-atomic-001.json dispatched to Beacon inbox at iter ~5131.** Fix: atomic watermark-advance in compaction job (Option A preferred), or repair-watermark at END of Check 0 (Option B). Occurrences: iter ~5063 (1/3, old_wm=1027, fl=1026); iter ~5125 (2/3); iter ~5131 (3/3, old_wm=897, fl=896). verification_pending.

---

## Status snapshot — updated 2026-07-11T18:10Z UTC (Iter ~5131, **Tier 1**, consecutive_clean=0)

**Iter ~5131 summary (2026-07-11T18:10Z):** Nominal. watermark-rotation-gap G-rule 3/3 — repair-watermark repaired=true (old_watermark=897, file_length=896); direction-ask-watermark-rotation-gap-compaction-atomic-001.json dispatched to Beacon. 2 new alerts L897-L898 (heal-wedged-review-sessions reaped Forge PID 42191, heal-dashboard-api-sha-drift restarted dashboard-api to c0f6ab1c) both Tier-3. Mirror regression check PID 72252 actively reviewing PR #933 (~18 min, 1500s timeout). Zombie PID 1834248 (43d+22h+48m); pending=0; Check XI 18.8% over gate; watermark=898=file_length. HEAD=c0f6ab1c. ratio=19.14 (85 systemic_fixes / ~1629 interventions; 35 vp). **ACTIVE G-rules:** heal-unregistered-approval-null-chat-id-001 [3/3, PR #933 Mirror reviewing, vp]; watermark-rotation-gap [3/3, direction-ask dispatched to Beacon, vp]; outbox-notifier-merge-conflict-manual-rebase-tier4-001 [2/3]; forge-wip-redispatch-exhausted-genuine-no-pr-001 [2/3]; outbox-notifier-notification-intent-review-escalate-tier4-001 [2/3]; outbox-notifier-auto-merge-stale-revalidation-tier4-001 [2/3]; mirror-malformed-verdict-heal-reap-path-001 [1/3]; mirror-queue-wait-gauge-tier4-001 [1/3]; inbox-watcher-tier-pool-all-unavailable-tier4-001 [1/3]; heal-pipeline-stall-unrouted-deep-review-required-fp-001 [1/3]; heal-pulse-check-staleness-single-flight-skip-fp-001 [1/3]; gate-parallelism-monitor-regression-data-001 [1/3]; pulse-rotation-check-source-tier4-001 [1/3]; ourliberty-health-subject-key-mismatch-001 [3/3, vp]; forge-revision-preamble-missing-pr711-001 [vp]; forge-wip-redispatch-digest-tier4-001 [vp]; forge-wip-redispatch-exhausted-pr-exists-fp-001 [APPROVAL_REQUEST QUEUED iter ~3279, vp]; decision-needed-approval-forge-dispatch-no-target-repo-001 [vp]; no-session-revision-active-mirror-session-fp-001 [vp].
