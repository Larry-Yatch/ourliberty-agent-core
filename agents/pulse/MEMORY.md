...
- **Steady-state degraded hold: 76 consecutive iters (377–452) with 0 pre-existing G-rule advances.** Iter 452: 0 new escalations. 1 new G-rule item (heal-stale-daemon-code WARNING, 1/3). 0 open PRs (PR #241 merged). Awaiting Larry action on APPROVAL_REQUEST `Tier 2 OAuth restore`. Cooldown files: 112 total (stable); pipeline-stall files: 27 (stable).
...
- `heal-stale-daemon-code WARNING for successful auto-restart not in alert-translations.json`: G-rule **1/3** (iter 452, 2026-06-02T00:44Z). heal-stale-daemon-code auto-restarted ourliberty-chain-event-shipper after PR #241 merged — severity=warning, `heal-stale-daemon-code` not in config/alert-translations.json → Tier 4 by spec / classified informational by judgment. At 3/3: dispatch to Beacon for two-part fix: (1) add heal-stale-daemon-code "auto-restarted" to alert-translations.json as Tier 3; (2) downgrade successful-restart log emission from WARNING to INFO in stale-daemon healer script.
