...
## G-rule outbox-notifier-merge-conflict-manual-rebase-tier4-001 → DISPATCHED ✅ (iter ~5183), verification_pending

**Rule:** `source=outbox-notifier, intent=merge_conflict_manual_rebase` alerts classify Tier-4 (novel, no translation match). Bot already DMs Larry with rebase instructions; Pulse double-DM is noise. Fix: add `source=outbox-notifier, intent=merge_conflict_manual_rebase` → Tier-3 (severity: INFO, tier: FYI) entry in `config/alert-translations.json`. direction-ask-merge-conflict-manual-rebase-tier3-001.json dispatched to Beacon inbox at iter ~5183. verification_pending (Forge PR). Occurrences: iter ~4977 (L928, PR #909, 1/3); iter ~5002 (2/3); iter ~5183 (L936, PR #945 post-Mirror-REVIEW_PASS CONFLICTING, 3/3).
...
## G-rule sync-push-fail-/dev/stdout-systemd-001 — 2/3 (new, iter ~5179)

**Rule:** `_lib_push_with_rebase.sh` writes to `/dev/stdout` (lines 118/123/124/133/134) which is unavailable in systemd service context, causing `No such device or address` errors and push failures for `ourliberty-sync.service`. Fires hourly when sync has Pulse runtime files to commit. Rollback preserves repo integrity. Fix: replace `/dev/stdout` redirects with temp-file or variable-capture approach in `_lib_push_with_rebase.sh`. Dispatch to Beacon at 3/3. Occurrences: iter ~5179 (2 fires: 17:01:02 + 17:50:45 MDT 2026-07-11).

---

## G-rule pulse-auto-dispatch-null-reply-chat-id → DISPATCHED ✅ (iter ~5184), verification_pending

**Rule:** Every time Pulse dispatches a direction-ask via `source=pulse` pulse-auto-dispatch path, outbox-notifier logs `has no valid reply_chat_id (got None); falling back to default Larry chat`. The fallback works (DM delivered), but the WARN fires on every pulse-auto-dispatch APPROVAL_REQUEST. Root cause: PR #933 (bf46ffaa, 2026-07-11) fixed `_emit_approval_request()` general path but the pulse-auto-dispatch APPROVAL_REQUEST creation code path was not updated to resolve `reply_chat_id` from TELEGRAM_ALLOWED_CHAT_IDS. Fix: update the pulse-auto-dispatch path in outbox_notifier.py to use the same chat_id resolution as PR #933; add a test. **direction-ask-pulse-auto-dispatch-null-reply-chat-id-001.json dispatched to Beacon inbox at iter ~5184.** verification_pending (Forge PR). Occurrences: 1st (pre-iter ~5182, rebase-pr860 task); 2nd (iter ~5183, 17:34:46 MDT); 3rd (iter ~5184, 18:26:49 MDT, merge-conflict-manual-rebase direction-ask).

---

## Status snapshot — updated 2026-07-12T00:45Z UTC (Iter ~5186, **Tier 1**, consecutive_clean=0)

**Iter ~5186 summary (2026-07-12T00:45Z):** 1 new alert Tier-3 silenced (L939 approval_request delivery confirm fix-pulse-envelope-builder-reply-chat-id-001). PR #131 (dashboard) MERGED 18:44 MDT. PR #946 REVIEW_REVISION → revision-1 dispatched to Forge 18:40 MDT. PR #946 was `Wire run_cycle + run_medic into tier dispatch pool`. fix-pulse-envelope-builder-reply-chat-id-001 APPROVAL_REQUEST pending Larry (DM'd 18:41 MDT). HEAD=d39c32b7=origin/main. sync consecutive_push_failures=2 (/dev/stdout systemd bug [2/3] carry). Zombie PID 1834248 (44d+5h). PR #945 CONFLICTING (rebase_obligation stall imminent). PR #947 Mirror in review. PR #940 chore/* by-design. PR #860 Forge rebase in inbox. systemic_fixes=86, vp=36, ratio=18.93, trend=worsening (stable). **ACTIVE G-rules:** pulse-auto-dispatch-null-reply-chat-id [3/3 DISPATCHED, vp — APPROVAL_REQUEST fix-pulse-envelope-builder-reply-chat-id-001 pending Larry]; sync-push-fail-/dev/stdout-systemd-001 [2/3]; outbox-notifier-merge-conflict-manual-rebase-tier4-001 [3/3 vp, Forge dispatched]; heal-pipeline-stall-forge-reject-no-pr-fp-001 [fix#1 VERIFIED, fix#2 PR#945 CONFLICTING, vp]; outbox-notifier-notification-intent-reject-tier4-001 [3/3 vp]; forge-wip-redispatch-exhausted-genuine-no-pr-001 [2/3]; outbox-notifier-notification-intent-review-escalate-tier4-001 [2/3]; outbox-notifier-auto-merge-stale-revalidation-tier4-001 [2/3]; mirror-malformed-verdict-heal-reap-path-001 [1/3]; mirror-queue-wait-gauge-tier4-001 [1/3]; inbox-watcher-tier-pool-all-unavailable-tier4-001 [1/3]; heal-pipeline-stall-unrouted-deep-review-required-fp-001 [1/3]; heal-pulse-check-staleness-single-flight-skip-fp-001 [1/3]; gate-parallelism-monitor-regression-data-001 [1/3]; pulse-rotation-check-source-tier4-001 [1/3]; ourliberty-health-subject-key-mismatch-001 [3/3, vp]; forge-revision-preamble-missing-pr711-001 [vp]; forge-wip-redispatch-digest-tier4-001 [vp]; forge-wip-redispatch-exhausted-pr-exists-fp-001 [APPROVAL_REQUEST QUEUED iter ~3279, vp]; decision-needed-approval-forge-dispatch-no-target-repo-001 [vp]; no-session-revision-active-mirror-session-fp-001 [vp]. unreviewed-merge-larry-authored-pr-001: REJECTED; Larry adopting label habit (no code fix).
