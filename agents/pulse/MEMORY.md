...
## G-rule forge-marker-task-id-mismatch-xii-v1 → DISPATCHED ✅ (iter ~5052)

**Rule:** Forge derives branch names from task_ids but strips suffix segments (e.g. `-001`), causing task_id mismatch with the envelope. Outbox-notifier dispatches Mirror review under the branch-derived truncated task_id, creating duplicate review files at $0.00 cost. Direction-ask `direction-ask-forge-marker-task-id-mismatch-3of3-001.json` dispatched to Beacon at 07:44Z UTC 2026-07-11 (3/3). Fix recommendation: outbox-notifier canonical-task-id lookup (Approach A: when branch task_id is a strict prefix of known envelope task_id, use canonical). verification_pending. Occurrences: iter ~4464 ('pulse-check-xii-v1' vs 'xii-v1'); iter ~4508 (full-vs-short slug mismatch); iter ~5052 (PR #928 branch='forge/heal-orphaned-mirror-claim-reinject-not-concluded-' vs envelope 'heal-orphaned-mirror-claim-reinject-not-concluded-001').
...
## G-rule RECONCILE_MISSING_REVIEW-.claimed-blindspot → DISPATCHED ✅ (iter ~5029)

**Rule:** `outbox_notifier.py`'s RECONCILE_MISSING_REVIEW path fires unnecessarily when the review task is already in `.claimed/` (claimed by inbox_watcher before the notifier's scan). Notifier checks only `~/agents/inboxes/mirror/` (not `.claimed/`), sees no review file, re-dispatches duplicate. Sister issue to `heal_undispatched_pr_review.py` fixed in PR #912. Direction-ask `direction-ask-reconcile-missing-review-claimed-blindspot-3of3-001.json` dispatched to Beacon inbox at 04:58Z UTC 2026-07-11. verification_pending (Forge code fix in outbox_notifier.py). PR #924 is the fix in Mirror review. `ourliberty-heal-orphaned-mirror-claims.service/timer` (installed 06:00Z UTC 2026-07-11 from commit e82307b2) is a complementary healer. Occurrences: iter ~4986 (1/3, PR #912 note); iter ~5028 (2/3); iter ~5029 (3/3, PR #923 gg-s4, 04:52:15Z); iter ~5036 (4th, PR #924 reconcile; 4th post-dispatch); iter ~5039 (5th, 23:59:49 MDT on outbox-notifier restart).
...
## Status snapshot — updated 2026-07-11T07:51Z UTC (Iter ~5053, **Tier 1**, consecutive_clean=0)

**Iter ~5053 summary (2026-07-11T07:51Z):** 0 new alerts. NEW: PR #924 acquired `deep-review-passed` label + flipped to MERGEABLE since iter ~5052. Pending approval deep-review-hold-pr924-eeadc669 still open — Larry approval needed to release hold. Mirror reviews for PR#927 + PR#928 ACTIVE in slots 0+1 (~29–38 min and ~20–21 min). Zombie PID 1834248 carry. HEAD=d9f87c50=origin/main. ratio=19.951 (carry/82). **ACTIVE G-rules:** RECONCILE_MISSING_REVIEW-.claimed-blindspot [3/3 DISPATCHED ✅, vp — PR #924 HELD+deep-review-passed, PR #928 Mirror active]; outbox-notifier-merge-held-deep-review-tier4-001 [3/3 DISPATCHED ✅, PR #927 active]; forge-marker-task-id-mismatch-xii-v1 [3/3 DISPATCHED ✅, vp]; outbox-notifier-merge-conflict-manual-rebase-tier4-001 [2/3]; forge-wip-redispatch-exhausted-genuine-no-pr-001 [2/3]; outbox-notifier-notification-intent-review-escalate-tier4-001 [2/3]; outbox-notifier-auto-merge-stale-revalidation-tier4-001 [2/3]; mirror-queue-wait-gauge-tier4-001 [1/3]; mirror-malformed-verdict-heal-reap-path-001 [1/3]; heal-unregistered-approval-null-chat-id-001 [1/3]; inbox-watcher-tier-pool-all-unavailable-tier4-001 [1/3]; heal-pipeline-stall-unrouted-deep-review-required-fp-001 [1/3]; heal-pulse-check-staleness-single-flight-skip-fp-001 [1/3]; gate-parallelism-monitor-regression-data-001 [1/3]; pulse-rotation-check-source-tier4-001 [1/3]; ourliberty-health-subject-key-mismatch-001 [3/3, vp]; forge-revision-preamble-missing-pr711-001 [vp]; forge-wip-redispatch-digest-tier4-001 [vp]; forge-wip-redispatch-exhausted-pr-exists-fp-001 [APPROVAL_REQUEST QUEUED iter ~3279, vp]; decision-needed-approval-forge-dispatch-no-target-repo-001 [vp]; no-session-revision-active-mirror-session-fp-001 [vp].
