...
## G-rule sync-push-fail-/dev/stdout-systemd-001 → COMPLETE ✅ (PR #955, iter ~5218)

PR #955 (`fix(sync): replace /dev/stdout push-log path with temp-file capture for systemd context`) MERGED 4f7fc37a at 2026-07-12T04:30:28Z UTC (Mirror REVIEW_PASS + AUTO_MERGE). Fix: `_lib_push_with_rebase.sh` no longer writes to `/dev/stdout` (unavailable in systemd service context). systemic_fix appended to PRIME ledger iter ~5218. Verification pending on next `ourliberty-sync.service` run (heal-stale-daemon-code will restart service with new code on next heartbeat cycle). Moving to Completed G-rules. Occurrences: iter ~5179 (2 fires MDT 2026-07-11); iter ~5204 (02:50:50Z UTC, 3/3 DISPATCHED).
