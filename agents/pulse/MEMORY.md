...
- **heal-pipeline-stall-state.json GONE (iter 116, 2026-05-30).** The monolithic state file `~/agents/state/heal-pipeline-stall-state.json` that iters 113–115 read no longer exists. The healer has been refactored to use `~/agents/state/alert-cooldown/` (individual files per cooldown key, e.g., `warning/heal-pipeline-stall:pipeline-stall:forge-no-pr:real-clr`). Check 3 future scans should use: `ls ~/agents/state/alert-cooldown/warning/heal-pipeline-stall*` rather than reading the old monolithic file. Current active stalls: same as iter 115 — Tier 2 rate_limit SKIPPED variants (forge, beacon-bot) still in cooldown. No new stall alerts.
