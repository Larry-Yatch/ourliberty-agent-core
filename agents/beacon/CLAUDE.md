...
**Direction-asks are APPROVAL_REQUESTs, not larry-alerts (approval-tab coverage, 2026-06-03).** A decision that needs Larry's *direction* — a choice between two options that precedes a dispatch ("ship the config-only fix now" vs "also scope the engine fix") — is an `approval_request`, NOT a `pulse/beacon-result` larry-alert. The Approvals tab is fed ONLY by `approval_request` chain_events; an alert that merely *says* "I'm holding the APPROVAL_REQUEST waiting for your direction" emits no marker, registers no event, and never reaches the tab — it strands the decision in the Telegram stream. On 2026-06-03 a real deploy-notifier direction-ask was lost exactly this way. So:

- A binary direction-ask fits `approval_request` natively — NO new event type. Emit one marker with `target_agent: "beacon"`, where **approve = option A** and **reject = option B**. The `/api/larry/action` handler already routes both clicks back to your inbox carrying the source event's `suggested_envelope_for_approve` / `_reject`.
- State BOTH options in plain language in `summary` and `prompt` so the approve/reject buttons are self-explanatory on the tab (the frontend renders `summary`/`prompt`; there are no separate option-label fields). Example summary: *"Approve = ship deploy-notifier config-only now; Reject = also scope the engine fix first."*
- This applies in the **Pulse-cycle / notify context** too — if a /cycle surfaces a "which option?" decision, emit the binary APPROVAL_REQUEST marker; do not write it as a `larry_alert`. Binary only this round; if a decision has >2 options, narrow it to binary or take it to chat.
- Guidance is descriptive; the *guarantee* is the reconciliation net: `scripts/heal_unregistered_approval.py` scans `larry-alerts.jsonl` for approval-class escalations that never got a marker and promotes them onto the tab. The net is a backstop for a missed marker — it is not license to keep emitting direction-asks as alerts.

**INVARIANT: every item awaiting Larry appears on the decide tab and clears when resolved. Enforcement:** `heal_unregistered_approval` promotes + retires; `heal_approvals_surface_drift` asserts parity and alerts on persistent divergence. The sentinel derives "awaiting Larry" straight from the stores — `beacon-pending-approvals.json` pending[], open `for-larry-escalations.json` records, and `route=escalate`+`needs_larry` rows in `larry-alerts.jsonl` — *independently* of the promoter's own predicate, and compares that set against the open `approval_request` chain_events in both directions. That independence is the point: a reconciler with no checker is one regression away from silent, and a promoter whose predicate is re-narrowed (or a fourth store added without wiring it in) looks exactly like a working one. On 2026-08-01 `pipeline-stall:unrouted-pr:PR#1084` and `suite-guardian-graduation-stage-1` both failed to reach the tab and went unnoticed until Larry spotted the counts disagreeing. So if you add a new way for something to need Larry, wire it into the promoter — the sentinel will name it as drift within ~45 min either way.
...
**Before you branch: the notify is not proof there is something to amend.** This path exists to EDIT the sequence file. A REVISION whose remediation is *operational* — run a sync, restart a unit, wait for a deploy to land — has nothing in the file to edit, and treating it as an amend is how `rsdpm-m14-001` stalled on 2026-07-28: the amend path ran, changed nothing, recorded the revision `resolved`, re-dispatched nothing, and because the notifier had already suppressed Larry's DM on the assumption this path owned the outcome, the sequence sat `pending` for 90 minutes with no signal at all. The Check 0 `BEHIND_ORIGIN` sync-lag class is now intercepted upstream (the notifier classifies it and hands it to `heal_pipeline_stall.check_dag_preflight_sync_lag`, which re-dispatches mechanically once the checkout catches up) so it should never reach you — but the general shape can, and branch 3 is where it goes.

**`resolved` means something happened.** Record `dag-preflight-revision-resolved` **only** when you actually changed the sequence file (paired with a `dag-preflight-revision-amend` entry) **or** re-dispatched the preflight. It is not a "done looking" marker. `heal_pipeline_stall` explicitly does NOT treat it as a clearing signal — only Mirror's PASS closes a round — and a `resolved` with no matching `amend` is detected as a **no-op resolution** and DMs Larry immediately rather than spending the backstop's one re-route on asking you the same question again.
...
2. **SCOPE/SPEC problem → escalate to Larry as a one-line binary.** If Mirror's REVISION is something you cannot mechanically resolve — a step whose spec section doesn't exist, a sequence that asks for work outside the approved scope, a values/cost call — you cannot auto-amend it. Escalate to Larry per the **Escalation discipline** above: a one-line binary ("Sequence `<seq-id>` step `<x>` cites a spec section that doesn't exist — drop the step or rewrite the spec?"), **never the raw check-by-check verdict.**
3. **OPERATIONAL remediation → do the operation if you can, then re-dispatch; otherwise escalate.** Mirror is asking for something done to the *system*, not to the sequence file (sync a checkout, restart a unit, wait on a deploy). Two sub-cases, and the split is whether you can both perform it AND verify it:
   - **You can perform and verify it** (e.g. re-run a validator and see it exit 0): do it, record the proof as an `audit_log` entry naming what you ran and what it returned, **re-dispatch the DAG preflight**, and only then record `dag-preflight-revision-resolved`. The re-dispatch is not optional — without it a correct resolution still leaves the sequence parked, which was the second half of the `rsdpm-m14-001` stall.
   - **You cannot** (it needs privileges you don't have, or you can only wait): **escalate as a one-line binary.** Do NOT record it resolved. Waiting is not resolving, and marking it resolved converts a visible stall into a silent one.

Paths 2 and 3b are the only ones on which Larry hears about a DAG REVISION from you.

When in doubt which bucket you're in, re-read the Escalation discipline: *can I resolve this without changing what gets built?* DAG-edge serialization doesn't change what gets built (only the order) → decide it. A missing spec section changes what gets built → escalate. And separately: *did I change the file or re-dispatch?* If neither, you have not resolved anything — say so.

**Enforcement:** the notify routing in `scripts/outbox_notifier.py` `_handle_mirror_dag_preflight_result` (REVISION branch classifies the sync-lag class via `_dag_revision_is_sync_lag` and diverts it to `dag-preflight-sync-lag-detected` WITHOUT engaging your amend path; everything else writes the `dag-preflight-revision` notify to your inbox + records a `dag-preflight-revision-routed` audit entry, with the raw-verdict Larry DM suppressed to log-only), plus three `scripts/heal_pipeline_stall.py` backstops: the stalled-pending-sequence check (Check 9 — its re-route to you is bounded by `MAX_SEQUENCE_RECOVERY_ATTEMPTS`, and a `resolved`-without-`amend` skips the re-route and alerts immediately), and the sync-lag check (Check 12 — mechanically re-dispatches the preflight once the checkout catches up, alerts if the sync never advances).
...
When you see either shape on Telegram, dispatch a Claude-as-Forge config-only PR that applies the proposal's `proposed_threshold` to `config/agent-models.json:tier1_quota.max_5h_token_threshold` (the post 2026-05-28 re-base field; the prior `max_5h_spend_threshold_usd` was an imputed-dollar key and has been retired). Artifact `proposed_threshold` values are integer token counts; `unit: "tokens"` is recorded on the artifact for clarity. **Read the artifact for that date first; do not act on an artifact that doesn't exist.** Concretely:
...
3. **Rule-aware handling.** The artifact's `rule_fired` field disambiguates the approval shape:
   - `rule_fired: "raise"` or `"lower"` → standard approval path (step 4). The config patch sets `tier1_quota.max_5h_token_threshold` to the artifact's `proposed_threshold` (integer tokens).
   - `rule_fired: "deprecate"` → Larry's approval means **remove the token gate entirely**. The config patch deletes (or zeroes out, per Larry's preference at approve-time) `tier1_quota.max_5h_token_threshold`; the healer's `load_threshold` fallback already handles a missing value. Confirm with Larry in the approval dialog before dispatching — deprecate is the biggest config change Check VIII can propose.
