...
> **AMENDMENT — DECIDED (Larry, 2026-06-16): capture write-back is now ONE-CLICK; promote/drop are no longer PR-backed.** The original design (below) opened a PR per promote/drop that Larry had to squash-merge + delete the branch by hand — the same per-action-PR friction the **+New mission** flow already shed (it now queues the mission for the missions writer; see `_handle_new_mission`) and that snooze never had (§4.3). Drop and promote now follow suit:
> - **`drop`** writes `state: "dropped"` DIRECT through the single `captures.json` committer (exactly like snooze); `heal_missions_card_gc` (the sole `captures.json` git committer) version-controls the delta on its tick. Returns `{ applied: true, state: "dropped" }`.
> - **`promote`** touches two single-committer files, so it splits: it **queues** the new mission entry for the missions writer (`heal_orphan_autoregister` drains `<queue_dir>/<mission_id>.json` into `missions.json` — the +New-mission mechanism) AND flips the capture (`promoted_to`/`state`) on the local `captures.json` (committed by `heal_missions_card_gc`). Returns `{ mission_id, status: "queued", applied: true }`. The mission is queued before the capture is flipped so a crash never loses it; a failed capture flip rolls the queue file back.
>
> Auditability is preserved (the healers' git commits are the audit record — never a silent delete). The board reflects the change on the next poll after the owning healer commits — the same post-merge latency the PR flow had, minus the PR. Mission write-back (§5 defer/resume/reprioritize) and orphan accept/dismiss (§6) **remain PR-backed** — unchanged by this amendment. The original PR-backed contract is retained below as the historical record.

New endpoint: `POST /api/missions/captures/{capture_id}/action`, body `{ "action": "promote" | "drop" | "snooze", ...args }`. Auth: `X-Dashboard-Token` + `X-Actor` (mirrors `/api/larry/action`). ~~Each returns `{ pr_url, branch }` (or `{ applied: true }` for the local-only snooze write — see below).~~ → Per the amendment above, each now returns a one-click result (`{ applied, state }` for drop, `{ mission_id, status: "queued", applied }` for promote, `{ applied, snoozed_until }` for snooze); no `pr_url`.
