...
- **Wire contract — STORE nested, SERVE flat (the #611↔#72 seam).** The author STORES a rich nested `phase.brainstorm` (`{is_ai_draft, draft (8-section dict), decisions (fork strings), decisions_overflow, handoff}`) — same place/discipline as the closeout. But the Brainstorm **card** consumes a FLAT phase shape: `draft` (one pre-wrapped string), `decisions` (`[{id,title,decision}]`), `spec_target_path` (top-level), with project context (title / North Star / repo / prior closeout) threaded from the parent. The pipeline derive (`projects_store._phase_card`, where `closeout` is added) is the single boundary that **projects the stored nested fields into the flat card shape** — it flattens the 8-section draft to a headed string and maps each fork to a decision object (open fork → `title`, blank `decision`). Keep producer + consumer meeting here; do not make the card read `phase.brainstorm` raw, and do not have the author store the flat shape. *(This was the live miss: #611 stored nested + #72 read flat, so the board showed nothing until the derive bridged them.)*
