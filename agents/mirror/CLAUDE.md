...
4b. **Bug-hunt pass (Phase F1 — the gate against escaped bugs).** Step 4's Correctness + Quality are NOT eyeball-only. Run a structured bug-hunt over the diff using the lenses in `/home/larry/agent-core/review/mirror-bughunt-lenses.md` — the eight escaped-bug lenses A–H, plus **Lens I (reuse/reinvention + catalog-on-build), which is ADVISORY ONLY: it queries the ourliberty-graph shelf librarian and yields narrative-only reuse-or-restock notes that NEVER block** (and fail-safe-skips when the librarian/checkout is absent), plus **Lens J (migration safety) — scope-gated to diffs shipping SQL a live database will run, and unlike Lens I it DOES block** — consulting the known-bug-patterns corpus at `/home/larry/agent-core/review/known-bug-patterns.json`. **Read both by ABSOLUTE path (like `marker.py`), NOT from your worktree** — your review worktree is a checkout of the *target* repo, and for an `ourliberty-dashboard` PR that checkout does NOT contain `review/`. For each lens, pull the corpus entries whose `review_lens` matches and test the diff against their `detection_signature`. **Read surrounding code / call sites where a lens calls for it (seam, concurrency, identifier-match, path-traversal) — do NOT review the hunk in isolation;** the cross-file flow is where these bugs hide. This pass exists because 64 correctness/reliability/data-loss/security bugs once passed review (`AUDIT_main_20260605.md`); the corpus is distilled from exactly those, and a backtest showed this pass catches ~89% of them vs. ~0% for unaided review.
   - **Routing (safety-first posture — block readily on the higher-severity classes):**
     ...
     - **Lens J (migration safety) — routes per its own section in the lenses doc**, and it is the one place the halt bullet above needs reading carefully. A rulebook or ordering fault (missing RLS, unhardened definer, an object a later migration also redefines) is inline-fixable → **REVIEW_REVISION**. Irreversible loss of existing data, or a blast radius you could not quantify, → **REVIEW_ESCALATE** carrying the verdict block verbatim: a deliberately destructive migration is not a bug Forge can fix, it is a question only Larry can answer, and escalation is the path that asks him. Reserve **REVIEW_EMERGENCY_HALT** for a migration targeting a **production** database or deleting **client content** (transcripts, meeting notes). Do not let the "destructive operation the diff performs" bullet swallow the whole class once apply-on-merge lands — halting stops the pipeline and tells nobody anything. **Lens J's verdict block goes in the narrative on every migration PR, including a `REVIEW_PASS`** — a clean migration Larry can read in plain words is the point, not just a blocked one.
...
- **`task_id` in the marker payload MUST match the envelope's `task_id` EXACTLY.** Including on retries. If the envelope says `task_id: "review-pr-89-e4-2-spec-doc-retry"`, your marker payload's `task_id` field must be that same string — NOT the original pre-retry `task_id`. The notifier validates marker.task_id == envelope.task_id and dead-letters on mismatch (`MalformedMirrorMarker: marker task_id (X) does not match envelope task_id (Y)`). Surfaced 3 times on 2026-05-24 across PR #5, PR #89-retry, PR #95. As defense-in-depth the notifier now auto-normalizes a **whitespace-only** difference (stray padding or a trailing newline on an otherwise identical id) back to the bare envelope id instead of dead-lettering — a safety net, not a license to be sloppy. Note this is deliberately narrower than Forge's version of the same gate: Forge also strips a `forge-`/`forge/` affix, but on a review path any non-whitespace difference could mean the verdict was attached to the WRONG PR, so it still dead-letters.
...
**Which checkout, and which syncer, depends on the sequence's `target_repo`** — the classifier resolves a cross-repo `spec_doc` against that repo's own checkout, so an RSDPM sequence is read from `/home/larry/RSDPM`, not agent-core. The two trees have different keepers, and they are not interchangeable: `ourliberty-sync.service` advances agent-core ONLY, while every other registered checkout is fast-forwarded by `ourliberty-sync-dispatch-repos.service`. Take the remediation from the classifier's own message rather than assuming agent-core's. Run the script-backed classifier and branch on its exit code:
    ...
    Note this command is **not** a pure query: on behind-origin it fast-forwards the target repo's checkout (ff-only; it declines a tree that is dirty, off `main`, or ahead of origin). That is deliberate here — parking a build on a checkout that is one commit stale is the failure it exists to prevent — so **run it plain, without `--no-refresh`**. The `--no-refresh` flag exists for an operator inspecting a checkout they have deliberately parked on an older commit; it is not for you.
    ...
    - **exit 1 (not authored):** flag REVISION with: *"Sequence `<seq-id>` spec_doc `<spec_doc>` is absent from the working copy AND origin/main; author + merge it before re-dispatching the DAG preflight."* **But read the `REFRESH:` line first.** The classifier never fetches on its own — it reads this checkout's local `origin/main` ref — so a spec that merged since the last fetch is invisible in BOTH places and lands here rather than on exit 3. That is exactly how incident 2026-06-10 mis-reported merged PR #415 as "never authored". The refresh therefore runs on exit 1 too, and if it advanced the tree you will simply get exit 0 instead. So read the `REFRESH:` line for **whether the fetch ran**, which is not the same question as whether the tree moved:

        - No `REFRESH:` line at all (agent-core — this path cannot refresh it), or `error — fetch failed …` → the tracking ref may still be stale. Say the spec *could not be confirmed* on origin/main and name what blocked it. Do NOT assert it was never authored.
        - `advanced (+N)` → the pull landed; you will normally get exit 0 instead.
        - `skipped — …` (dirty tree, not on `main`, commits ahead) → **the fetch still ran.** `sync_one` fetches BEFORE it inspects the branch or the working tree, precisely so the classification is honest on every decline path, and only the *merge* was refused. The tracking ref is current, so exit 1 here is trustworthy: the flat "author + merge it" claim is supported.

    In short: a fetch failure or an absent refresh makes exit 1 unconfirmable; a `skipped` merge does not.
